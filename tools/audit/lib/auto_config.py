    ...
    return {
        "memory_safety": [
            {"name": "raw_new", "pattern": "\\bnew\\s+\\w+", "file_glob": "*.cpp,*.h",
             "severity": "high", "description": "Raw new — should use smart pointers (CWE-401)",
             "exclude_pattern": "placement new|nothrow|new Q\\w+\\(|new Q\\w+\\s*\\{|"
                                "QVBoxLayout|QHBoxLayout|QGridLayout|QFormLayout|"
                                "QAction|QMenu|QLabel|QWidget",
             "skip_comments": True},
            {"name": "raw_delete", "pattern": "\\bdelete\\s+", "file_glob": "*.cpp,*.h",
             "severity": "high", "description": "Raw delete — should use smart pointers (CWE-401)",
             "skip_comments": True},
            {"name": "null_macro", "pattern": "\\bNULL\\b", "file_glob": "*.cpp,*.h",
             "severity": "low", "description": "NULL macro — use nullptr"},
            {"name": "c_style_cast",
             "pattern": "\\(\\s*(int|float|double|char|void|unsigned|long|short|size_t|"
                        "uint\\d+_t|int\\d+_t|ptrdiff_t|ssize_t|GLuint|GLint|GLfloat|GLenum|GLsizei)\\s*\\*?\\s*\\)",
             "file_glob": "*.cpp,*.h", "severity": "medium",
             "description": "C-style cast — use static_cast/reinterpret_cast (CWE-704)",
             "exclude_pattern": "static_cast|reinterpret_cast|const_cast|dynamic_cast",
             "skip_comments": True},
            {"name": "malloc_usage", "pattern": "\\b(malloc|calloc|realloc)\\s*\\(",
             "file_glob": "*.cpp,*.h", "severity": "high",
             "description": "C-style allocation — use containers or smart pointers (CWE-401)",
             "skip_comments": True},
            {"name": "fopen_no_raii", "pattern": "\\bfopen\\s*\\(",
             "file_glob": "*.cpp,*.h", "severity": "medium",
             "description": "C-style file I/O — use std::ifstream/ofstream or RAII wrapper (CWE-772)",
             "skip_comments": True},
            {"name": "use_after_move", "pattern": "std::move\\s*\\(\\s*(\\w+)\\s*\\)",
             "file_glob": "*.cpp", "severity": "high",
             "description": "std::move — verify object not used after move (CWE-416)",
             "skip_comments": True},
        ],
        "buffer_overflow": [
            {"name": "unsafe_gets", "pattern": "\\bgets\\s*\\(",
             "file_glob": "*.cpp,*.h,*.c", "severity": "critical",
             "description": "gets() is inherently unsafe — use fgets or std::getline (CWE-242)",
             "skip_comments": True},
            {"name": "unsafe_strcpy", "pattern": "\\bstrcpy\\s*\\(",
             "file_glob": "*.cpp,*.h,*.c", "severity": "high",
             "description": "strcpy without bounds — use strncpy or std::string (CWE-120)",
             "skip_comments": True},
            {"name": "unsafe_strcat", "pattern": "\\bstrcat\\s*\\(",
             "file_glob": "*.cpp,*.h,*.c", "severity": "high",
             "description": "strcat without bounds — use strncat or std::string (CWE-120)",
             "skip_comments": True},
            {"name": "unsafe_sprintf", "pattern": "\\bsprintf\\s*\\(",
             "file_glob": "*.cpp,*.h,*.c", "severity": "high",
             "description": "sprintf without bounds — use snprintf (CWE-120)",
             "skip_comments": True},
            {"name": "unsafe_vsprintf", "pattern": "\\bvsprintf\\s*\\(",
             "file_glob": "*.cpp,*.h,*.c", "severity": "high",
             "description": "vsprintf without bounds — use vsnprintf (CWE-120)",
             "skip_comments": True},
            {"name": "unsafe_scanf", "pattern": "\\b(scanf|sscanf|fscanf)\\s*\\(",
             "file_glob": "*.cpp,*.h,*.c", "severity": "medium",
             "description": "scanf family — verify %s has width limit (CWE-120)",
             "skip_comments": True},
            {"name": "format_string", "pattern": "\\b(printf|fprintf|syslog)\\s*\\(\\s*[a-zA-Z_]",
             "file_glob": "*.cpp,*.h,*.c", "severity": "high",
             "description": "Format function with non-literal format string (CWE-134)",
             "exclude_pattern": "\\b(printf|fprintf|syslog)\\s*\\(\\s*(stderr|stdout|LOG_)",
             "skip_comments": True},
            {"name": "off_by_one_size",
             "pattern": "<=\\s*\\w+\\.(size|length|count)\\s*\\(",
             "file_glob": "*.cpp,*.h", "severity": "medium",
             "description": "Off-by-one: <= with .size()/.length() — should be < (CWE-193)",
             "skip_comments": True},
        ],
        "security": [
            {"name": "path_traversal",
             "pattern": "(QDir|QFile|std::ofstream|std::ifstream|fopen).*\\+.*"
                        "(?:Id|Name|Path|name|path|id|file|dir)",
             "file_glob": "*.cpp", "severity": "high",
            ...
            {"name": "command_injection",
             "pattern": "(?:system|popen|exec[lv]p?)\\s*\\(.*\\+|"
                        "QProcess.*start.*\\+|"
                        "\"(?:cd|rm|mv|cp|sh|bash)\\s.*\\+",
             "file_glob": "*.cpp", "severity": "high",
             "description": "Shell command from string concatenation — command injection risk",
             "skip_comments": True},
            ...
            {"name": "decompression_bomb",
             ...
             "file_glob": "*.cpp", "severity": "high",
             "description": "Decompression without size limit — decompression bomb (CWE-409)",
             "skip_comments": True},
            {"name": "socket_permissions",
             "pattern": "\\b(?:listen|bind)\\s*\\(.*(?:Unix|Local|AF_UNIX)",
             "file_glob": "*.cpp", "severity": "medium",
             "description": "Unix socket — verify restrictive file permissions (CWE-732)",
             "skip_comments": True},
            {"name": "unsafe_deserialization",
             "pattern": "\\b(?:luaL_dofile|luaL_loadfile|luaL_loadbuffer)\\b",
             "file_glob": "*.cpp", "severity": "high",
            ...
            {"name": "predictable_temp",
             ...
             "file_glob": "*.cpp", "severity": "medium",
             "description": "Predictable temp path — use mkstemp/QTemporaryFile (CWE-377)",
             "skip_comments": True},
        ],
        "best_practices": [
            {"name": "deprecated_auto_ptr", "pattern": "\\bstd::auto_ptr\\b",
             "file_glob": "*.cpp,*.h", "severity": "high",
             "description": "std::auto_ptr removed in C++17 — use std::unique_ptr",
             "skip_comments": True},
            {"name": "deprecated_random_shuffle", "pattern": "\\bstd::random_shuffle\\b",
             "file_glob": "*.cpp,*.h", "severity": "high",
             "description": "std::random_shuffle removed in C++17 — use std::shuffle with <random>",
             "skip_comments": True},
            {"name": "deprecated_bind_helpers",
             "pattern": "\\bstd::(bind1st|bind2nd|unary_function|binary_function)\\b",
             "file_glob": "*.cpp,*.h", "severity": "medium",
             "description": "Removed in C++17 — use lambdas or std::bind",
             "skip_comments": True},
            {"name": "deprecated_throw_spec", "pattern": "\\)\\s*throw\\s*\\(",
             "file_glob": "*.cpp,*.h", "severity": "medium",
             "description": "Dynamic exception spec deprecated — use noexcept",
             "skip_comments": True},
            {"name": "throw_in_destructor", "pattern": "~\\w+\\s*\\([^)]*\\)[^{]*\\{[^}]*\\bthrow\\b",
             "file_glob": "*.cpp,*.h", "severity": "high",
             "description": "throw in destructor — calls std::terminate during stack unwinding",
             "skip_comments": True},
            {"name": "catch_by_value",
             "pattern": "catch\\s*\\(\\s*(?:std::)?\\w+(?:::\\w+)*\\s+\\w+\\s*\\)",
             "file_glob": "*.cpp,*.h", "severity": "medium",
             "description": "Catch by value — use const& to avoid object slicing",
             "exclude_pattern": "catch\\s*\\(\\s*\\.\\.\\.\\s*\\)",
             "skip_comments": True},
            {"name": "raw_mutex_lock", "pattern": "\\.lock\\s*\\(\\s*\\)",
             "file_glob": "*.cpp,*.h", "severity": "medium",
             "description": "Raw mutex lock — use std::lock_guard or std::scoped_lock (CWE-667)",
             "exclude_pattern": "lock_guard|scoped_lock|unique_lock",
             "skip_comments": True},
            {"name": "mutable_static",
             "pattern": "static\\s+(?!const\\b)(?!constexpr\\b)(?!thread_local\\b)\\w+\\s+(?!mutex\\b)\\w+\\s*=",
             "file_glob": "*.cpp", "severity": "medium",
             "description": "Non-const static variable — potential data race if multithreaded (CWE-362)",
             "skip_comments": True},
        ],
        "performance": [
            ...
            {"name": "pass_string_by_value",
             "pattern": "\\(\\s*std::string\\s+\\w+",
             "file_glob": "*.cpp,*.h", "severity": "low",
             "description": "std::string by value — consider const& to avoid copy",
             "exclude_pattern": "operator=|operator<<|swap",
             "skip_comments": True},
            {"name": "pass_vector_by_value",
             "pattern": "\\(\\s*std::vector<[^>]+>\\s+\\w+",
             "file_glob": "*.cpp,*.h", "severity": "low",
             "description": "std::vector by value — consider const& to avoid copy",
             "exclude_pattern": "operator=|swap",
             "skip_comments": True},
            {"name": "size_eq_zero",
             "pattern": "\\.size\\(\\)\\s*==\\s*0",
             "file_glob": "*.cpp,*.h", "severity": "info",
             "description": ".size() == 0 — use .empty() instead"},
        ],
        "code_quality": [
        ...
        "secrets": [
            {"name": "hardcoded_password",
             "pattern": "(?i)(password|passwd|pwd|secret|api_key|apikey|auth_token)\\s*=\\s*\"[^\"]{4,}\"",
             "file_glob": "*.cpp,*.h,*.py,*.js,*.ts,*.java,*.go,*.rs",
             "severity": "critical",
             "description": "Hardcoded credential — use environment variable or config (CWE-798)",
             "exclude_pattern": "example|placeholder|xxx|test|TODO|dummy",
             "skip_comments": True},
            {"name": "private_key_header",
             "pattern": "-----BEGIN\\s+(RSA\\s+|EC\\s+|DSA\\s+)?PRIVATE\\s+KEY-----",
             "file_glob": "*", "severity": "critical",
             "description": "Private key in source — must not be committed (CWE-321)"},
            {"name": "aws_access_key", "pattern": "AKIA[0-9A-Z]{16}",
             "file_glob": "*", "severity": "critical",
             "description": "AWS access key — rotate immediately (CWE-798)"},
            {"name": "github_token", "pattern": "ghp_[a-zA-Z0-9]{36}",
             "file_glob": "*", "severity": "critical",
             "description": "GitHub personal access token — rotate immediately (CWE-798)"},
            {"name": "generic_api_key",
             "pattern": "(?i)(api[_-]?key|access[_-]?token)\\s*[=:]\\s*[\"'][a-zA-Z0-9]{20,}[\"']",
             "file_glob": "*", "severity": "high",
             "description": "Possible API key/token in source (CWE-798)",
             "exclude_pattern": "example|placeholder|test|dummy|xxx",
             "skip_comments": True},
        ],
    }
...
def _python_patterns() -> dict:
    return {
        "security": [
            ...
            {"name": "yaml_unsafe_load", "pattern": "yaml\\.load\\(",
             "file_glob": "*.py", "severity": "medium",
             "description": "yaml.load without SafeLoader — code execution risk",
             "exclude_pattern": "SafeLoader|safe_load", "skip_comments": True},
            {"name": "path_traversal", "pattern": "os\\.path\\.join.*(?:request|input|param|arg|name)",
             "file_glob": "*.py", "severity": "high",
             "description": "Path construction from user input — verify sanitization",
             "skip_comments": True},
            {"name": "sql_injection", "pattern": "(?:execute|cursor).*(?:format|%s.*%|f\")",
             "file_glob": "*.py", "severity": "high",
             "description": "SQL with string formatting — use parameterized queries",
             "skip_comments": True},
            {"name": "temp_file", "pattern": "tempfile\\.mktemp\\(",
             "file_glob": "*.py", "severity": "medium",
        ...
        "best_practices": [
            {"name": "mutable_default_arg",
             ...
             "skip_comments": True},
            {"name": "wildcard_import", "pattern": "from\\s+\\w+\\s+import\\s+\\*",
             "file_glob": "*.py", "severity": "high",
             "description": "Wildcard import — pollutes namespace, use explicit imports"},
            {"name": "shadow_builtin",
             "pattern": "^\\s*(list|dict|set|type|id|input|open|print|range|map|filter|"
                        "format|hash|len|max|min|sum|str|int|float|bool|tuple|bytes|object)\\s*=",
             "file_glob": "*.py", "severity": "medium",
             "description": "Variable shadows Python builtin"},
            {"name": "eq_none", "pattern": "==\\s*None\\b",
            ...
            {"name": "assert_in_production",
             ...
             "description": "assert removed with -O flag — use explicit validation for non-test code",
             "exclude_pattern": "test_|_test\\.py|conftest\\.py"},
        ],
        "code_quality": [
            ...
            {"name": "print_debug", "pattern": "\\bprint\\s*\\(",
             ...
             "exclude_pattern": "test_|__main__|cli\\.py"},
        ],
    }
    ...
    return {
        "security": [
            ...
            {"name": "child_process_exec",
             ...
             "file_glob": "*.js,*.mjs", "severity": "high",
             "description": "Command execution with concatenation — injection risk",
             "skip_comments": True},
            {"name": "path_traversal",
             "pattern": "path\\.(?:join|resolve).*(?:req\\.|params|query|body)",
             "file_glob": "*.js,*.mjs", "severity": "high",
             "description": "Path from user input — verify sanitization",
             "skip_comments": True},
            {"name": "prototype_pollution",
             "pattern": "\\[\\s*(?:key|prop|name|k)\\s*\\]\\s*=",
             "file_glob": "*.js,*.mjs", "severity": "medium",
             "description": "Dynamic property assignment — prototype pollution risk",
             "exclude_pattern": "Map|Set|WeakMap", "skip_comments": True},
        ],
        "code_quality": [
