...
8. **Always use the latest version of every dependency; older pins require a written reason.** Every dependency (FetchContent, vendored single-header, system package, CI action) must be on its latest stable release. An older pin is only allowed when the newer version provably breaks the build or introduces an incompatibility — in which case the reason is documented in `THIRD_PARTY_NOTICES.md` beside the relevant row, or inline next to the `FetchContent_Declare`. "We haven't tested it yet" is not a valid reason. Every dependency upgrade gets a cold-eyes review (fresh subagent, no authoring context).
