...
1. **Research → design → review → code.** New features/phases: web research, then `docs/phases/phase_NN_<topic>_design.md` (architecture, API, steps, performance, accessibility, CPU/GPU placement) with cited sources, then user review (blocking, not a formality), then implementation. This is the project's verify-step format.
