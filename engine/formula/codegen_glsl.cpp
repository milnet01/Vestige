...
// Collapse a free-text field to a single line for safe embedding in a GLSL
// `//` comment. description/source/name come from the (possibly untrusted)
// library JSON; a `\n`/`\r` would terminate the comment and turn the
// remainder into emitted code. Replace CR/LF with a space (AUDIT.md §H11
// comment-injection sibling to the identifier validation in
// FormulaDefinition::fromJson).
...
// Format the accuracy/R² provenance value. Canonical closed-form formulas
// ship accuracy=1.0 ("exact closed form, not a fit"); the source string
// disambiguates the one tunable template.
std::string formatAccuracy(float accuracy)
{
    std::ostringstream oss;
    oss << std::setprecision(6) << accuracy;
    return oss.str();
}
                                            ...
                                            QualityTier tier)
{
    ...
    // Provenance comment (3D_E-0012): name — description, then source + R².
    // Always on so a generated shader self-documents which formula and fit
    // produced it. Free-text fields are CR/LF-sanitised so they cannot break
    // out of the `//` comment into emitted code.
    out << "// " << sanitizeComment(formula.name) << " — "
        << sanitizeComment(formula.description) << "\n";
    if (!formula.source.empty())
    {
        out << "// source: " << sanitizeComment(formula.source)
            << "   R²: " << formatAccuracy(formula.accuracy) << "\n";
    }
                                       ...
                                       const std::string& fileBanner)
{
    ...
    // Optional provenance banner (tool version + source-library hash),
    // composed by the --export-glsl verb (tools layer).
    if (!fileBanner.empty())
        out << fileBanner;
    out << "\n";
