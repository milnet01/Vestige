...
public:
    ...
    /// @brief Number of distance constraints (stretch + shear) generated by initialize().
    uint32_t getConstraintCount() const { return m_constraintCount; }

    /// @brief Number of colours produced by greedy graph colouring of the constraint graph.
...
private:
    ...
    // Constraint graph + colouring (rebuilt on initialize, reused per frame).
