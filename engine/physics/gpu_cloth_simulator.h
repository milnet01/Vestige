...
private:
    ...
    // CPU mirror — Step 2 returns this directly. Later steps make it a lazy
    // readback target populated by glGetNamedBufferSubData when callers ask.
    std::vector<glm::vec3> m_positionMirror;
    std::vector<glm::vec3> m_normalMirror;
