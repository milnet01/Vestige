...
#include "renderer/camera.h"                // viewer for the shadow caster's LOD origin (3D_E-0042)
...
#include "renderer/scoped_cull_face.h"      // two-sided blade casting (3D_E-0042)
...
bool GrassRenderer::init(const std::string& assetPath)
{
    ...
    // Shadow caster (3D_E-0042): the SAME vertex shader, so the cast blade is generated by
    // the same code as the drawn blade — only the fragment stage differs (depth + RSM flux,
    // no shading). A caster failing to load is not fatal: the field still draws and receives.
    if (!m_shadowShader.loadFromFiles(assetPath + "/shaders/grass.vert.glsl",
                                      assetPath + "/shaders/grass_shadow.frag.glsl"))
    {
        Logger::warning("GrassRenderer: grass shadow-caster shaders failed to load — "
                        "the field will not cast shadows");
    }
...
std::size_t GrassRenderer::drawField(Shader& shader,
                                     ...
                                     float maxDistance)
{
    shader.setMat4("u_viewProjection", clipFromWorld);
    shader.setVec3("u_cameraPos", cameraPos);
    shader.setFloat("u_clumpCellSize", m_config.clumpScale);
    shader.setFloat("u_clumpStrength", m_config.clumpStrength);

    // --- G4 wind (§5.4): the VS bends each blade in the wind direction; strength 0 ⇒ calm.
    shader.setMat4("u_view", view);
    shader.setFloat("u_time", time);
    shader.setVec2("u_windDir", windDir);
    ...
    shader.setFloat("u_lodNearMid", m_lodBands.nearMid);
    shader.setFloat("u_lodMidFar", m_lodBands.midFar);
    shader.setFloat("u_lodBandWidth", m_lodBands.bandWidth);
    shader.setFloat("u_lodNearFrac", m_lodBands.nearFraction);
    shader.setFloat("u_lodMidFrac", m_lodBands.midFraction);
    shader.setFloat("u_lodFarFrac", m_lodBands.farFraction);
    shader.setFloat("u_lodRankBand", m_lodBands.rankBand);
    ...
    std::size_t drawn = 0;
    const FrustumPlanes planes = extractFrustumPlanes(clipFromWorld);
    for (const GrassChunk& c : m_chunks)
    {
        const AABB box{c.aabbMin, c.aabbMax};
        if (!isAabbInFrustum(box, planes))
        {
            continue;   // outside the view (or, in the shadow pass, the light) frustum
        }
        ...
        // Shadow cast radius (3D_E-0042). A radial distance is >= the view-space depth the
        // cascade split is expressed in, so this under-casts a little at the frustum edges —
        // where cascade 0's shadow is about to be replaced by cascade 1's anyway.
        if (dNear > maxDistance)
        {
            continue;
        }
        ...
        shader.setUInt("u_baseOffset", c.baseOffset);
        shader.setInt("u_segments", lod.segments);
        shader.setFloat("u_chunkCount", static_cast<float>(c.count));
                           ...
                           CascadedShadowMap* csm)
{
    ...
    // so the GPU grass and billboard foliage share one sun/shadow look.
    ...
    m_drawnChunks = drawField(m_shader, viewProjection, view, cameraPos, time,
                              std::numeric_limits<float>::max());
}

void GrassRenderer::renderShadow(const Camera& camera,
...
{
    if (!m_initialized || m_bladeCount == 0 || m_shadowShader.getId() == 0)
    {
        return;
    }
    ...
    // Blades are two-sided ribbons (design §5.1) — a back-facing blade must still cast, or
    // half the field's shadows vanish depending on sun azimuth. RAII restores the shadow
    // pass's prior cull state so the caller stays composable (same guard TreeRenderer uses).
    ScopedCullFace cullGuard{false};

    // The chunk cull runs against the LIGHT frustum (lightSpaceMatrix) while the LOD still
    // keys on the CAMERA, so a chunk behind the viewer can cast into view but every cast
    // blade is exactly the blade the viewer would see.
    drawField(m_shadowShader, lightSpaceMatrix, camera.getViewMatrix(),
              camera.getPosition(), time, maxCastDistance);
}
