...
- **Key bindings.** The settings file's `controls.bindings` saves each rebind
  against its action's string `id` (`engine/input/input_bindings_wire.h`).
  Renaming or removing an action id is breaking: `applyInputBindings` drops a
  saved binding whose id it does not know, logs a warning, and the key reverts
  to its default. Changing a default binding is breaking too, because a user
  who never rebound a key relies on the default. Adding an action is a PATCH.
...
An entry here looks like a surface and is not, because a user cannot reach it.
It becomes a surface when the roadmap item named in it ships, and this list is
updated in the same change.

- **Visual scripting graphs.** `ScriptingSystem` is never constructed outside
  tests, so no authored graph runs (`3D_E-0627`). The item that switches it on
  is `3D_E-S0042`. `NodeEditor.json`
...
| 4 | 2026-09-25 | 3 | 1 | 0 | 0 | 1 | 1 | New run, armed by 3D_E-0683 giving `ENGINE_VERSION` its rule. Every lane held every question. Q1, two lanes plus a third as an open question, merged: "a release bump moves it with no further edit" was false for the weekly cadence, which tags without editing `project(VERSION)`, so a cadence release would stamp the baseline. Fixed in CODE, not by softening the rule, since the user chose the real version: `release.yml` now resolves the tag before configuring and passes it as `VESTIGE_VERSION_STAMP`, validated in `engine/CMakeLists.txt`. The paragraph now says so, and names `readMetadata` rather than "the loader". Unchecked by any lane, flagged as open: the `settings_migration`, `localization_audit.py` and `perf_gate.py` claims in section 1, for which the packet carried no windows. Loop 2 is owed; the cap for a standard is 3. |
| 5 | 2026-09-25 | 3 | 1 | 0 | 1 | 2 | 2 | Loop 2 of the run armed by 3D_E-0683. Every lane held every question. No finding landed on the armed change: all three lanes checked the `ENGINE_VERSION` paragraph against the serializer, CMake and `release.yml` and confirmed it. The packet now carried windows for `settings_migration`, `localization_audit.py` and `perf_gate.py`, and all three lanes confirmed those claims, closing row 4's open item. Both verified findings sit OUTSIDE the armed change, in `### Not yet a surface`, and both are drift since 2026-09-21. Q1, all three lanes: the Keybindings entry called rebinding unwired, but 3D_E-0628 shipped the rebinding path, so key bindings were already a surface. Moved into section 1 with its rule, checked against `applyInputBindings`, which drops a saved binding whose action id it does not know. Q3, all three lanes (tagged Q1, Q2 and Q3): the scripting entry keyed its trigger to 3D_E-0627, which shipped by withdrawing a claim, so a conformer could not tell whether the trigger had fired. Re-keyed to 3D_E-S0042, the item that switches scripting on. Both fixed as amendments bringing the document into line with shipped code and roadmap state. DISMISSED, three. A Q2 that the roadmap's 0.2.0 line names a break that section 1 classes as a PATCH: the document is consistent and the roadmap has drifted, so it is surfaced rather than fixed here. The user chose to audit 0.2.0's work before relabelling it. Two immaterial ones: "Two kinds of heading" does not count the roadmap's preamble heading, and `perf_gate.py` reads four columns rather than "every field". Roadmap changes in the same commit, by user decision: 3D_E-S0042 moved to 0.5.0, whose line now cites it. SECOND SHARE: 0 of 2 findings fell inside the armed span. The review of 3D_E-0683's change ends here, with nothing found against it. |
