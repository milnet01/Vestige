...
Both sit inside the corrected band, but note the all-sky figure is ~15% above the band's *64-tap end*. **That comparison is retired-harness too and no longer holds**: the same all-sky pass re-takes at ~0.52 ms as an uncontended minimum, which is *below* the 0.6 ms end rather than above it. 3D_E-0657 re-decided which end the reference row gates; see below. The residual is optimism in research § 7's own extrapolation, independent of the sample-count correction: it predicted 0.3 ms for the one-sample case that measures 0.48–0.55 ms. That extrapolation scaled Hillaire's PS4 compute figures by FP32 throughput, and this pass is texture-bandwidth-bound, not compute-bound. **The benchmark gates the band's 64-tap end (0.6 ms), because that is the end that describes what ships (3D_E-0657).** It used to gate the upper bound (1.2 ms), on the same reasoning the volumetric gate uses for its stack total, and that choice was verified by a 128-tap mutation measuring **1.21 ms** and going red (3D_E-0616). **That verification did not survive 3D_E-0626.** It was a median under the retired harness; re-taken as an uncontended minimum on 2026-09-26, the 128-tap mutation measures **0.95–0.97 ms** and the 1.2 ms gate **passed it on all three runs**. A reference gate that cannot see a doubled tap count protects nothing. 0.6 ms is not fitted to that measurement: it is the band's published 64-tap end, written into this table before either re-take, so the shipped pass can fail it. Against it the shipped 64-tap pass measures 0.52–0.53 ms, 12% inside, with the minimum's run-to-run spread about 2%; the 128-tap mutation measures 0.95–0.97 ms and goes red. **The gate now polices one configuration, 64 taps at `renderScale` 1.0 on the RX 6600.** Raising `NUM_SAMPLES` on purpose moves this row's budget with it, to the band's matching point. The mutation is possible at all because the benchmark loads `god_rays.frag.glsl` from disk, so the shader under test is the shipped one. This row is timed at `renderScale` 1.0; what the gather resolution is and is not pinned against is covered once below, under *What the god-ray gate still does NOT cover*.
...
|---|---|---|---|
| God rays, `renderScale` 1.0 | ~0.52 ms | 0.6 ms (was 1.2 ms until 3D_E-0657) | 2.3% |
| Volumetric froxel dispatch | ~0.40 ms | 2.0 ms | 4.0% |
| GI inject dispatch | ~0.10–0.19 ms | 0.4 ms | 56% |

The GI inject spread is a residual recorded rather than fixed. At roughly a tenth of a millisecond, drained every frame, that pass never holds boost clocks and its whole distribution shifts between runs — its minimum tracks its median, so this is power state and not outliers. Its headroom against the 0.4 ms budget is wide enough that the gate still adjudicates, and a tighter figure would need a different measurement shape. **Every other per-pass figure in this section predates this harness and is a single sample; re-taking them is 3D_E-0657.** The 128-tap red above is re-taken (2026-09-26); the no-sky figure, the harness sync overhead and § 11.10's noise figure are not.
...
| High, Ultra — **and an unset or unrecognised value, which resolve to High** | `renderScale` 1.0 | the 0.6 ms reference row |
...
**Where each budget actually gets enforced.** The tier row is the only one with an automated home: `scripts/wintest.sh` reads `VESTIGE_WINTEST_QUALITY_PRESET` (default `medium`) and exports it to the test as `VESTIGE_QUALITY_PRESET`, and runs the Release binary that `scripts/local-ci.sh --windows` builds (`wintest.sh` does not build), so neither guard swallows it on the GTX 1050. **Setting `VESTIGE_QUALITY_PRESET` directly does not reach that run — the script overwrites it**; hold that box to the reference row with `VESTIGE_WINTEST_QUALITY_PRESET=high`. **The other three rows — reference, volumetric and GI inject — have no automated home at all.** The volumetric and GI-inject gates assert only where `budgetsApplyToThisMachine` holds, which is High or Ultra, so they skip on the one box that runs automatically. `scripts/local-ci.sh` exports `LIBGL_ALWAYS_SOFTWARE=1` unconditionally, so the software-renderer guard skips it in the local mirror exactly as it does on GitHub's GPU-less runners — the 0.6 ms reference assertion fires only on a hand-run `ctest` against a real driver on the dev rig, which is how 3D_E-0616's and 3D_E-0657's 128-tap runs were obtained. Recorded because a gate nothing runs automatically looks identical to one that passes.
