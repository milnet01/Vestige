...
| **L5** | Settings integration + language dropdown | S | Bump `kCurrentSchemaVersion` 2 → 3 in `settings.h`; add `Settings::Localization { language: "en" }`; add `migrate_v2_to_v3(json&)` in `settings_migration.{h,cpp}`; add the language-picker UI to the existing Settings menu Accessibility tab. |
