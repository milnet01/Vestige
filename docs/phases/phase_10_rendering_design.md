...
- **Static GI floor (the no-double-count anchor).** `SHProbeGrid` (7 `RGBA16F` 3D textures, units 17–23) supplies baked diffuse irradiance; the scene shader **sets** `ambient = (diffuseIBL + specularIBL) * ao * u_iblMultiplier` (`scene.frag.glsl:1285`, an **assignment**), where `diffuseIBL = kD * irradiance * albedo * u_iblDiffuseScale` (`:1281`) from `evaluateSHGridIrradiance` (`:599-665`). The clearcoat block later reads `ambient` (`:1319`). **R4 adds its dynamic indirect term *after* the `ambient =` assignment (`:1285`)**, layered on the floor — see §11.2 for the energy model that keeps it from double-counting the baked bounce.
