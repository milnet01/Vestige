...
| `OpenAL Soft 1.25.2` (`<AL/al.h>`, `<AL/alc.h>`, `<AL/alext.h>`) | external | Audio device, mixer, source playback, HRTF (`ALC_SOFT_HRTF`), EFX. Pinned via `FetchContent` in `external/CMakeLists.txt`. Upgraded 2026-04-30 from 1.24.1 and 2026-09-25 to 1.25.2 (see §16 change log). |
...
| 2026-09-25 | 1.1.1 | milnet01 | §13 dependency row: OpenAL Soft 1.25.1 → 1.25.2 (3D_E-0673). Fixes an STL-hardening assertion in the reverb effect, which the parametric reverb path uses. Also adds 3D processing to distortion, chorus, flanger, pitch shifter and frequency shifter, none of which the engine uses, and moves exported symbols from protected to default visibility. The public AL headers are unchanged. |
