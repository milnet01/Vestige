...
- **Cache key ≠ canonical path** — known §32 violation. `loadTexture` keys on the *original* `filePath` argument plus `:srgb` / `:linear`. Two callers that pass `models/foo.glb` and `models/./foo.glb` produce two cache entries (sandbox canonicalisation only feeds `Texture::loadFromFile`). CODING_STANDARDS §32 mandates `path::lexically_normal()` for asset-registry keys; current behaviour does not yet comply. Tracked as Open Q4 — fix is a one-line `lexically_normal()` call before the cache lookup, deferred until a callsite actually trips on the duplication.
