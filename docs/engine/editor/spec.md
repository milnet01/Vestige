...
**Async loaders flagged for follow-up (Open Q5):** an `engine/resource/AsyncTextureLoader` is **planned but not yet shipped** (see resource spec §15 Q1). Today the Asset Browser loads thumbnails synchronously on the main thread. When the async loader lands, the editor's design is to subscribe via `EventBus::publish<TextureLoadedEvent>` with the publisher running on the loader's worker thread — the editor's callback would then need to marshal results back to the main thread (no editor-side queue today).
...
| 5 | Asset Browser thumbnails currently load synchronously on the main thread. The planned `AsyncTextureLoader` (resource spec §15 Q1) will publish `TextureLoadedEvent` from a worker thread; when it ships, the editor needs an explicit main-thread marshalling queue (callback won't run on main). | `milnet01` | post-MIT release (Phase 12) |
