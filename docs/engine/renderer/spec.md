...
| `PointShadowMap` | class (RAII) | Depth-cubemap shadow for one point light × 6 faces; max `MAX_POINT_SHADOW_LIGHTS = 2` simultaneous. `engine/renderer/point_shadow_map.h:23` |
...
- Add-light call (`addPointLight` / `addSpotLight`) returns `false` if the per-frame light cap is exceeded (`MAX_POINT_LIGHTS = 8`, `MAX_SPOT_LIGHTS = 4` — see `engine/renderer/light.h:65-66`); the renderer logs a warning at most once per frame to avoid spamming the console.
...
4. **`renderScene(renderData, camera, aspect, …)`.** Steady path:
   ...
   2. **`renderShadowPass(...)`**: switch via `ScopedForwardZ` to forward-Z + `GL_LESS` + clearDepth 1.0; for each cascade (staggered — 1 cascade per frame on a 4-frame cycle for cascades 2/3, full update on cascades 0/1) render shadow-caster meshes plus, when their casters are set, foliage (`m_foliageShadowCaster`), trees (`m_treeShadowCaster`) and — into **cascade 0 only** — GPU grass (`m_grassShadowCaster`, 3D_E-0042; capped at cascade 0's far split so the cast stops where receivers stop sampling that cascade). SDSM near/far bounds (read from the previous frame via `DepthReducer::readBounds`) drive the cascade split. Restore reverse-Z on scope exit.
...
| Aspect | Value |
|--------|-------|
| Allocation pattern | Heap (mostly `std::unique_ptr` from `Renderer::initFramebuffers`) for long-lived GL handles. Reusable per-frame `std::vector`s (`m_culledItems`, `m_sortedTransparentItems`, `m_shadowCasterItems`, `m_cascadeCulledCasters`, `m_scratchFoliageChunks`, `m_instanceBatches`) keep capacity across frames so the hot path does not heap-alloc. |
...
| Peak working set (CPU side) | Single-digit MB for renderer-owned data: `m_prevWorldMatrices` (one mat4 per dynamic entity, capped by scene size), instance-batch map + vector pool, the reusable cull/transparent/shadow vectors. |
...
| Lifetimes | Scene-load (IBL, SH grid, light probes) · engine-lifetime (FBO chain, shaders, SMAA LUTs, sampler fallbacks, color-grading presets) · per-frame (cull / batch vectors, whose capacity is reused rather than reallocated). |
...
| Light cap exceeded (`addPointLight` past 8, `addSpotLight` past 4) | Return `false` + `Logger::warning` (rate-limited to once per frame) | Caller drops the light; visual cue is that some lights are missing. Future fix is a clustered/forward+ light-list. |
...
| Out of GPU memory (`glTextureStorage*` / `glBufferStorage` failure) | `glGetError() == GL_OUT_OF_MEMORY` → `Logger::fatal` | Engine aborts; treat as fatal at init. Steady-state allocation is bounded (the per-frame vectors reuse their capacity rather than reallocating). |
...
- Photosensitive defaults: never raise without an explicit design-doc revision. **`bloomIntensityScale` is a MULTIPLIER, not a ceiling.** `limitBloomIntensity`
  returns `sanitised intensity * limits.bloomIntensityScale` unconditionally when
  safe mode is on (`engine/accessibility/photosensitive_safety.h`), so a
  scene-authored 0.10 uploads as **0.06**, and §5's cold-start default of 0.04
  uploads as 0.024. There is no threshold and no value below which the scale stops
  applying. This bullet previously described a 0.6× *cap* under which 0.10 "stays
  0.10" — anyone writing `std::min(intensity, 0.6f)` from that, or asserting it in
  a safe-mode test, diverges from the shipped helper for every intensity below the
  scale, which is the common case.
...
| 10 | Forward+ clustered light list: 8 point + 4 spot uniform caps (`light.h`, and `scene.frag.glsl`'s matching `const int`) are tight for big interior scenes (Solomon's Temple sanctuary lampstands). Forward+ tiled / clustered light culling is the standard 2025 answer. | milnet01 | Phase 11 entry |
