...
**Cold start (`Renderer::Renderer(EventBus&)` + `loadShaders` + `initFramebuffers` — `engine/renderer/renderer.cpp:121`, `:262`, `:530`):**
...
|---------|-------------------|--------|
| Color-vision filter (protanopia / deuteranopia / tritanopia) | `Renderer::setColorVisionMode` | Final composite multiplies by `colorVisionMatrix(mode)` post-tonemap, pre-gamma. Identity for `Normal`. Reference: Viénot, Brettel, Mollon 1999 (`color_vision_filter.h:25`). |
