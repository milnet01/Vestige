...
|---|------|---------:|-----:|-------|
| W2 | §3.5 GUI — "Discover via PySR" button | medium | large | W1 shipped in 1.10.0. Still needed for W2: (a) process-level cancellation via SIGTERM on the child PID (expose PID from `runDriverCaptured`), (b) incremental stdout streaming so in-progress PySR expressions land in the panel as they're discovered. Panel UX: button + Cancel + progress bar + leaderboard of discovered expressions + "Import as library formula" action. The library-import side is its own challenge — parsing a PySR string like `sin(log(x^2))` into an `ExprNode` means either writing an expression parser or handing the user a "save as template" file dialog. |
