...
TEST(CodegenGlsl, GenerateFunction)
{
    ...
    // 3D_E-0012: provenance comment carries the formula name, source, and R².
    EXPECT_NE(code.find("fresnel_schlick"), std::string::npos);
    EXPECT_NE(code.find("source:"), std::string::npos);
    EXPECT_NE(code.find("R²:"), std::string::npos);
...
TEST(CodegenGlsl, ProvenanceCommentSanitisesNewlines)
{
    // 3D_E-0012 / AUDIT §H11 comment-injection sibling: a newline in a
    // free-text field must not break out of the `//` comment into emitted
    // GLSL. The emitter collapses CR/LF to spaces.
    FormulaDefinition f;
    f.name = "inj_test";
    f.description = "line one\nfloat evil() { return 0.0; }";
    f.source = "src\r\nmore";
    f.inputs = {{"x", FormulaValueType::FLOAT, "", 0.0f}};
    f.output = {FormulaValueType::FLOAT, ""};
    f.expressions[QualityTier::FULL] = ExprNode::variable("x");

    const std::string code = CodegenGlsl::generateFunction(f);
    // The injected function signature must not appear on its own comment-free
    // line: no raw newline survives inside the description/source comments.
    EXPECT_EQ(code.find("\nfloat evil"), std::string::npos);
    EXPECT_EQ(code.find("src\r"), std::string::npos);
}

TEST(CodegenGlsl, GenerateFileEmitsBanner)
{
    // 3D_E-0012: the --export-glsl verb passes a provenance banner (tool
    // version + library hash) the engine codegen can't compute itself.
    auto templates = PhysicsTemplates::createAll();
    std::vector<const FormulaDefinition*> ptrs;
    for (const auto& t : templates) ptrs.push_back(&t);

    const std::string banner = "// Generated by Vestige Formula Workbench v9.9.9\n"
                               "// library hash: deadbeefdeadbeef\n";
    const std::string file =
        CodegenGlsl::generateFile(ptrs, QualityTier::FULL, banner);

    EXPECT_NE(file.find("Workbench v9.9.9"), std::string::npos);
    EXPECT_NE(file.find("library hash: deadbeefdeadbeef"), std::string::npos);
    // Banner sits before the prelude / functions.
    EXPECT_LT(file.find("library hash:"), file.find("float safeDiv("));
    // Default (no banner) omits it — back-compat.
    EXPECT_EQ(CodegenGlsl::generateFile(ptrs).find("library hash:"),
              std::string::npos);
}
