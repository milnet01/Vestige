...
/// @brief SMAA pass 2: blending weights from edge searches and the area/search tables.
///
...
uniform sampler2D u_edgeTexture;   // pass 1 output, bilinear
uniform sampler2D u_areaTexture;   // AreaTex, 160x560 RG8, bilinear
uniform sampler2D u_searchTexture; // SearchTex, 64x16 R8, nearest
...
void main()
{
    // SMAA 1x: subsample indices are zero.
    fragColor = SMAABlendingWeightCalculationPS(v_texCoord, v_pixCoord, v_offset,
                                                u_edgeTexture, u_areaTexture, u_searchTexture,
                                                vec4(0.0));
}
