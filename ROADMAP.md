...
- [ ] **W8 (part 2/2).** Wire streaming music path (`audio_music_stream` has no loader consumer today). Needs an AudioMusicPlayer class that drives the existing `MusicStreamState` chunk pump from a .ogg loader, plus integration with the SceneManager / level-music system. **Design doc shipped 2026-05-18:** `docs/phases/phase_10_audio_music_player_design.md` — covers per-layer streaming pipeline (stb_vorbis pull-data API into AL `Queue/UnqueueBuffers`), API surface for `AudioMusicPlayer`, SceneManager integration via a new `music` envelope section, performance budget (~0.5 ms worst-case at six active layers vs. 16.6 ms frame budget), test plan (10 verify rows, headless-runnable), and CPU/GPU placement rationale (CPU end-to-end). Awaiting blocking review per project rule 1 before code lands.
...
**Launched 2026-04-15.** `v0.1.3-preview` tagged, `milnet01/Vestige` flipped public, GitHub Discussions enabled. Pre-launch checklist (LICENSE / CONTRIBUTING / CODE_OF_CONDUCT / THIRD_PARTY_NOTICES / ASSET_LICENSES / SPDX headers across 703 files / fresh-clone build / asset-licence boundary / VestigeAssets repo split / personal-path scrub / gitleaks + secret-history rewrite / public README / GitHub issue+PR templates / SECURITY.md disclosure section / CI hardening / CMake-matrix CI) all complete.
