...
- [~] **R4.** `ScopedBlendState` + `ScopedCullFace` RAII applied to foliage, water, tree, particle renderers. Replaces hand-rolled enable/disable that assumes caller-state. **Foliage shipped 2026-04-25** (red `1f96cdb`, green `71fb0e9`); water / tree / particle remain. The two new RAII classes ship in `engine/renderer/scoped_blend_state.{h,cpp}` and `engine/renderer/scoped_cull_face.{h,cpp}` following the R3 `ScopedShadowDepthState` template-injectable IO pattern (production GL backend + recording mock for tests). `ScopedBlendState` saves/applies/restores the enable bit + per-channel src/dst factors via `glGetIntegerv(GL_BLEND_SRC_RGB / DST_RGB / SRC_ALPHA / DST_ALPHA)` and `glBlendFuncSeparate`; `ScopedCullFace` saves/restores just the enable bit. `FoliageRenderer::render` (alpha-blend + cull-off path) and `FoliageRenderer::renderShadow` (cull-off path) now use the guards instead of bare `glEnable` / `glDisable` calls + manual restores at end-of-function. The bug closed for foliage: previously the manual restore re-enabled cull-face on the way out regardless of the caller's prior state, which broke any caller that ran with cull off (e.g. editor debug-draw mode). 9 new tests in `tests/test_scoped_blend_state.cpp` (4) + `tests/test_scoped_cull_face.cpp` (5) pin the bracket contracts; the same RAII machinery serves the water / tree / particle rollouts when those land. 2987 / 2988 pass post-fix (1 pre-existing skip unchanged; +9 vs R9's 2978). **Follow-up items**: water-renderer rollout, tree-renderer rollout, particle-renderer rollout — each touches similar bare `glEnable(GL_BLEND)` / `glEnable(GL_CULL_FACE)` patterns and can be done as a focused commit per subsystem now that the RAII is shipped.
