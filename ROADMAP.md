...
- [ ] **W8 (part 2/2).** Wire streaming music path (`audio_music_stream` has no loader consumer today). Needs an AudioMusicPlayer class that drives the existing `MusicStreamState` chunk pump from a .ogg loader, plus integration with the SceneManager / level-music system.
...
- [ ] **Sh4.** GPU cloth wind aerodynamic drag in `cloth_wind.comp.glsl` — per-triangle `0.5·dragCoeff·area·(vRel·n)·n` (current GPU wind is isotropic exponential drag only). `setWindQuality()` stored but not used by shader; three tiers produce the same output on GPU today. **Status 2026-05-02:** still open. Larger than Sh1–Sh3 because per-triangle drag scatters force back to 3 vertices, which on GPU is a write-conflict problem (multiple triangles share vertices) — design doc needs to choose between (a) atomicAdd over int-packed floats, (b) colour-grouped triangle pass à la `cloth_constraints` per-colour dispatch, or (c) per-vertex gather over incident triangles (would need a topology-adjacency SSBO). FULL tier on CPU also caches per-particle FBM noise + per-triangle turbulence factors; the GPU port has to decide whether to recompute on shader or upload the same caches. Per project rule 1 (research → design → review → code) this needs a small `docs/phases/phase_09b_gpu_wind_full_tier.md` design doc with cited sources before implementation, blocking-reviewed by the user. Cl1 (parity harness) only depends on Sh1–Sh3 for the constraints / collision / friction parity loops; the per-triangle-drag parity check can land independently once Sh4 ships.
