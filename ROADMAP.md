...
- 📋 [3D_E-0031] **Meadow realism A — real PBR ground textures on terrain (replace the flat-colour placeholder).**
  ...
  **Layman:** Give the ground a real grass/dirt photo texture instead of flat cartoon green — the biggest single fix for the "too cartoony" look.
  ...
  Progress (2026-07-17): A1 + A2 complete (commits up to A2 part 2). A2 part 2 shipped — the textured ground path now shades with the engine's **real Cook-Torrance GGX** (reuse of scene.frag.glsl's distributionGGX/geometrySmith/geometrySchlickGGX/fresnelSchlick + PI; dielectric F0=0.04). The design's §4.4 Blinn-Phong→GGX Workbench fit was verified numerically infeasible (best-case per-roughness max-err ≈0.245 @ r=0.8 vs the ≤0.05 bar) and dropped by user decision; design revised + re-run through cold-eyes (2 loops, converged — see phase design §13). New GGX-consistency test `tests/test_terrain_ggx_parity.cpp` (GPU runs the actual terrain GLSL vs the ggx_distribution reference + a verbatim-source lock to scene.frag.glsl). Remaining: A3 (detail normals + triplanar), A4 (tiling break-up), A5 (assets/quality tiers/perf + CHANGELOG/licence rows). NOTE: --visual-test surfaces ~10386 pre-existing GL_INVALID_OPERATION in glDrawElements on the meadow — unchanged by this slice (identical count before/after), not GGX-related; worth a separate look.
