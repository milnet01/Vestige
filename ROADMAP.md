...
- ✅ [3D_E-S0001] **FW W5 (cont.) — add a reference-regression spec per library formula for broader regression coverage as the formula library grows. Shipped 2026-06-01: added the 12 remaining *coefficient-bearing* formulas (`gerstner_wave`, `coulomb_friction`, `terminal_velocity`, `ggx_distribution`, `schlick_geometry`, `aces_tonemap`, `spot_cone_falloff`, `bloom_threshold`, `exposure_ev`, `dof_coc`, `height_blend`, `thermal_erosion`) → 22/22 fittable formulas now covered; `test_reference_harness` green. The original "~17" estimate (27 builtins − 10) over-counted: the harness *fits coefficients*, and 5 builtins have zero coefficients (`wind_deformation`, `caustic_depth_fade`, `water_absorption`, `ease_in_sine`, `fast_neg_exp`) — `CurveFitter::fit` early-returns "No coefficients to fit" for them, so they can't carry a reference-regression spec under the current harness. Each spec is auto-discovered (directory glob); degenerate fits (g/Cd ratio, ACES scale-degeneracy, thin-lens alias) assert fit quality only, and nonlinear-rational fits (GGX, Schlick) follow the `aerodynamic_drag` start-on-answer convention. Follow-up shipped 2026-06-02 (v1.18.0): the "0-coefficient evaluation-regression" harness mode now covers the 5 input-only formulas (`wind_deformation`, `caustic_depth_fade`, `water_absorption`, `ease_in_sine`, `fast_neg_exp`) — when a `reference_cases/*.json` carries an `evaluation_points` array the harness evaluates the FULL-tier expression at committed `inputs → expected_output` golden points (derived from each formula's math, default abs tolerance 1e-4) instead of fitting. Harness coverage now 27/27 builtins; `test_reference_harness` green (44 tests).**
  Kind: implement.

- ✅ [3D_E-S0002] **Audit X1 — Phase 3 of the audit self-learning loop: a "propose-fix" layer that, after N runs, emits a markdown file flagging rules whose FP set has an exclude-pattern signature the user hasn't applied yet. Shipped 2026-06-01 (audit v2.19.0): `compute_proposals()` + `render_proposals_markdown()` in `lib/stats.py`; wired into `lib/runner.py` at the stats/suppress boundary; `--propose-fixes-show` CLI flag; `propose_fixes:` config block; 20 tests (all 870 audit tests green). Mines namespace-prefix signatures (`tinygltf::`, `JPH::`) from the FP matched-text set, drops tokens already matched by the existing `exclude_pattern`, prunes redundant specifics when a general prefix is kept. Advisory only — writes `.audit_propose_fixes.md`, never edits config. Scope: tier-2 pattern rules only (the only ones with an `exclude_pattern` knob).**
  Kind: implement.

- ✅ [3D_E-S0003] **Audit/FW X2 — write a unified-pattern doc ("how we built self-learning into two tools with the same observe → act structure"). Shipped 2026-06-01: `docs/research/self_learning_pattern.md` — the observe→act structure, the two implementations (Audit Tool 3-phase, Formula Workbench fit-history + reference harness), five shared invariants (with per-tool Python/C++ idiom differences called out explicitly), a worked "apply it to renderer perf autotuning" example, and a new-subsystem checklist. Cold-eyes reviewed (7 loops to converge — citation-dense doc; loops caught a fabricated API name, version/path errors, a best-fit-vs-similarity mischaracterisation, pipeline-order errors, and several over-generalised "both tools" claims; loop 6 fixed the per-tool-idiom class at the root).**
  Kind: implement.

- 📋 [3D_E-S0004] **FW W9 — cross-formula fit target. The reference harness fits a formula's coefficients only against (a) its *own* curve at canonical coefficients (`synthesizeDataset`, `reference_harness.cpp:220`) or (b) explicit `evaluation_points` (eval-only, no fit). It cannot fit one formula (e.g. a cheap Schlick phase) against a *different* reference formula's curve (e.g. exact Henyey-Greenstein). Surfaced 2026-06-18 by Phase-10 fog slice 11.7 (dropped — `docs/phases/phase_10_fog_design.md` §7). Fix: add an optional `reference_formula` (or `target_outputs`) field so `synthesizeDataset` can source fit targets from a second formula. Low priority — the Schlick case needed neither this nor the swap (no perf pressure + an unreachable accuracy bound), but cross-formula approximation is the general "fit a cheap formula to an expensive reference" pattern for cases where perf *does* matter.**
  Kind: implement.
...
- ✅ [3D_E-0006] **FW (GAP/HIGH): importance-sampling / PDF formula templates.**
  (Shipped 2026-06-17, workbench v1.19.0 — `cosine_hemisphere_pdf`, `ggx_vndf_pdf`, `mis_power_heuristic`; scalar PDFs only, sample-direction routines stay hand-written per design §2.)
...
- ✅ [3D_E-0007] **FW (GAP/MEDIUM): real-time-RT denoiser / temporal-accumulation blend templates.**
  (Shipped 2026-06-17 — `denoise` category: `temporal_alpha`, `edge_stopping_depth/normal/luminance`, `adaptive_sample_count`.)
...
- ✅ [3D_E-0008] **FW (GAP/MEDIUM): Russian-roulette survival-probability template.**
  (Shipped 2026-06-17 — `rr_survival`, engine variant labelled vs the PBRT luminance form.)
...
- ✅ [3D_E-0009] **FW (INTEGRATION/MEDIUM): documented headless GLSL-export path for an external Vulkan project.**
  (Shipped 2026-06-17 — `--export-glsl [<library.json>] --out <dir> [--tier]`, name-sorted/deterministic, strict untrusted-input load.)
...
- ✅ [3D_E-0010] **FW (GAP/LOW): srgb_to_linear / linear_to_srgb output-transform pair.**
  (Shipped 2026-06-17 — `color` category, exact IEC 61966-2-1 FULL + cheap-gamma APPROXIMATE.)
...
- ✅ [3D_E-0011] **FW (workflow/MEDIUM): --self-benchmark over a directory of datasets.**
  (Shipped 2026-06-17 — directory batch, one section per `*.csv`; single-file path byte-identical.)
...
- ✅ [3D_E-0012] **FW (workflow/LOW): emit a provenance comment in generated GLSL.**
  (Shipped 2026-06-17 — always-on per-function name/source/R² + file banner with tool version + FNV-1a-64 library hash. RMSE/AIC are benchmark-only outputs, not on the definition, so out of scope for plain codegen.)
...
- 📋 [3D_E-0022] **FW (GAP/MEDIUM): add an `audio` formula-template category for audio-DSP curves.**
  ...
  Progress (2026-07-01): the `audio` formula-template category shipped (AX4 S1,
  commit 34f1744 — impact_loudness_gain / impact_pitch_scale / aggregate_event_rate)
  and is now consumed at runtime via `engine/audio/procedural/audio_curves.h`
...
Reference: see `ARCHITECTURE.md` §§10–19 and the phase-09 design docs in `docs/phases/phase_09*.md` for the domain-driven architecture this phase formalised. (Earlier drafts cited a `Vestige Overhaul.md`; that file is no longer in the repo — its content lives in the architecture doc and the phase 9 design docs.)
...
- ✅ [3D_E-S0005] **Terrain System — wrap `environment/terrain`, `renderer/terrain_renderer`, brush tools. Owns: heightfield editing, splatmap texturing, collision mesh, LOD, terrain-vegetation integration, road/path system**
  Kind: implement.

- ✅ [3D_E-S0006] **Vegetation System — wrap `environment/foliage_manager`, `environment/density_map`, `renderer/foliage_renderer`, `environment/biome_preset`. Owns: all tree/grass/shrub entities, procedural placement, wind animation (queries EnvironmentForces), LOD, species presets**
  Kind: implement.

- ✅ [3D_E-S0007] **Water & Fluid System — wrap `renderer/water_renderer`, `renderer/water_fbo`, `scene/water_surface`. Owns: ocean/lake/river/puddle entities, surface simulation, rendering (reflections, refractions, caustics), buoyancy (queries EnvironmentForces), shore blending**
  Kind: implement.

- ✅ [3D_E-S0008] **Cloth & Soft Body System — wrap `physics/cloth_simulator`, `physics/cloth_component`, `physics/cloth_presets`, `physics/fabric_material`, `physics/cloth_mesh_collider`, `physics/spatial_hash`. Owns: all cloth entities, XPBD simulation, fabric presets, collision, wind response (queries EnvironmentForces). Performance budget enforced: distant/offscreen cloths auto-reduce quality**
...
- ✅ [3D_E-S0009] **Destruction & Physics System — wrap `physics/fracture`, `physics/deformable_mesh`, `physics/breakable_component`, `physics/dismemberment`, `physics/rigid_body`, `physics/grab_system`, `physics/ragdoll`. Owns: rigid bodies, destructibles, dismemberment, debris, grabbing, joints, ragdoll**
  Kind: implement.

- ✅ [3D_E-S0010] **Character & Animation System — wrap `animation/` (skeletal, state machine, motion matching, IK, facial, lip sync, morph targets), `physics/physics_character_controller`. Owns: animation playback, blending, motion matching, IK, facial animation, character controller, ragdoll transition**
  Kind: implement.

- ✅ [3D_E-S0011] **Particle & VFX System — wrap `renderer/particle_renderer`, `renderer/gpu_particle_system`, `scene/particle_emitter`, `scene/gpu_particle_emitter`, `scene/particle_presets`. Owns: CPU/GPU particles, VFX presets, decals, trails, screen effects**
  Kind: implement.

- ✅ [3D_E-S0012] **Lighting System — wrap all light/shadow/probe/IBL/radiosity code. Owns: all light types, shadow mapping, IBL, light probes, radiosity baking, volumetric light shafts**
  Kind: implement.

- ✅ [3D_E-S0013] **Atmosphere & Weather System — wrap `environment/environment_forces` (already implemented), `renderer/skybox`, `renderer/environment_map`. Extends EnvironmentForces with sky rendering, clouds, time-of-day, fog, weather transitions, lightning**
  Kind: implement.
...
- ✅ [3D_E-S0014] **Audio library integration — OpenAL Soft selected; `engine/audio/audio_engine.{h,cpp}` + `engine/systems/audio_system.{h,cpp}`; dr_libs/stb_vorbis decoders.**
  Kind: implement.
...
- ✅ [3D_E-S0020] **Audio occlusion (raycast-based, material-based transmission) — deferred to Phase 10.**
...
- ✅ [3D_E-S0023] **In-game UI rendering — `engine/ui/sprite_batch_renderer.{h,cpp}` + `UIElement` hierarchy (`UICanvas`, `UIImage`, `UILabel`, `UIPanel`) + `engine/systems/ui_system.{h,cpp}`; sprite shaders `assets/shaders/ui_sprite.{vert,frag}.glsl`. Separate from the ImGui editor overlay.**
  Kind: implement.

- ✅ [3D_E-S0024] **In-world UI — `engine/ui/ui_world_label.{h,cpp}` (3D-anchored floating text with frustum-culled world-to-screen projection via the pure-CPU `ui_world_projection.{h,cpp}` helper) + `ui_interaction_prompt.{h,cpp}` (`UIWorldLabel` subclass with "Press [KEY] to action" formatting + linear distance-based alpha fade between `fadeNear` and `fadeFar`). Nameplates use `UIWorldLabel` directly with a per-frame `worldPosition` setter from game code that follows the entity. 11 unit tests in `tests/test_ui_world_projection.cpp`.**
  Kind: implement.

- ✅ [3D_E-S0025] **Screen-space UI (HUD, minimap, crosshair) — `engine/ui/ui_crosshair.{h,cpp}` (centred plus with configurable arms / gap), `ui_progress_bar.{h,cpp}` (ratio-based fill, clamped), `ui_fps_counter.{h,cpp}` (smoothed EMA + TextRenderer drawing). Minimap deferred (needs render-target + camera-frustum overlay).**
  Kind: implement.

- ✅ [3D_E-S0026] **Menu system (main menu, pause, settings) — `engine/ui/menu_prefabs.{h,cpp}` ships factory functions `buildMainMenu`, `buildPauseMenu`, `buildSettingsMenu` populating a `UICanvas` with positioned widgets per the `vestige-ui-hud-inworld` Claude Design layouts. Widget set: `UIButton` (default/primary/ghost/danger/sm), `UISlider`, `UICheckbox`, `UIDropdown`, `UIKeybindRow` — see `engine/ui/ui_*.{h,cpp}`. `UITheme` widened with the design's full token set (palette + sizing + type + font logical names) and a `UITheme::plumbline()` static for the alternative monastic-minimal register. Settings ships chrome only (header / sidebar / footer); per-game projects append their per-category controls. Nav state machine + signal wiring is per-game integration.**
  Kind: implement.

- ✅ [3D_E-S0027] **UI theming — `engine/ui/ui_theme.h` (`UITheme` struct: bg / text / accent palettes + crosshair / progress-bar sizes + default text scale; `UISystem::getTheme()` returns mutable ref so game projects can override per-field at startup).**
  Kind: implement.

- ✅ [3D_E-S0028] **Input routing — `UISystem::setModalCapture(bool)` for sticky modal capture (pause menu, dialog) + `updateMouseHit()` for cursor-over-interactive-element capture. `wantsCaptureInput()` is the union; game input handlers consult it each frame.**
  Kind: implement.

- ✅ [3D_E-S0029] **Editor: visual UI layout editor, theme editor — `engine/editor/panels/ui_layout_panel.{h,cpp}` shipped as a `Window → UI Layout` ImGui panel. Inspects the element tree of any `UICanvas` passed in (per-element position / size / anchor / visibility / interactivity live-editable) + full color-picker surface over `UITheme` (backgrounds / strokes / text / accent / HUD / sizes). Vellum and Plumbline reset buttons for quick-switch between registers. Follow-up enhancements: drag-place widget palette (needs viewport mouse capture + drag math) and JSON canvas serialisation (needs per-element-type reflection). Both gated on factoring the editor's ImGui viewport out of `editor.cpp`.**
  Kind: implement.

###### AI & Navigation System (Basics)
*Scope: requires Recast/Detour library integration and dedicated design document.*

- ✅ [3D_E-S0030] **Navmesh generation from terrain and static geometry — Recast via `engine/navigation/nav_mesh_builder.{h,cpp}` + `engine/systems/navigation_system.{h,cpp}`.**
  Kind: implement.

- ✅ [3D_E-S0031] **Pathfinding (A* on navmesh) — Detour via `engine/navigation/nav_mesh_query.{h,cpp}`; `NavAgentComponent` for per-entity agents.**
  Kind: implement.

- ✅ [3D_E-S0032] **Editor: navmesh visualization and bake controls — `engine/editor/panels/navigation_panel.{h,cpp}`. `Window → Navigation` menu toggle; ImGui controls for all `NavMeshBuildConfig` fields; `Bake` / `Clear` buttons drive `NavigationSystem::bakeNavMesh()`; "Show polygon overlay" toggle draws every navmesh polygon's edges via `DebugDraw::line` (configurable colour + Y-lift). 6 unit tests in `tests/test_navigation_panel.cpp`. Patrol-path placement deferred to Phase 16 (AI behaviour trees).**
  Kind: implement.
...
- ✅ [3D_E-S0033] **Node graph data structures (NodeGraph, Node, Port, Connection in engine/formula/node_graph.h/.cpp)**
  Kind: implement.

- ✅ [3D_E-S0034] **Node type registry (factory helpers: createMathNode, createFunctionNode, createLiteralNode, createVariableNode, createOutputNode)**
  Kind: implement.

- ✅ [3D_E-S0035] **Connection validation (cycle detection via BFS, type checking, duplicate rejection)**
  Kind: implement.

- ✅ [3D_E-S0036] **Graph serialization (JSON round-trip with version tag, 61 unit tests)**
  Kind: implement.

- ✅ [3D_E-S0037] **Graph-to-ExpressionTree conversion (bidirectional: toExpressionTree + fromExpressionTree)**
  Kind: implement.

- ✅ [3D_E-S0038] **Visual scripting data model (ScriptValue, Blackboard, ScriptGraph, ScriptNodeDef, ScriptConnection in engine/scripting/)**
  Kind: implement.

- ✅ [3D_E-S0039] **Node type registry (NodeTypeDescriptor with execute functions, categorized palette lookup)**
  Kind: implement.

- ✅ [3D_E-S0040] **Graph interpreter (ScriptContext: impulse-driven execution, lazy data pull, call depth + node count safety limits)**
  Kind: implement.
...
- ✅ [3D_E-S0044] **Six-scope variable model (Flow / Graph / Entity / Scene / Application / Saved) via Blackboard**
  Kind: implement.

- ✅ [3D_E-S0045] **43 unit tests for scripting infrastructure**
  Kind: implement.

- ✅ [3D_E-S0046] **Design document with library evaluation (docs/phases/phase_09e_design.md, imgui-node-editor chosen)**
  Kind: implement.

- ✅ [3D_E-S0047] **Phase 9E-2: EventBus bridge — string-dispatched subscription from event nodes to typed C++ engine events; per-instance subscription tracking; automatic cleanup on scene unload**
  Kind: implement.

- ✅ [3D_E-S0048] **Phase 9E-2: 12 event nodes (OnKeyPressed, OnKeyReleased, OnMouseButton, OnSceneLoaded, OnWeatherChanged, OnCustomEvent wired; OnTrigger/Collision/Audio/Variable stubs registered for palette completeness)**
  Kind: implement.

- ✅ [3D_E-S0049] **Phase 9E-2: 15 action nodes (PlaySound, SpawnEntity, DestroyEntity, SetPosition, SetRotation, SetScale, ApplyForce, ApplyImpulse, SetVisibility, SetLightColor, SetLightIntensity, PublishEvent live; PlayAnimation/SpawnParticles/SetMaterial stubs)**
  Kind: implement.

- ✅ [3D_E-S0050] **Phase 9E-2: 22 pure nodes (GetPosition, GetRotation, FindEntityByName, MathAdd/Sub/Mul/Div, MathClamp, MathLerp, GetDistance, VectorNormalize, DotProduct, CrossProduct, BoolAnd/Or/Not, CompareEqual/Less/Greater, ToString, HasVariable, Raycast)**
  Kind: implement.

- ✅ [3D_E-S0051] **Phase 9E-2: 7 flow control nodes (SwitchInt, SwitchString, ForLoop with iteration cap, WhileLoop with safety valve, Gate, DoOnce, FlipFlop — stateful nodes persist across executions via ScriptNodeInstance::runtimeState)**
  Kind: implement.

- ✅ [3D_E-S0052] **Phase 9E-2: 4 latent nodes (WaitForEvent, WaitForCondition, Timeline with onTick progress callback, MoveTo with entity-lookup-by-ID for safety)**
  Kind: implement.

- ✅ [3D_E-S0053] **Phase 9E-2: ScriptCustomEvent for PublishEvent ↔ OnCustomEvent round-trip with name filtering**
  Kind: implement.

- ✅ [3D_E-S0054] **Phase 9E-2: 29 new unit tests — registration, math, vector, boolean, comparison, flow control, latent scheduling, and end-to-end EventBus bridge delivery (70 registered node types total)**
  Kind: implement.

- ✅ [3D_E-S0055] **Node graph renderer (imgui-node-editor integration) — Phase 9E-3**
...
- ✅ [3D_E-S0083] **Settings system (resolution, quality presets, keybindings)**
...
- ✅ [3D_E-S0084] **Loading screens (for scene transitions)**
...
- ✅ [3D_E-S0085] **Information plaques — approach an object to see a text description**
...
- ✅ [3D_E-S0093] **AX1. Geometric / ray-traced audio occlusion. Current occlusion is per-source material + raycast fraction set by the gameplay layer. Lift to a proper geometric pass: cast N rays per source per frame against the physics world, derive open-path fraction + diffraction edge + transmission material from the actual scene geometry (no manual per-source labelling). Pool of jobs scheduled via MT2 (Phase 10.6) so cost stays off the audio thread. Reference: Steam Audio's direct-sound model (Apache 2.0, source code readable as the algorithmic spec). Closes the "I added a wall but the sound still goes through it cleanly" gap that the current path requires the level designer to think about.**
  Shipped 2026-07-01 (slices S1–S5, commits 42da904 S1 / 4b7a34c S2 / c18b8ee S3 / 50ea8e4 S4 / f107df7 S5). New AudioOcclusionSystem (PostCamera, before AudioSystem) drives occlusionFraction/occlusionMaterial from scene geometry each frame: volumetric multi-ray fraction (default 8 rays, max 16, Fibonacci-sphere sampling; fraction=blocked/N so doorway edges muffle gradually), nearest-blocker SurfaceMaterial → preset (Default→Concrete so untagged walls occlude), reusing computeObstructionGain/LowPass verbatim. Rays cast across MT2 in one parallelFor (materials resolved on main per Jolt's body-lock contract; deterministic in synchronous mode), bounded by a 512-ray/frame budget + round-robin (never engages within shipped ranges) and culled by spatial+audibility+distance; per-source temporal slew releases culled sources. Settings (occlusionEnabled/RayCount/MaxDistance/SourceRadius) + editor toggle/sliders via a sibling AudioOcclusionApplySink. Non-goal: edge-diffraction filter (rayCast returns no normal — the multi-ray fan approximates partial paths perceptually, not a modeled delay). Deferred: in-viewport debug ray viz. Design docs/phases/phase_10_audio_occlusion_design.md (cold-eyes 5 loops). AX2 (convolution reverb) + AX3 (acoustic pre-bake) get their own design doc next.
  Kind: implement.

- ✅ [3D_E-S0094] **AX2. Convolution reverb with per-room baked impulse responses — current reverb is a 6-preset EFX table (Cave / LargeHall / Outdoor / …) lerped across zones. Upgrade to per-zone *measured* or *baked* impulse responses stored as `.wav` IRs, convolved at runtime via FFT (uniform-partitioned convolution; classic Gardner 1995 algorithm — paper, no proprietary code). Tabernacle-scale spaces, especially temples / caves, sound flat under preset reverb and acoustically correct under IR convolution. License-safe FFT: `pocketfft` (BSD-3, header-only).**
  SHIPPED 2026-07-04 across slices R1–R5 (37bdda4 aux-slot + parametric backend + per-source send · 02ef866 IR load + convolution backend + ReverbIrPool · da3dd27 ReverbZoneComponent + ReverbSystem zone selection/blend/slew · 2f75950 settings v4→v5 + editor authoring · this commit audit/CHANGELOG/flip). Two written deviations from this bullet's wording: (1) native OpenAL convolution (AL_SOFTX_convolution_effect) + parametric AL_EFFECT_REVERB fallback instead of a self-built pocketfft partitioned-FFT convolver — a self-built convolver needs OpenAL's mixed master PCM, which OpenAL Soft doesn't expose (a mixer rewrite); no pocketfft dependency added; (2) IR swap is snap-with-a-wet-dip, not a dual-slot crossfade (documented v2 refinement). Design of record: docs/phases/phase_10_audio_reverb_design.md (cold-eyes 5 loops). AX3 (acoustic pre-bake) is the follow-on bundle.
  Kind: implement.

- ✅ [3D_E-S0095] **AX3. Acoustic simulation pre-bake — at level-build time, run a sound-propagation simulation over the static geometry and bake per-probe IRs into the scene (Steam Audio "probe-based reflections" pattern). Runtime samples the nearest probe per source, costs zero ray-tracing during play. Pairs with the existing `Phase 5G` environment painting + probe placement infrastructure. Steam Audio's baker is Apache 2.0; we can either integrate it directly or implement the algorithm clean-room from the Funkhouser / Tsingos beam-tracing literature.**
  SHIPPED 2026-07-09 across slices B1–B5 (b9b4a11 AcousticProbeComponent + serialization + nearest-probe lookup · 60275b5 pure image-source bake core `bakeProbeIr` + absorption table + Sabine/Schroeder RT60 · 5ba9513 bake driver: Jolt facet extraction + coplanar merge + MT2 parallel bake + WAV/JSON sidecar · a558e1f runtime nearest-probe IR over authored zones · this commit editor probe authoring + "Bake Acoustics" button + `--bake-acoustics` CLI + audit/CHANGELOG/flip). Clean-room image-source (Allen & Berkley 1979) + statistical Sabine tail — NOT a Steam Audio integration (no Apache-2.0 dependency added). Three written deviations from this bullet's wording: (1) `bakeProbeIr` takes an explicit `roomVolumeM3` (a facet has no vertices; Sabine V is the extracted-triangle AABB volume, passed in); (2) one Jolt `GetTrianglesStart/Next` decode path for every static shape (box/hull/mesh/sphere/capsule) instead of stored-mesh-primary + Jolt-fallback; (3) `--bake-acoustics` reuses windowed init then exits (no headless mode exists — needs a display/xvfb like `--visual-test`). Design of record: docs/phases/phase_10_audio_reverb_design.md § 6 (joint AX2/AX3 doc, cold-eyes 5 loops).
  Kind: implement.

- ✅ [3D_E-S0096] **AX4. Procedural footstep / impact audio — material-aware footstep / weapon-impact / object-collision sounds derived from the surface material + impact velocity, not by hand-authoring per-surface clips. `assets/audio/synthesis/footstep_modal.json` describes per-material modal banks (stone / wood / cloth / metal / water / sand); runtime synth + round-robin sample swap. Cuts asset budget by ~10× for footsteps + impacts. Reference: Cook's "Real Sound Synthesis for Interactive Applications" + Stam's "Stable Fluids for sound" — both algorithmic, no proprietary code.**
  Shipped 2026-07-01 — AX4 procedural/material-aware audio bundle, slices S1–S10 (SurfaceMaterial tag, Jolt collision-event bus, modal+PhISEM synth core, Workbench-fit curves, material banks, footstep + impact emission, scripting OnCollision nodes, settings + editor material picker, Jolt-free header split). See CHANGELOG [Unreleased] and docs/phases/phase_10_procedural_audio_design.md.
  Kind: implement.

- ✅ [3D_E-S0097] **AX5. Audio LOD ladder — distant sources downgrade through tiers: full-spatial → cheap-spatial (skip per-source low-pass) → 2D drop → mute. Tier picked by distance + occlusion + bus priority. Shipped 2026-06-29 (slice 3 of the audio quick-wins bundle). New `engine/audio/audio_lod.{h,cpp}` — pure `audioLodTier(distance, maxDistance, occlusionFraction, priority, previousTier, cfg) → {Full, CheapSpatial, Drop2D, Mute}`. Driven by an effective audibility-loss ratio `max(distance/maxDistance, occlusionFraction)` (occlusion correlates with inaudibility via the always-applied gain path, so a heavily-occluded source can shed spatial detail without audible loss — no new magic threshold; occlusion reuses the distance-factor boundaries). `previousTier`-seeded hysteresis dead-band (±`cfg.hysteresis`) stops boundary flapping; `SoundPriority::Critical` clamps to ≥ CheapSpatial so dialogue keeps its 3D position. Pure + pool-agnostic by design (the §12-Q2 Mute keep-alive/release decision belongs in the apply layer). `composeAudioSourceAlState` gained a trailing defaulted `lodTier`; `AudioSystem::update` owns the per-entity previous-tier map (`m_lodTiers`, reaped with `m_activeSources`) and feeds it the listener distance. Settings: `AudioSettings::lodEnabled` (default on) rides the same v4 schema as AX6/AX8 with a sibling `AudioLodApplySink`/`AudioEngineLodApplySink`, a panel checkbox, and a debug status line. Deferred (flagged, project Rule 5): the Q2 *pool-pressure Mute-release* optimization — Mute currently keeps the AL source alive at gain 0 (the resolved default, correct; a muted source only occupies a slot like any quiet source did pre-AX5). Releasing a muted source to free a pool slot touches the voice-eviction layer and is a focused follow-up; correctness is unaffected. 12 new tests (8 tier-decision + 4 compose-stage) + settings round-trip/migration/apply coverage; 3584 pass.**
  Kind: implement.
...
- ✅ [3D_E-S0104] **AX12. Real-time spectrum / waveform viewer in editor — `AudioPanel` Debug tab gains live FFT spectrum + scrolling waveform of the master bus + per-bus solo. Aids debugging "why is my mix muddy" without a separate DAW. Pure-function FFT via `pocketfft`; ImGui-side render via ImPlot which is already pulled in for the Formula Workbench.**
  Shipped 2026-07-11. Design docs/phases/phase_10_audio_spectrum_viewer_design.md (5 cold-eyes loops). NOTE — the implementation supersedes this bullet's original sketch: NO pocketfft (reuses the in-repo radix-2 FFT, extracted to engine/audio/audio_spectrum) and it graphs a per-bus, producer-push CPU "analysis mix" (Music + procedural Sfx), NOT the master bus — OpenAL Soft exposes no post-mix or per-voice PCM, so a faithful master-bus tap would need loopback (breaks device hot-swap) or full PCM retention. Solo is output-only (eviction/occlusion-safe). Reduce-motion via a panel-local Freeze toggle (no persisted setting). Steps S1–S5 implemented + committed; full test suite green (3799 pass); ImPlot already vendored (no new dependency).
  Kind: implement.
...
- ✅ [3D_E-S0108] **Multi-language text support (UTF-8, language selection) — covered by slices L1 (UTF-8 decoder + codepoint Font API) + L4 (LocalizationService + language switching) in the design doc.**
  Progress (2026-06-10): slice L4 SHIPPED — LocalizationService.setLanguage(code) loads <dir>/<code>.json, swaps the active table, and publishes LanguageChangedEvent for live panel re-fetch (the runtime language-switch path). Combined with L1's UTF-8 decoder, multi-language text is now functional end-to-end. Remaining: L5 surfaces language selection in the Settings menu (schema v2→v3 + Language dropdown). Full suite 3383 green. ^3d_e-0003
  SHIPPED 2026-06-11 — slice L5 (commit bef6601: Settings::localization.language + migrate_v2_to_v3 + Language picker tab + LanguageChangedEvent live-apply) completes the L1+L4+L5 coverage. Runtime language switch + persisted selection now work end-to-end.
  Kind: implement.

- ✅ [3D_E-S0109] **Translatable string table system (all UI/plaque text referenced by key, not hardcoded) — covered by slice L4 (`StringTable` + `LocalizationService` + `tr()`) + L6 (`tools/localization_audit.py` CI lint).**
  Progress (2026-06-10): slice L4 SHIPPED — StringTable (flat key→value JSON loader) + LocalizationService (ISystem; active→English→key fallback) + free tr() + LanguageChangedEvent + seed {en,he,el,la}.json. Main-menu buttons migrated to tr() as proof-of-integration. The key-referenced string-table system this bullet asks for now exists; remaining is L6's tools/localization_audit.py CI lint that enforces no-hardcoded-strings. § 8 tests 13-17 green; full Debug+ASan suite 3383 green. Also fixed an L2-introduced CI leak (FontGLTest.MruCacheSkipsStackWalk llvmpipe JIT shader-state) via ScopedLeakCheckDisable. ^3d_e-0004
  SHIPPED 2026-06-11 — slice L6 (commit 04773a0: tools/localization_audit.py strict CI lint enforces no-hardcoded-strings + en.json key coverage; wired as the LocalizationAuditStrict ctest gate). With L4's StringTable + tr(), the key-referenced string-table system is complete.
  Kind: implement.

- ✅ [3D_E-S0110] **Hebrew, Greek, and Latin text rendering (right-to-left support for Hebrew) — covered by slices L1 (UTF-8 + uint32 glyph map), L2 (`FontStack` + Hebrew/Greek glyph-range loading), L3 (`rtl::toVisualOrder` per-run reverse — lightweight, not full UAX#9).**
  ...
  Progress (2026-06-10): slice L3 SHIPPED — engine/utils/rtl.{h,cpp} lightweight logical→visual reorder (per-Hebrew-run reverse incl. interior spaces; multi-word phrases reverse as a unit; NOT full UAX#9). Wired into all 3 TextRenderer paths before glyph emit via a reused decode/reorder scratch (HUD stays allocation-free). 8 rtl tests; full suite green (3377). This bullet's L1+L2+L3 coverage is now COMPLETE — Hebrew/Greek/Latin all render with correct RTL; remaining localization work (string table, language picker) is L4/L5. ^3d_e-0002
  SHIPPED 2026-06-10 — all 3 covering slices landed: L1 (commit 16ad7a4), L2 (73e8804), L3 (this commit). Hebrew/Greek/Latin render with correct RTL for Hebrew. Lightweight reorder, not full UAX#9 (mixed Hebrew+numerals deferred per design § 6).
  Kind: implement.

- ✅ [3D_E-S0111] **Language selection in settings menu — covered by slice L5 (settings schema migration v2→v3 + own `Language` settings section dropdown + `LanguageChangedEvent` live-apply path). ^3d_e-0005**
...
- ✅ [3D_E-S0121] **Subsurface scattering (SSS) — light transmission through thin/translucent materials**
  ...
  Marked complete 2026-06-20 (implemented 2026-06-19): Phase 10 Rendering slice R3 — analytic subsurface scattering (sssFrontScatter/sssBackScatter in scene.frag.glsl + engine/renderer/subsurface_math.h), commit 4995951. Design-of-record §10; CHANGELOG recorded. ROADMAP status was stale.
  Kind: implement.

- ✅ [3D_E-S0122] **Screen-space global illumination (SSGI) — real-time dynamic indirect light ^3d_e-0020**
  Marked complete 2026-06-24 (implemented this session): Phase 10 Rendering slice R4 — froxel near-field dynamic GI (Variant A). gi_inject.comp.glsl + engine/renderer/gi_math.h CPU mirror + the att3 injection source / GI read in scene.frag.glsl + VolumetricFogPass ping-pong cache + dynamicGiEnabled/reduceMotionGi accessibility wire. Design-of-record §11; CHANGELOG recorded. Tests: test_gi.cpp (CPU EMA/reproject/slice parity) + test_gi_gpu.cpp (full-scene-shader compile, gi_inject compile, 3-way giSliceCoord parity, GPU depth-match gate) + test_fog_benchmark.cpp GI inject ≤0.4ms gate — all green on RX 6600. Variant B (full Froxel Bounce, 3D_E-0015) remains the documented upgrade.
  Kind: implement.

- 💭 [3D_E-0015] **"Froxel Bounce" — cheap real-time dynamic single-bounce GI cached in the volumetric-fog froxel grid (the default GI that ships with the engine; ray-traced GI stays an opt-in high-end alternative).**

  **The gap this fills.** Vestige already ships the *static* half of GI: the `RadiosityBaker` bakes multi-bounce diffuse light into the `SHProbeGrid` (7 RGBA16F 3D textures). What's missing is the *dynamic* half — indirect light that responds to a moving sun, opened doors, or moving objects **without a re-bake**, cheaply enough to run on most GL 4.5 / RDNA2-class hardware and ship on by default. The existing Phase 13 GI options (Brixelizer SDF-RT, radiance cascades, idTech-8 surfels, ReSTIR) are all heavier and/or research-grade; this is the lightweight default. Per the user's direction (2026-06-19): **ship a very cheap but good GI by default; let users who want ray-traced GI choose it** (the "Vulkan and Ray Tracing → Ray tracing — global illumination" item below remains the opt-in high-end path).

  **The idea (single bounce, no ray tracing), reusing infrastructure already paid for.** Co-locate the dynamic-GI irradiance cache **inside the existing volumetric-fog froxel grid** (`VolumetricFogPass`, view-frustum-aligned 160×90×64 3D texture, exponential depth slices, compute-built). GI then piggybacks on the fog grid's data structure, its compute-dispatch slot, its temporal reprojection, and — crucially — its exponential-depth distance-LOD (distant froxels are larger ⇒ free GI level-of-detail, which matches GI's low-frequency nature). Two complementary injectors seed one bounce of indirect radiance into the grid:
  - **Off-screen sun bounce via a *lightweight* reflective shadow map (RSM).** The engine already rasterises CSM cascade depth. Add **one** extra colour attachment to the **coarsest cascade only**, writing reflected flux `albedo · sunColour · NdotL`; each texel is a virtual point light. World position is reconstructed from the cascade depth + light matrix, so only flux is added (cheaper than a classic pos+normal+flux RSM). This captures sun bouncing off surfaces **outside the view frustum** — exactly what screen-space GI fundamentally cannot do.
  - **On-screen near-field bounce via screen-space colour injection.** Last frame's lit HDR colour is splatted as indirect radiance into the froxel each visible point falls into — the SSGI signal, but written into the cache (so it survives view changes via reprojection) rather than gathered per-pixel.

  A single compute **propagation** pass then spreads injected radiance to neighbouring froxels (light-propagation-volume style, anisotropy borrowed from the fog phase-function machinery), amortised at 1–2 froxel-steps per frame; **temporal accumulation** (reusing TAA history + the Phase-10 R1/R2 motion buffer) grows the effective bounce distance across frames at no extra per-frame cost. Each shaded fragment finally reads its froxel's indirect irradiance trilinearly — identical to how fog inscatter is already sampled — and adds it to the diffuse term. The baked `SHProbeGrid` provides the far-field / fully-static **ambient floor**; Froxel Bounce adds only the **dynamic delta** on top, so the two compose without double-counting.

  **Honest novelty.** Every ingredient is established — RSM (Dachsbacher & Stamminger 2005), light-propagation volumes (Kaplanyan 2010), screen-space GI, clustered/froxel grids (Wronski 2014), probe-cached irradiance (DDGI-family), temporal accumulation. The new, defensible engineering contribution is the **synthesis fitted to this engine**: caching the dynamic GI *in the fog froxel grid itself* so GI and fog share one volume, one compute stage, one reprojection and one distance-LOD; seeded by a *single-cascade flux-only* RSM for off-screen bounce plus screen-space colour for near-field; layered over the existing radiosity-baked SH floor. This is a novel combination and a strong fit — **not** a claim of unprecedented GI physics.

  **Known limitation (surfaced, not hidden).** A view-frustum froxel grid does not cache bounce from directly *behind* the camera. Mitigations: the RSM covers off-screen-but-sunlit surfaces (most of the missing energy in a daylit walkthrough), and the SH bake is the floor everywhere. For the primary use case — architectural walkthroughs of static structures with a dynamic sun — this is an acceptable trade; a fully-dynamic open-world would want a world-space grid instead (a later upgrade, not this slice).

  **Why it's cheap & broadly runnable.** All-GPU compute (no hardware RT); reuses a grid + passes already inside the 2.0 ms fog budget; scales down by froxel resolution, by dropping the RSM cascade, or by fewer propagation steps. Estimated added cost ≈ 0.7 ms at 1080p on the RX 6600 (one coarse-cascade attachment write + one inject + one propagate compute + folded per-pixel sampling) — inside the 60 FPS hard floor. **Compute-ordering caveat:** the inject/propagate dispatches must run during the froxel-build stage (alongside fog inject/scatter), **before** the scene composite binds its sampler textures, or they clobber the composite's texture-unit bindings (the documented compute-in-composite unit-0 hazard).

  **CPU / GPU placement (Rule 7).** GPU: RSM flux write, screen-space injection, propagation, per-pixel froxel sample (all per-texel/per-pixel, data-parallel). CPU: dispatch orchestration, froxel/RSM config, VPL-count clamp (branchy, sparse).

  This is the concrete candidate realization of the Phase 10 **R4 "screen-space global illumination"** slice (`docs/phases/phase_10_rendering_design.md`), upgrading it from screen-space-only to off-screen-capable single-bounce. It is **parked as a considered design** — it goes through the normal research → design-of-record → cold-eyes → blocking user review gate before any implementation, like every other slice.
  **Layman:** Most "bounced light" (a red wall tinting a nearby floor, sunlight spilling through a doorway and lighting a room) is faked or pre-baked today, so it doesn't update when the sun moves or a door opens. This is a cheap way to make that bounced light *live* — it rides on top of the fog system we already built, so it costs very little, runs on ordinary graphics cards, and turns on by default. People with high-end cards can still switch to the slower, more accurate ray-traced version if they prefer.
  Kind: research.
  Source: user-request-2026-06-19.

- ✅ [3D_E-S0123] **Motion vectors from geometry pass via MRT. Today the renderer uses a per-object overlay pass that re-renders opaque geometry after the full-screen camera-motion pass with per-draw `u_model` / `u_prevModel` matrices (`assets/shaders/motion_vectors_object.{vert,frag}.glsl`). This fixes rigid-body TAA ghosting but re-draws every opaque item. Cleaner: emit motion vectors directly from the main scene pass via an MRT attachment (RG16F motion buffer alongside the HDR color target) and drop the overlay pass. Must also handle skinned and morph-target meshes — currently those fall through to rigid-body motion only, which undershoots by the animation delta. Prerequisite for correct TAA + FSR 2.x on any content with animated characters. Once the previous-frame normal buffer is retained alongside motion vectors, `V_mask = α(1 − n_cur · n_prev)` (nVidia GDC 2024 "rain puddles" technique) is a cheap complementary rejection signal — catches animated surfaces where motion vectors agree but the surface is different (undulating water, foliage, animated decals). See Phase 10.8 Decal System for the decal-side consumer.**
...
- ✅ [3D_E-0028] **Optimise the planar-reflection water pass — cull + LOD + throttle the reflection render.**
  ...
  **Layman:** Water mirrors the world by secretly drawing the whole scene twice; make that second pass much cheaper.
  ...
  Resolved (2026-07-31): frustum-cull the reflection AND refraction passes — both re-rendered the whole scene whenever the scene merely *contained* water, not when any was on screen. New pure `waterSurfaceWorldBounds()` (water_surface.h) + existing `isAabbInFrustum` gate the pair in the render loop. Measured on the meadow benchmark uncapped: 11.94 -> 10.78 ms frame, 83.6 -> 93.5 FPS (+12%) when no water is in frustum; identical work when it is. Verified by 5 new unit tests (in-front / behind / beyond-far-plane / bounds / bad numWaves) and a `--visual-test` gate instrument showing 66 of 193 sampled frames culled. Levers 2-3 from the bullet (reduced-detail reflection draw, N-frame throttle) NOT implemented — unnecessary once the pass is skipped outright, and both carry temporal-artifact risk; re-open if a water-heavy scene needs more. Bullet claim corrected: `reflectionResolutionScale` is inert (nothing in the render path reads it); the reflection FBO is quarter-res via hardcoded `/4` in water_system.cpp:27 and engine.cpp:1254,1279.
  Correction (2026-07-31): the figures first recorded here (11.94 -> 10.78 ms, 83.6 -> 93.5 FPS) were captured while a stray `vestige --play` process from an earlier screenshot run was still alive and sharing the GPU — `pkill -INT` had not killed it. Re-measured clean-room (process table verified empty before and after each run, 3-run repeatability +/-1.2 FPS): 13.92 -> 12.32 ms, 72.7 -> 82.6 FPS, +13.6%. The win is confirmed and marginally larger; only the absolute numbers were wrong. Benchmarks now go through a guard script that aborts if any engine instance is already running.
...
- ✅ [3D_E-0029] **Cache static-geometry shadow maps + throttle far CSM cascade updates.**
  ...
  **Layman:** Stop re-drawing shadows for things that never move every single frame.
  Kind: perf.
  Source: in-session-2026-07-11 (perf sweep; fixture 3D_E-0027).
  Resolved (2026-07-31): far-cascade throttle implemented to the cadence docs/engine/renderer/spec.md already specified — cascades 0-1 every frame, 2 and 3 staggered on a 4-frame cycle that never rebuilds both in one frame. Clean-room measured: 13.92 -> 12.66 ms, 72.7 -> 80.5 FPS (+10.8%) on the meadow with the pond in view. Fixed a latent desync while doing it: `CascadedShadowMap::update` recomputed EVERY cascade's light-space matrix each frame while the loop skipped rasterising cascade 3, so a throttled cascade sampled its stored depth through a matrix describing a different volume. `update()` now takes a `cascadeMask` and leaves a skipped cascade's matrix and texel size untouched; one predicate drives both the mask and the render skip so they cannot drift. First `cascadeCount` frames force all cascades (a never-rasterised layer holds undefined depth). Three prior claims corrected: the code skipped only cascade 3 and only on odd frames, its comment claimed a cadence it did not implement, and there is no ShadowPass GPU timer to watch (measured via frame time instead; adding one would mean plumbing the profiler into Renderer). NOT DONE — static-caster depth caching, the bullet's other half: in this fixture the camera moves continuously so cascade matrices change nearly every frame and the cache would invalidate almost immediately; high complexity, low ROI here. Re-open for scenes with a mostly-static camera. TWO CAVEATS: (a) shadow swim under a throttled cascade is a TEMPORAL artifact and the visual-test harness only takes static captures, so it is unverified — needs a moving-camera check; (b) the measured scaling is non-linear in an unexplained way (rebuilds/frame 3.5 -> 2.75 gained +0.4%, 2.75 -> 2.5 gained +10%), repeatable but not understood — re-check independently before relying on the figure.
...
- ✅ [3D_E-0030] **Automated performance-regression gate from the meadow benchmark + profiler CSV.**
  ...
  **Layman:** Make the computer automatically warn us if a change makes the engine slower.
  ...
  Resolved (2026-07-17): shipped the hardware-agnostic gate — `tools/perf_gate.py` reduces a profiler CSV (drop warm-up interval, median) and compares it to a committed baseline JSON via an ordered MISSING→INCONCLUSIVE→SKIPPED→compare classification; exit codes 0 pass / 1 regression / 2 inconclusive-data / 3 config-error. Design: docs/phases/phase_10_perf_regression_gate_design.md (cold-eyes 5 loops, converged). ctest gates `PerfGateSelftest` + `PerfGateCatchesRegression` (WILL_FAIL) green over `tests/fixtures/perf_gate/` (7 fixtures). The investigate finding: the *live* gate runs on real GPU hardware — CI is GPU-less (llvmpipe), so only the comparator self-test runs on CI; the real RX 6600 baseline is captured later by the user via `--update-baseline` (documented operational follow-up, not remaining engineering).

- ✅ [3D_E-0032] **Weak/gamer-hardware perf scalability — Tier 1 wave 1: render-scale + quality presets + FXAA/CAS.**
  Strategy + Tier-1 design docs cold-eyes-converged and signed off (`docs/phases/phase_10_performance_scalability_strategy.md`, `..._tier1_render_scale_and_presets_design.md`). Wave-1 shipped: (1) **render-scale** — the play-mode scene+post chain (and water reflection FBO) render at `renderScale × play-res` and blit-upscale to the window (24% GPU cost at 0.5×, matching the pixel-count prediction); (2) **`applyQualityPreset`** maps Low/Medium/High/Ultra onto renderScale + AA/SSAO/bloom + a fog/GI heavy-post perf-gate (AND-ed with the accessibility gate — INV-A11Y — so a preset never re-enables an accessibility-disabled pass); Custom leaves hand-tuned knobs alone; (3) **FXAA** (Lottes 3.11, preset 39) as the cheap-tier AA — the scene FBO now renders single-sample for NONE/FXAA (also fixes a latent NONE=4×MSAA waste), plus a **CAS** sharpen pass to claw back the upscale softness. Headless preset tests + GPU FXAA/CAS shader tests; verified end-to-end (Low preset engages FXAA, 24-frame visual test clean).
  **Layman:** Let weaker PCs and handhelds hold 60 FPS by rendering the 3D world a bit smaller and sharpening it back up, with Low/Medium/High/Ultra presets that flip the right settings in one click.
  Kind: perf.
  Source: user-request-2026-07-17 (weak-HW ultrathink). Remaining: Tier-2 (shadow/foliage/water preset rows — need new setters) and Tier-3 (temporal FSR2-class upscaler).
...
- ✅ [3D_E-0031] **Meadow realism A — real PBR ground textures on terrain (replace the flat-colour placeholder).**
  ...
  **Layman:** Give the ground a real grass/dirt photo texture instead of flat cartoon green — the biggest single fix for the "too cartoony" look.
  ...
  Progress (2026-07-17): A1 + A2 complete (commits up to A2 part 2). A2 part 2 shipped — the textured ground path now shades with the engine's **real Cook-Torrance GGX** (reuse of scene.frag.glsl's distributionGGX/geometrySmith/geometrySchlickGGX/fresnelSchlick + PI; dielectric F0=0.04). The design's §4.4 Blinn-Phong→GGX Workbench fit was verified numerically infeasible (best-case per-roughness max-err ≈0.245 @ r=0.8 vs the ≤0.05 bar) and dropped by user decision; design revised + re-run through cold-eyes (2 loops, converged — see phase design §13). New GGX-consistency test `tests/test_terrain_ggx_parity.cpp` (GPU runs the actual terrain GLSL vs the ggx_distribution reference + a verbatim-source lock to scene.frag.glsl). A3 complete (2026-07-18): the textured ground path now applies the per-layer tangent-space **detail normals** — sampled + height-blended by the same weights as albedo — onto the macro normal via a Ben-Golus **Whiteout** blend (top-down world-XZ frame), with **per-axis triplanar** normal + albedo/AO/roughness sampling on steep pixels (pond-bowl banks). New pure `whiteoutBlendNormal` CPU mirror in `terrain_material_blend.h` + directional parity test (`TerrainNormalBlendTest` in `tests/test_terrain_material_set.cpp`: +X detail tilt raises world-N.x, −X lowers it, result unit-length). Verified: terrain shader compiles/links GL-error-free, `--visual-test` meadow renders textured ground with surface relief and clean (unstretched) pond banks. A4 complete (2026-07-18): distance-tiling break-up — the top-down albedo fades to a ~4× coarser far-scaled sample over view distance (`u_distanceTiling` 25→120 m, `FAR_TILING_SCALE` 0.25; albedo-only extra taps, skipped on near pixels via the `distBlend>0.001` guard) atop the A2 `tilingDetail` macro-variation. New pure `distanceTilingBlend` CPU mirror + `TerrainDistanceTilingTest` (near/far endpoints, smoothstep midpoint = 0.5, monotonic). Verified GL-error-free; far field reads without obvious repetition. A5 complete (2026-07-18) — **phase A done**: ground-texture **quality tiers** wired to the graphics `QualityPreset` via a `u_groundQuality` uniform (0=Low/1=Medium/2=High) — High = all features, Medium drops the distance-tiling second sample, Low = albedo+AO only (no detail-normal taps, linear weight blend, flat colour on slopes ≈ pre-texturing cost); default High, no `FormulaQualityManager` coupling (design §6). Plumbed through `applyQualityPreset`→`RendererQualitySink::setTerrainGroundQuality`→`TerrainRenderer::setGroundQuality` (same boot + live-apply path as AA/SSAO/bloom/heavy-post), pinned by the extended `SettingsApply.QualityPreset*` tests. CC0 ambientCG layer set + ASSET_LICENSES/THIRD_PARTY_NOTICES rows already committed (A1). Verified: full suite green (3850 tests), shader GL-error-free, `--visual-test` renders correctly at High **and** Low. Perf gate (terrain pass ≤3.0 ms / ≥60 FPS at High on RX 6600) is a manual Performance-panel read per design §6 (automated CSV waits on 3D_E-0027 S6). NOTE: --visual-test surfaces ~10386 pre-existing GL_INVALID_OPERATION in glDrawElements on the meadow foliage — unchanged by these slices (not terrain-related); worth a separate look.

- ✅ [3D_E-0038] **Meadow realism B — realistic grass (real blade textures, denser/taller/varied).**
  The foliage renderer always builds a hand-drawn procedural blade texture and ignores `FoliageTypeConfig.texturePath`. Wire a real grass-blade alpha texture (CC0, single card — not a UV-indexed atlas) through that slot with a procedural fallback, and add height / colour / density variation, leaning on the engine's existing distance-fade foliage path (star mesh + per-instance distance-alpha; there is no billboard-LOD) so the field stays at 60 FPS. Grass that reads as real blades, not cardboard. Phase B of the realism overhaul; depends on nothing but pairs with 3D_E-0031.
  **Layman:** Make the grass look like real grass — thicker, taller, varied — instead of hand-drawn cards.
  Resolved (2026-07-18): shipped B1 (FoliageRenderer::setTypeTexture honours texturePath with a known-good-upload procedural fallback + shared uploadRGBA8 helper; pure-predicate unit tests), B2 (CC0 grass_blades.png cropped from OpenGameArt + colour-graded green, meadow wire-up with foliage_local/ override, widened scale/tint variation, ASSET_LICENSES + THIRD_PARTY rows; --visual-test verified real green grass, 0 GL errors), B3 (FoliageQuality Low/Med/High tier on the existing QualityPreset via RendererQualitySink — grass draw distance 100/70/45 m + Low drops grass shadows; preset→tier unit tests). Full suite green (3854). Design: docs/phases/phase_10_meadow_grass_design.md (cold-eyes converged, 4 loops). Owed: maintainer's Release Performance-panel Foliage read on RX 6600 (≥60 FPS gate; at High the grass path is unchanged from A so low-risk).
  Kind: feature.
  Source: user-request-2026-07-11 (realism overhaul; fixture 3D_E-0027).
...
- ✅ [3D_E-0039] **GPU procedural grass — one-shared-SSBO Bézier-blade meadow field (G1–G5).**
  ...
  **Layman:** Grow the grass on the graphics card instead of pasting flat pictures of it — a million real blades that clump into tufts and thin out with distance, so the meadow looks alive up close and stays smooth far away.
  Progress: G1 blade geometry + shader (VS Bézier ribbon + CPU-parity test), G2 CPU placement + PCG gating + clumping (commit 86008d6), G3 distance LOD + per-chunk frustum cull (commit 355dfb9 — `grass_lod.h` + 6 unit tests) shipped.
  Resolved (2026-07-21): G4 shading + wind + CSM shadow-receive (commit 7ca8856 — half-Lambert + backlit translucency + height AO + per-clump tint; vertex-shader wind bend; receive-only, grass does NOT cast in v1, scope cap logged) and G5 quality tiers + perf gate + meadow wire-up all shipped. G5: `GrassQuality` Low/Med/High on the `RendererQualitySink` preset path (`GrassRenderer::setQuality` dials LOD distance; preset→tier unit test); billboard grass retired (GPU field is the only meadow grass; Foliage pass freed); `grass_dense` + `grass_lookdown` profile poses. **Perf (RX 6600, Release, High):** Grass pass ~1.2 ms (max 1.28) for ~1.04 M blades, GPU frame ~8 ms, vsync-locked 60 FPS with ~50% headroom — clears the 60-FPS bar. Full suite green. v2 (GPU compute cull + `glMultiDrawArraysIndirect` + GPU-side placement) remains a separate future phase.
  Kind: feature.
...
- ✅ [3D_E-0033] **Meadow realism C — realistic trees & plants.**
  ...
  **Layman:** Swap the cartoon trees for realistic-looking ones — the treeline you keep pointing at.
  Kind: feature.
  Source: user-request-2026-07-11 (realism overhaul; fixture 3D_E-0027).
  Design **v2** 2026-07-21 (docs/phases/phase_10_meadow_realism_c_trees_plants_design.md). The v1 approach (baked hemi-octahedral impostor + decimated Poly Haven photoscans) was **dropped** during T1: the photoscans were 7–17 M tris (unusable as a real-time LOD0). v2 sources **LOLIPOP game-ready CC-BY packs** (pine/fir/maple/birch) that already ship the artist's LOD0–2 meshes + billboard — so **no custom impostor bake and no decimation**. Approach: revive TreeRenderer (extend its 2-bucket LOD scheme to 3), route field+hero placement through placeTree, CSM cast/receive, foliage-style wind, pond reflection; shared per-species textures keep VRAM ~145 MiB. **CC-BY attribution** owed (ASSET_LICENSES.md + THIRD_PARTY_NOTICES.md). Slices **T1–T7**. v2 re-run through cold-eyes to convergence. Implementing T1 (asset prep: fetch + split).
...
- 💭 [3D_E-0625] **WITHDRAWN — the god-ray pass was thought to have no headroom on the tier it exists for; the harness was wrong, not the pass.**
...
- 📋 [3D_E-0657] **Fog design section 8 still quotes single-sample per-pass figures, and one of them is load-bearing.**
  ...
  the 0.69 ms all-sky and 0.48-0.55 ms no-sky god-ray figures, the 10.6 us
  harness sync overhead, and the 0.1 ms noise-modulation figure in fog
  design section 11.10.

  The one with a consequence is the 1.21 ms 128-tap mutation result. Design
  section 8 cites it as proof the 1.2 ms reference budget catches a doubled
  tap count -- the falsifying property that makes that gate worth having.
  It was a median under the retired harness, and the same all-sky pass now
  reads ~0.52 ms as an uncontended minimum where it read 0.69 ms as a
  median. That ratio maps the 128-tap case to ~0.9 ms, UNDER the budget, so
  the reference gate may no longer catch what it was verified to catch.
  Re-run the mutation before relying on that row; if it passes, the 1.2 ms
  figure needs re-deriving rather than the measurement re-fitting to it.

  Re-taking the rest is not just a re-run -- the god-ray pair needs the
  all-sky and no-sky scenes reconstructed. Until then, read those figures
  as indicative.
...
- ✅ [3D_E-S0130] **Volumetric god rays / crepuscular rays — visible light shafts from the sun through openings**
  ...
  - God rays through gaps in clouds (when volumetric clouds are available from Phase 15) ^3d_e-0017
  Marked complete 2026-06-20 (implemented 2026-06-18): Phase 10 fog slice 11.5 screen-space god rays, commit 5472701. CHANGELOG recorded. ROADMAP status was stale.
  Kind: implement.

- ✅ [3D_E-S0131] **Mist / ground fog — localized fog volumes placeable per-entity**
...
- ✅ [3D_E-S0259] **Cl1. CPU↔GPU cloth parity harness: headless test that drives identical `ClothConfig` on both backends for 2s, asserts per-particle position delta < `epsilon`. Depends on Sh1–Sh4. CLAUDE.md Rule 7 parity gate.**
...
- ✅ [3D_E-S0271] **Ts19-D1. Remove three forwarding wrappers (`smallConfig` / `smallClothConfig`) in `tests/test_cloth_{simulator,collision,solver_improvements}.cpp` — replace ~50 call-sites with `Testing::clothSmallConfig(...)` and delete the local stubs. (Slice 18 Ts3 extracted the body; the stubs were left behind.)**
  Kind: implement.

- ✅ [3D_E-S0272] **Ts19-D2. `makeTestSkeleton` is duplicated across `tests/test_advanced_physics.cpp`, `tests/test_motion_matching.cpp`, `tests/test_crossfade.cpp` (and 1-2 more animation files). Extract to a shared header.**
  Kind: implement.

- ✅ [3D_E-S0273] **Ts19-D3. `UITheme::defaultTheme()` is constructed inline at 30+ call sites across `tests/test_ui_theme_accessibility.cpp`, `tests/test_ui_runtime_panel.cpp`, `tests/test_ui_system_screen_stack.cpp`, `tests/test_ui_widgets.cpp`. Extract to a `UIThemeFixture` (or one shared helper).**
  Kind: implement.

- ✅ [3D_E-S0274] **Ts19-D4. `PhysicsWorld` init/shutdown boilerplate appears 7+ times across `tests/test_physics_world.cpp`, `tests/test_physics_constraint.cpp`, `tests/test_physics_character_controller.cpp`. Extract a `PhysicsWorldTest` fixture.**
  Kind: implement.

- ✅ [3D_E-S0275] **Ts19-D5. Atlas-JSON-to-temp-file helpers duplicated in `tests/test_sprite_atlas.cpp:22`, `tests/test_sprite_panel.cpp:25`, `tests/test_sprite_renderer.cpp:27`. Extract to `sprite_test_helpers.h`.**
  Kind: implement.

- ✅ [3D_E-S0276] **Ts19-D6. `makeIdentityLut` / `lutLookup_cpu` in `tests/test_color_grading_parity.cpp` are byte-identical to `generateNeutralLut` / `lutLookup` in `tests/test_color_grading.cpp`. Extract to `color_grading_test_helpers.h`.**
  Kind: implement.

###### Fixtures

- ✅ [3D_E-S0277] **Ts19-F1. `DefaultHudPrefab` 9 tests in `tests/test_default_hud.cpp` each reconstruct `UICanvas` + `UITheme` + `UISystem`. Extract a fixture.**
  Kind: implement.

- ✅ [3D_E-S0278] **Ts19-F2. `UISystemFocus` 12 tests in `tests/test_ui_focus_navigation.cpp` construct `UISystem` on the stack with no fixture. Convert to `TEST_F`.**
  Kind: implement.

- ✅ [3D_E-S0279] **Ts19-F3. `ControllerFixture` in `tests/test_character_controller_2d.cpp:23` is a plain struct used inline. Convert to a proper `::testing::Test` fixture so ctor/`SetUp` failures are reported cleanly.**
  Kind: implement.

###### Assertions

- ✅ [3D_E-S0280] **Ts19-A1. `tests/test_advanced_physics.cpp:877` — volume-conservation `EXPECT_GT(totalVolume, 0.1f)` is too loose to detect a real loss. Tighten to the analytical fragment-sum value with ~1% tolerance.**
  Kind: implement.

- ✅ [3D_E-S0281] **Ts19-A2. `tests/test_ik_solver.cpp:43` — `ReachableTarget` uses `startChanged || midChanged`; a single-joint regression (one joint locked) still passes. Use `&&` for the unambiguous two-joint reach case.**
  Kind: implement.

- ✅ [3D_E-S0282] **Ts19-A3. `tests/test_scene_serializer.cpp:194` — `EntityJsonShapeReferenceDocument` / `SceneEnvelopeShapeReferenceDocument` assert values the test itself just wrote. Replace with a real round-trip or mark explicitly as a documentation snapshot.**
  Kind: implement.

- ✅ [3D_E-S0283] **Ts19-A4. `tests/test_cloth_simulator.cpp:335` — `GetConstraintCount` asserts `EXPECT_GT(count, 0u)`; analytical value for a 3×3 stretch+shear+bend grid is 26. Tighten to `EXPECT_GE(count, 26u)`.**
  Kind: implement.

###### Flakiness / tolerances

- ✅ [3D_E-S0284] **Ts19-FL1. `tests/test_root_motion.cpp:112,170` — `EXPECT_NEAR` tolerances of 0.2/0.5 on analytically exact linear-interpolated deltas. Tighten to ~1e-3.**
  Kind: implement.

- ✅ [3D_E-S0285] **Ts19-FL2. `tests/test_particle_data.cpp:150` — `SpawnsParticlesOnUpdate` accepts ±50% on a deterministic emission. Replace with the exact expected count.**
  Kind: implement.

###### Isolation

- ✅ [3D_E-S0286] **Ts19-I1. `tests/test_color_grading_parity.cpp:140` — `ASSERT_TRUE(prog.valid())` aborts after `lutTex` allocation but before `glDeleteTextures` → GL texture leak on failure. Wrap `lutTex` in a `ScopedGLTexture` RAII guard.**
  Kind: implement.

- ✅ [3D_E-S0287] **Ts19-I2. `tests/test_settings.cpp:81` — `ConfigPath` tests use manual `setenv`/`unsetenv` save+restore. Extract an `EnvGuard` RAII helper so an early-exit path can't pollute later tests.**
  Kind: implement.

###### Coverage gaps

- ✅ [3D_E-S0288] **Ts19-CG1. `tests/test_rigid_body.cpp:59` — `destroyBody()` on a dynamic body is never explicitly tested; a Jolt body-remove leak for dynamic bodies wouldn't be caught.**
  Kind: implement.

- ✅ [3D_E-S0289] **Ts19-CG2. `tests/test_gpu_cloth_simulator.cpp:461` — `reset()` before `initialize()` contract is unpinned. Add `ResetBeforeInitIsNoOp`.**
  Kind: implement.

- ✅ [3D_E-S0290] **Ts19-CG3. `tests/test_terrain_size_caps.cpp` — no happy-path test; the acceptance branch of the validator is untested. Add `TerrainValidationAcceptsWithinLimits`.**
  Kind: implement.

- ✅ [3D_E-S0291] **Ts19-CG4. `tests/test_pbr_material.cpp:232` — `AlphaCutoffRoundTrips` / `DoubleSidedToggles` use `EXPECT_NO_THROW` only; no getter exists. Expose getters or use the serializer round-trip to assert the value.**
  Kind: implement.

###### Accuracy

- ✅ [3D_E-S0292] **Ts19-AC1. `tests/test_color_grading.cpp:180` — warm/cool/contrast/desaturate transform tests reimplement the production formula locally. Drive via real `ColorGradingLut` (upload + sample, parity-test style) so production drift is detected.**
  Kind: implement.

###### Parametrisation

- ✅ [3D_E-S0293] **Ts19-PA1. `tests/test_cloth_presets.cpp:21,35,49,64` — 5-element preset initializer list copy-pasted across 4 tests; adding a preset requires 4 edits. Convert to `TEST_P` over `ValuesIn(allPresetsExceptCustom)`. Also: `CUSTOM` is in the name-list test but missing from the mass/substeps/compliance property checks.**
  Kind: implement.

###### Naming (LOW, opportunistic)

- ✅ [3D_E-S0294] **Ts19-N1. `tests/test_fabric_material.cpp:231` — typo `FineLInenMatchesExistingPreset` → `FineLinenMatchesExistingPreset`.**
  Kind: implement.

- ✅ [3D_E-S0295] **Ts19-N2. `tests/test_ibl.cpp:210` — typo `RoughSurfaceSpreadssamples` → `RoughSurfaceSpreadsSamples`.**
  Kind: implement.

- ✅ [3D_E-S0296] **Ts19-N3. `tests/test_ruler_tool.cpp:41` — `SecondClickCompleteMeasurement` → `SecondClickCompletesMeasurement`.**
  Kind: implement.

###### Performance (LOW)

- ✅ [3D_E-S0297] **Ts19-P1. `tests/test_cloth_simulator.cpp:226` — `GroundPlaneStopsParticles` runs 300 frames at 5 substeps (1500 XPBD iters) for a trivial floor check; cloth reaches the floor in <60 frames at g=-9.81. Reduce to 60 frames.**
  Kind: implement.

- ✅ [3D_E-S0298] **Ts19-P2. `tests/test_memory_tracker.cpp` — `MemoryTracker::update()` reads sysfs 120× in a loop. Reduce to 1–3 samples; the contract is observability, not statistical convergence.**
  Kind: implement.
...
- ✅ [3D_E-S0299] **Ts20-IS1. `tests/test_cube_loader_hardening.cpp:44` — reset `CubeLoader::setSandboxRoots({})` in `SetUp()` (was in `TearDown()` only); symmetric reset hardens against partial-prior-test failures.**
  Kind: implement.

- ✅ [3D_E-S0300] **Ts20-IS2. `tests/test_lip_sync_sandbox.cpp:36` — same pattern, `LipSyncPlayer::setSandboxRoots({})` in `SetUp()`.**
  Kind: implement.

- ✅ [3D_E-S0301] **Ts20-FL3. `tests/test_input_bindings.cpp` — added file-local `GlfwScope` RAII guard; replaced the three manual `glfwInit()`/`glfwTerminate()` pairs in the GLFW-dependent display-label tests. Terminate now runs even on ASSERT/skip.**
  Kind: implement.

- ✅ [3D_E-S0302] **Ts20-AC2. `tests/test_bloom.cpp` — replaced the local `bt709Luminance` reimplementation with `Vestige::bloomLuminance` from `engine/renderer/bloom_downsample_karis.h`. Coefficient drift in the SUT now fails the test.**
  Kind: implement.

- ✅ [3D_E-S0303] **Ts20-AC3. `tests/test_ibl.cpp` — added file-header clarifying SCOPE: this file pins the mathematical formulae (math-spec), not the runtime shader; shader parity is `test_ibl_parity.cpp`. Triage's "remove" suggestion was wrong — the math-spec coverage is valuable.**
  Kind: implement.

- ✅ [3D_E-S0304] **Ts20-AS11. `tests/test_particle_data.cpp:346` — `EXPECT_GE(count, 0)` → `EXPECT_GT(count, 0u)`; old assertion was tautological on an unsigned count.**
  Kind: implement.

- ✅ [3D_E-S0305] **Ts20-AS12. `tests/test_physics_constraint.cpp:424` — float `!=` replaced with `EXPECT_GT(std::abs(delta), 0.01f)`; old form passed on tiny FP noise.**
  Kind: implement.

- ✅ [3D_E-S0306] **Ts20-AS14. `tests/test_subtitle_narrator_style.cpp:88` — OR collapsed into two separate assertions; both colour AND italic must differ between narrator modes.**
  Kind: implement.

- ✅ [3D_E-S0307] **Ts20-AS6. `tests/test_cloth_simulator.cpp:783` — added `EXPECT_GT(sim.getLraConstraints().size(), 0u)` after `rebuildLRA()`; the LRA capture test was previously silent on whether constraints were actually built.**
  Kind: implement.

- ✅ [3D_E-S0308] **Ts20-HA4. `tests/test_node_graph.cpp:1095` — dropped white-box `EXPECT_EQ(newId, 1u)` after `clear()`; replaced with `EXPECT_GT(newId, 0u)`. The exact reset value is an impl detail.**
  Kind: implement.

- ✅ [3D_E-S0309] **Ts20-VE1. `tests/test_gpu_cloth_simulator.cpp:153` — removed redundant `SUCCEED()`; replaced with a comment explaining the compile-only contract being pinned.**
  Kind: implement.

- ✅ [3D_E-S0310] **Ts20-CV2 + DS3. `tests/test_biome_preset.cpp:24-31` — `SerializeDeserialize` now verifies all `foliageLayers`/`scatterLayers`/`treeLayers` fields (typeId/speciesId + density); previously only `[0].density` and `[0].density` for trees were checked. Existing `@file` header confirmed (DS3 reclassified as FP).**
  Kind: implement.

- ✅ [3D_E-S0311] **Ts20-DS6. `tests/test_physics_fixed_step_callback.cpp:91` — `CallbackDtIsAlwaysFixedTimestep_Ph1` now reports the offending `dt` on failure rather than just `EXPECT_TRUE(allMatch)`.**
  Kind: implement.
...
- ✅ [3D_E-S0312] **Ts20-SY1. `tests/test_system_registry.cpp:178` (CRITICAL, isolation) — `dummyEngine()` `reinterpret_cast<Engine&>(char[1])` is UB under UBSan even though never dereferenced. Fix needs either a minimal `EngineStub` translation-unit or a `SystemRegistry::initializeAll(Engine*)` nullable overload. Production-API decision, not a test-only change.**
  Kind: implement.

- ✅ [3D_E-S0313] **Ts20-DE1, Ts20-DE2. `tests/test_environment_forces.cpp:105,176` (HIGH, determinism) — `WindVelocityAfterGusting` and `GustStateTransitions` rely on unseeded RNG. Fix needs `EnvironmentForces::setGustRngSeed(uint32_t)` SUT API addition. ^3d_e-0001**
  Kind: implement.
...
- ✅ [3D_E-S0315] **Ts20-FL1. `tests/test_async_driver.cpp:287,339` — `sleep_for(50ms/100ms)` before `cancel()` assumes OS spawn timing. Poll until `isRunning()` is true.**
  Kind: implement.

- ✅ [3D_E-S0316] **Ts20-FL2. `tests/test_reference_harness.cpp:134` — temp-file paths unstamped under `ctest -j`. Use `vestigeTestStamp()`.**
  Kind: implement.

- ✅ [3D_E-S0317] **Ts20-FL4. `tests/test_environment_forces.cpp:357` — float accumulation tolerance too tight at 100 ticks. Loosen to `0.05f` or derive expected value from rate.**
  Kind: implement.

- ✅ [3D_E-S0318] **Ts20-FL5. `tests/test_photosensitive_retrofit.cpp:99` — flicker phase not pinned at guaranteed-difference point. Drive emitters to half-clamped-period phase offset.**
  Kind: implement.

###### Isolation (3)

- ✅ [3D_E-S0319] **Ts20-IS3. `tests/test_fit_history.cpp:62` — four tests share `/tmp/fh_*.json` with no PID stamp. Use stamped paths.**
  Kind: implement.

- ✅ [3D_E-S0320] **Ts20-IS4. `tests/test_entity_serializer_registry.cpp:265` — `Logger::clearEntries()` in test bodies, order-dependent. Move into fixture `SetUp()`.**
  Kind: implement.

- ✅ [3D_E-S0321] **Ts20-IS5. `tests/test_reference_harness.cpp:120` — silent fail on missing reference-cases dir. Guard with `GTEST_SKIP()` if dir missing.**
  Kind: implement.

###### Accuracy (1)

- ✅ [3D_E-S0322] **Ts20-AC4. `tests/test_hdr_pipeline.cpp:31` — BT.709 coefficients hardcoded with no shader parity. Extract via source-grep (mirror Ts20-AC2 fix) or remove redundant test.**
  Kind: implement.

###### Performance (2)

- ✅ [3D_E-S0323] **Ts20-PE1. `tests/test_cloth_simulator.cpp:263` — `SphereCollisionPushesParticlesOut` runs 120 frames vs documented 60. Reduce.**
  Kind: implement.

- ✅ [3D_E-S0324] **Ts20-PE2. `tests/test_cloth_solver_improvements.cpp:508` — paired simulators run 120 frames unnecessarily. Reduce to 60.**
  Kind: implement.

###### Duplication (10)

- ✅ [3D_E-S0325] **Ts20-DU1. `tests/test_animation_sampler.cpp:101` — LinearQuat tests share 13 identical lines. Extract `makeQuatChannel(q0, q1)` helper.**
  Kind: implement.

- ✅ [3D_E-S0326] **Ts20-DU2. `tests/test_logger.cpp:83` — thread-spawn barrier duplicated in two tests. Extract `runConcurrent(nThreads, body)` helper.**
  Kind: implement.

- ✅ [3D_E-S0327] **Ts20-DU3. `tests/test_lip_sync.cpp:478` — blend-shapes setup byte-identical in two fixtures. Extract `setupLipSyncPipeline()`.**
  Kind: implement.

- ✅ [3D_E-S0328] **Ts20-DU4. `tests/test_command_history_dirty.cpp:29` — `IncrementCommand` defined twice. Move to shared `editor_command_test_helpers.h`.**
  Kind: implement.

- ✅ [3D_E-S0329] **Ts20-DU5. `tests/test_cloth_collision.cpp:25` — `makeTriangle`/`Quad`/`Cube` helpers not in `cloth_test_helpers.h`. Promote.**
  Kind: implement.

- ✅ [3D_E-S0330] **Ts20-DU6. `tests/test_cloth_presets.cpp:135` — 5×5 grid setup duplicated across 13 tests. Add `clothLiveParamConfig(w, h)` helper.**
  Kind: implement.

- ✅ [3D_E-S0331] **Ts20-DU7. `tests/test_fabric_material.cpp:267` — CollisionShapes group repeats boilerplate across 6 tests. Extract `CollisionShapeTest` fixture.**
  Kind: implement.

- ✅ [3D_E-S0332] **Ts20-DU8. `tests/test_motion_matching.cpp:586` — MotionDatabase build boilerplate in 5 consecutive tests. Extract fixture.**
  Kind: implement.

- ✅ [3D_E-S0333] **Ts20-DU9. `tests/test_subtitle.cpp:16` — `makeLine` helper duplicated across `test_subtitle.cpp` + `test_subtitle_renderer.cpp`. Extract to `subtitle_test_helpers.h`.**
  Kind: implement.

- ✅ [3D_E-S0334] **Ts20-DU10. `tests/test_ui_theme_accessibility.cpp:354` — Vellum/Plumbline contrast tests byte-identical. Use typed/parametrized test.**
  Kind: implement.

###### Splitting (4 — excludes SP3 above)

- ✅ [3D_E-S0335] **Ts20-SP1. `tests/test_cloth_collision.cpp` (698 lines, 6 subsystems) — split into `test_bvh.cpp`, `test_spatial_hash.cpp`, `test_cloth_mesh_collider.cpp`, `test_cloth_simulation_collision.cpp`.**
  Kind: implement.

- ✅ [3D_E-S0336] **Ts20-SP2. `tests/test_command_history.cpp:285` (780+ lines, 8 concerns) — extract `EntityActions` tests to `test_entity_actions.cpp`.**
  Kind: implement.

- ✅ [3D_E-S0337] **Ts20-SP4. `tests/test_formula_library.cpp:313` — `UnaryFunctions` tests 11 operators in one body. Split into named sub-tests or add `SCOPED_TRACE`.**
  Kind: implement.

- ✅ [3D_E-S0338] **Ts20-SP5. `tests/test_lip_sync.cpp` (745 lines, 6 suites) — extract `test_viseme_map.cpp`, `test_audio_analyzer.cpp`, `test_lip_sync_player.cpp`.**
  Kind: implement.

- ✅ [3D_E-S0339] **Ts20-SP6. `tests/test_ui_theme_accessibility.cpp:25` — `WithScaleMultipliesEveryPixelSize` packs 32 EXPECTs into one body. Split into 3-4 logical groups.**
  Kind: implement.

###### Assertions (10)

- ✅ [3D_E-S0340] **Ts20-AS2. `tests/test_animation_state_machine.cpp:247` — `SelfTransitionBlocked` ambiguous guard. Add explicit check or cite mechanism.**
  Kind: implement.

- ✅ [3D_E-S0341] **Ts20-AS3. `tests/test_advanced_physics.cpp:888` — `SeedBiasTowardImpact` threshold 5/20=25% barely above baseline. Tighten to `EXPECT_GE(nearImpact, 8)` or document.**
  Kind: implement.

- ✅ [3D_E-S0342] **Ts20-AS4. `tests/test_cloth_simulator.cpp:276` — `ClearSphereColliders` asserts only fall, not clearance. Add sphere, confirm penetration after clear.**
  Kind: implement.

- ✅ [3D_E-S0343] **Ts20-AS5. `tests/test_cloth_simulator.cpp:102` — stale pointer after `simulate()`. Copy scalar before simulate (match `UnpinRestoresMass` pattern at line 168).**
  Kind: implement.

- ✅ [3D_E-S0344] **Ts20-AS7. `tests/test_cloth_solver_improvements.cpp:456` — `ThickParticle_SphereCollisionOffsetIncludesRadius` only checks `!isnan`. Compute min-distance, assert offset.**
  Kind: implement.

- ✅ [3D_E-S0345] **Ts20-AS8. `tests/test_camera.cpp:156` — `ViewMatrixChanges*` use bool-differs scan. Assert specific elements.**
  Kind: implement.

- ✅ [3D_E-S0346] **Ts20-AS9. `tests/test_domain_systems.cpp:73` — reference-accessor tests void-discard returns. Assert observable properties.**
  Kind: implement.

- ✅ [3D_E-S0347] **Ts20-AS10. `tests/test_entity_serializer_registry.cpp:320` — substring `"2"` too loose. Assert `"dropped 2"` or `"2 component"`.**
  Kind: implement.

- ✅ [3D_E-S0348] **Ts20-AS13. `tests/test_scripting.cpp:153` — `ConvertFloatToString` checks only non-empty. Add `StartsWith("3.14")`.**
  Kind: implement.

- ✅ [3D_E-S0349] **Ts20-AS15. `tests/test_memory_tracker.cpp:100` — `RecordFreeUnderflowClampsAtZero` count-vs-byte underspecified. Add policy comment.**
  Kind: implement.

- ✅ [3D_E-S0350] **Ts20-BE1. `tests/test_emissive_lighting.cpp:45` — `AttenuationFormulaMathReference` derives + self-tests. Expose `EmissiveLightComponent::computeAttenuation()` and call it; or delete.**
  Kind: implement.

###### Hardcoded data (6)

- ✅ [3D_E-S0351] **Ts20-HA1. `tests/test_biome_preset.cpp:40` — `BuiltInPresets` checks names at hard-coded indices. Use set/contains.**
  Kind: implement.

- ✅ [3D_E-S0352] **Ts20-HA2. `tests/test_fit_history.cpp:39` — `makeEntry()` timestamp hard-coded; tie-break tests rely on lexicographic order. Add comment or parameterise.**
  Kind: implement.

- ✅ [3D_E-S0353] **Ts20-HA3. `tests/test_logger.cpp:127` — `MAX_ENTRIES` hard-coded locally. Expose `Logger::MAX_ENTRIES` as public constant.**
  Kind: implement.

- ✅ [3D_E-S0354] **Ts20-HA5. `tests/test_camera.cpp:258` — `near=0.1f`, `far=1000.0f` hard-coded. Expose `DEFAULT_NEAR_PLANE`/`DEFAULT_FAR_PLANE`.**
  Kind: implement.

- ✅ [3D_E-S0355] **Ts20-HA6. `tests/test_curve_fitter.cpp:309` — `FormulaPresetLibrary` hardcodes preset names + coefficients. Query at runtime; assert difference not literal.**
  Kind: implement.

- ✅ [3D_E-S0356] **Ts20-HA7. `tests/test_skeleton.cpp:61` — `MAX_JOINTS == 128` magic number not cross-referenced to shader. Add comment + `static_assert` if possible.**
  Kind: implement.

###### Coverage gaps (15)

- ✅ [3D_E-S0357] **Ts20-CV1. `tests/test_aabb.cpp:14` — no test for inverted min/max. Add `min > max` constructed AABB.**
  Kind: implement.

- ✅ [3D_E-S0358] **Ts20-CV3. `tests/test_benchmark.cpp:74` — `BenchmarkCsv` temp files unstamped. Use `vestigeTestStamp()`.**
  Kind: implement.

- ✅ [3D_E-S0359] **Ts20-CV4. `tests/test_catmull_rom_spline.cpp` — no tangent test at interior segment boundary on 3+ point spline. Add 3-point spline test.**
  Kind: implement.

- ✅ [3D_E-S0360] **Ts20-CV5. `tests/test_cloth_constraint_graph.cpp:246` — `DihedralCount` only tests 4×4. Add 3×2 (M=2, N=1).**
  Kind: implement.

- ✅ [3D_E-S0361] **Ts20-CV6. `tests/test_bloom_downsample_karis.cpp:53` — no centre=0/corner=bright test. Add reversed-intensity case.**
  Kind: implement.

- ✅ [3D_E-S0362] **Ts20-CV7. `tests/test_color_grading_parity.cpp:113` — GPU parity only voxel-centre inputs. Add inter-voxel inputs (0.1/0.5/0.9 fractions).**
  Kind: implement.

- ✅ [3D_E-S0363] **Ts20-CV8. `tests/test_engine_paths.cpp:25` — only `captionMapPath` tested. Add parametric tests for `shaderPath`/`fontPath`/etc. Reclassified FP (2026-05-20): no `shaderPath`/`fontPath` path helpers exist — they are inline string concatenations in `editor.cpp` / `terrain_renderer.cpp`. `captionMapPath` is the only `engine_paths` helper and already has 4 cases (default / absolute / trailing-slash / empty). No coverage gap.**
  Kind: implement.

- ✅ [3D_E-S0364] **Ts20-CV9. `tests/test_entity_serializer_depth_cap.cpp:50` — `AcceptsDepthAtBoundary` only tests 128, not 127. Add 127 case.**
  Kind: implement.

- ✅ [3D_E-S0365] **Ts20-CV10. `tests/test_event_bus.cpp:152` — `UnsubscribeDuringDispatch` missing self-removal case. Add A-removes-itself test.**
  Kind: implement.

- ✅ [3D_E-S0366] **Ts20-CV11. `tests/test_gltf_fs_sandbox.cpp:107` — no symlink-traversal test. Add `fs::create_symlink` (gate with try/catch).**
  Kind: implement.

- ✅ [3D_E-S0367] **Ts20-CV12. `tests/test_json_size_cap.cpp:59` — no test for caller-tag in logged error. Assert tag string in `Logger::getEntries()` after rejection.**
  Kind: implement.

- ✅ [3D_E-S0368] **Ts20-CV13. `tests/test_lip_sync.cpp:443` — `loadTrackFromTSV` error paths untested. Add tests for blank rows, non-numeric timestamps, negative times.**
  Kind: implement.

- ✅ [3D_E-S0369] **Ts20-CV14. `tests/test_point_shadow_map.cpp:36` — no MAX+1 rejection test. Add shadow-light over-limit case. Deferred (2026-05-20): the MAX cap lives in `Renderer::selectShadowCastingPointLights()` (renderer.cpp), which needs a GL context to instantiate — not headless-testable. A unit test needs a pure `selectShadowCasters(lights, maxCasters)` helper extracted from the renderer (a SUT-API change), deferred per the big-bucket policy at the top of this section rather than refactored inside a test-only sweep.**
  Kind: implement.

- ✅ [3D_E-S0370] **Ts20-CV15. `tests/test_pbr_material.cpp:294` — `BlinnPhongDefaultsUnchanged` only checks 3 of 5+ fields. Add ambient/opacity.**
  Kind: implement.

- ✅ [3D_E-S0371] **Ts20-CV16. `tests/test_skeleton_animator.cpp` — no ROTATION/SCALE channel tests; only TRANSLATION. Add rotation (90° Y) + scale tests.**
  Kind: implement.

###### Fixtures (3)

- ✅ [3D_E-S0372] **Ts20-FX1. `tests/test_script_templates.cpp:25` — `TemplateRegistry` static initialised once with bool guard, not per-test. Replace with `::testing::Test` fixture.**
  Kind: implement.

- ✅ [3D_E-S0373] **Ts20-FX2. `tests/test_settings.cpp:42` — `TmpDir` local reimplements `vestigeTestStamp()`. Consolidate.**
  Kind: implement.

- ✅ [3D_E-S0374] **Ts20-FX3. `tests/test_editor_viewers.cpp:21` — `TextureViewerTest`/`HdriViewerTest`/`ModelViewerTest` empty fixtures. Convert to `TEST()`.**
  Kind: implement.

###### Parametrisation (6)

- ✅ [3D_E-S0375] **Ts20-PA1. `tests/test_caption_map.cpp:200` — `FiresOnPlaySound*` tests repeat identical setup. Use `PlaySoundInvoker` struct with `TEST_P`.**
  Kind: implement.

- ✅ [3D_E-S0376] **Ts20-PA2. `tests/test_color_vision_filter.cpp:49` — `Protanopia/Deuteranopia/Tritanopia` blocks identical. `INSTANTIATE_TEST_SUITE_P` over `{mode, expected-matrix}`.**
  Kind: implement.

- ✅ [3D_E-S0377] **Ts20-PA3. `tests/test_pbr_material.cpp:87` — `Metallic/Roughness/Ao/Emissive/Clearcoat` clamp pattern identical. Parametrise.**
  Kind: implement.

- ✅ [3D_E-S0378] **Ts20-PA4. `tests/test_editor_viewers.cpp:215` — 6 template-find tests follow same pattern. Extract `findTemplate(GameTemplateType)` helper.**
  Kind: implement.

- ✅ [3D_E-S0379] **Ts20-PA5. `tests/test_instanced_rendering.cpp:131` — `SingleInstanceBelowThreshold`/`TwoInstancesMeetsThreshold` assert counts not path. Expose `batchUsesInstancing` flag or rename.**
  Kind: implement.

- ✅ [3D_E-S0380] **Ts20-PA6. `tests/test_ui_world_projection.cpp:89` — `FadeAlpha` boundary tests separate per distance variant. Table-driven `{distance, expectedAlpha}`.**
  Kind: implement.

###### Doc-strings (4)

- ✅ [3D_E-S0381] **Ts20-DS1. `tests/test_aabb.cpp:63` — `IntersectsTouching` undocumented closed/half-open. Add comment: "touching AABBs treated as intersecting (closed intervals)".**
  Kind: implement.

- ✅ [3D_E-S0382] **Ts20-DS2. `tests/test_audio_ambient.cpp:192` — `deltaSeconds=0.0f` unclear intent. Add comment explaining `>=` semantics.**
  Kind: implement.

- ✅ [3D_E-S0383] **Ts20-DS4. `tests/test_cloth_solver_backend.cpp:38` — seed parameter undocumented. Add comment; use explicit seed on line 60.**
  Kind: implement.

- ✅ [3D_E-S0384] **Ts20-DS5. `tests/test_foliage_chunk.cpp` — original FoliageManager tests have no per-test comments. Add contract notes (density/spacing/jitter).**
  Kind: implement.

- ✅ [3D_E-S0385] **Ts20-DS7. `tests/test_water_surface.cpp:1` — no GL-context precondition in file header. Add to `@brief`: "tests run without GL context (VAO/mesh ops skipped)".**
  Kind: implement.

###### Verbosity (3)

- ✅ [3D_E-S0386] **Ts20-VE2. `tests/test_stasis_system.cpp:54` — `DefaultValues` checks 9 fields individually. Use `EXPECT_EQ(state, StasisState{})` if `operator==` available.**
  Kind: implement.

- ✅ [3D_E-S0387] **Ts20-VE3. `tests/test_scripting.cpp:1029` — `NodeLibraryTest` math tests repeat long accessor chain 10+ times. Extract `getOutputFloat(...)` helper.**
  Kind: implement.

- ✅ [3D_E-S0388] **Ts20-VE4. `tests/test_navigation_panel.cpp:44` — `OverlayColorIsInZeroToOneRange` packs GE/LE per line. Split / add labels.**
  Kind: implement.
...
- ✅ [3D_E-S0389] **CE1. Decide whether to commit a project `.clang-tidy` config at repo root, or amend AUDIT_STANDARDS.md §3 to state that clang-tidy is per-developer config only. (Today: no `.clang-tidy` exists; CODING_STANDARDS.md §17 says "planned but not yet present.")**
  Kind: implement.

- ✅ [3D_E-S0390] **CE2. Decide whether to pin Dear ImGui (`docking`), imgui-filebrowser (`master`), ImPlot (`master`) to specific commit SHAs in `external/CMakeLists.txt`, or formally document them as branch-tracking exceptions in SECURITY.md §5. (Currently: THIRD_PARTY_NOTICES.md notes the exceptions under "Branch-tracking deps"; SECURITY.md §5 carries a softened "with three documented exceptions" line. Final disposition is owner's call.)**
  Kind: implement.

- ✅ [3D_E-S0391] **CE3. Land `engine/utils/result.h` with the `Result<T,E> = std::expected<T,E>` alias (CODING_STANDARDS.md §11 and `phase_11a_design.md` §9 both depend on it). Until then, both docs say "use the standard name directly" — a deferred code item.**
...
- ✅ [3D_E-S0392] **CE4. Settings-schema migration ownership conflict between `phase_10_5_first_run_wizard_design.md` (proposes v1→v2 migration) and `phase_10_localization_design.md` (also proposes v1→v2). `engine/core/settings.h:64` already pins `kCurrentSchemaVersion = 2`, so the active question is who owns v2→v3. Lock the sequence in both docs before either slice lands.**
  Kind: implement.

- ✅ [3D_E-S0393] **CE5. Scene-format-version bump coordination: `phase_10_audio_music_player_design.md` Slice 8 plans `CURRENT_FORMAT_VERSION 1 → 2`. If another phase-10 doc also touches scene-JSON fields, the bump owner needs to be designated and the other doc has to fold its fields into the same v1→v2 migration (or wait for v2→v3).**
  Kind: implement.

- ✅ [3D_E-S0394] **CE6. `phase_10_8_camera_modes_design.md` §4.3 body still describes `CinematicCameraMode` with separate `(SplinePath*, FovTrack*, float t)` inputs, but §9 approval log (Q6) says the approved shape is a `CinematicTake` bundle. Rewrite §4.3 (and the §4.6 / Slice CM7 references) to use the approved bundle before any implementer reads §4.3 as authoritative.**
  Kind: implement.

- ✅ [3D_E-S0395] **CE7. `phase_10_7_design.md` Slice A1 (`AudioBus` field on `AudioSourceComponent`) — the field already exists at `engine/audio/audio_source_component.h:49`. Verify the serializer side too, then re-classify the rest of Slice A1 as "field present, wire serializer + gain-resolution pass" so implementers don't re-add the field.**
  Kind: implement.

- ✅ [3D_E-S0396] **CE8. Add `docs/phases/phase_10_index.md` mirroring `phase_08_index.md` / `phase_09_index.md`. The Phase 10.x sub-phase mesh (10.5 / 10.7 / 10.8 / 10.fog / 10.settings / 10.ui / 10.localization / 10.audio-music / 10.9 W-slices) has no index doc, and readers across both this project and the cold-eyes-2026-05-18 sweep struggled to navigate it.**
  Kind: implement.

**Doc structure / clarity follow-ups (mechanical, low priority — bundle into a sweep when next adjacent):**

- ✅ [3D_E-S0397] **CE9. Apply SPEC_TEMPLATE.md §8 "all-TBD one-liner" form to the eight engine specs whose §8 tables are still full-table-of-TBDs (physics, audio, input, ui, scripting, navigation, formula, editor — environment may already comply, verify). Template line: "if every cell is TBD, replace the table with a single line."**
  Kind: implement.

- ✅ [3D_E-S0398] **CE10. Reconcile audio-spec §15 closed-question convention with editor-spec convention. Audio spec keeps struck-out closed Qs in §15 for one revision; editor spec says §15 is open-only and closed Qs live in §16. Pick one and update the other plus SPEC_TEMPLATE.md to codify.**
  Kind: implement.

- ✅ [3D_E-S0399] **CE11. `phase_10_audio_music_player_design.md` step 7b proposes a `docs/engine/audio/spec.md` amendment. Land the amendment (or close as not-needed) before code-implementation step 9 so the doc-vs-spec drift can't leak into shipped code.**
  Kind: implement.

- ✅ [3D_E-S0400] **CE12. Phase 24 design doc still lacks an Accessibility section and a Change log per SPEC_TEMPLATE shape; add when phase enters implementation. CPU/GPU section was added in this sweep.**
  Kind: implement.

- ✅ [3D_E-S0401] **CE13. Renderer spec §6 / `renderer.cpp:2656` / `:693` / `:756` style line citations are brittle — rewrite to function-name-only (`renderer.cpp::renderScene`) when next touched. Same advice for any spec citing `engine.cpp:NN` exact lines.**
  Kind: implement.

- ✅ [3D_E-S0402] **CE14. ARCHITECTURE.md §19 audit-ID block ("M9/M10/M11 + L6") — confirm against which audit cycle these IDs come from; the v0.1.0 CHANGELOG §M sequence skips M11. Either re-label with the right cycle or scrub the IDs in favour of a commit-SHA pointer.**
  Kind: implement.

- ✅ [3D_E-S0403] **CE15. Spec §15 open-question housekeeping: physics §15 Q16 self-flags CODING_STANDARDS §30 drift (`engine/physics/jolt_layers.h` → `physics_layers.h`); fold the corresponding fix into the standards doc when next touched.**
  Kind: implement.

**Long-deferred (acknowledged-but-not-this-cycle):**

- ✅ [3D_E-S0404] **CE16. Add a Table-of-Contents block at the top of ROADMAP.md (~2410 lines) and CHANGELOG.md (~9400 lines). Both currently rely on search/scroll; a TOC of `## Phase NN` / `## [vX.Y.Z]` anchors would cut implementer scan time at very low maintenance cost. Acknowledged LOW; not done this round.**
  Kind: implement.

- ✅ [3D_E-S0405] **CE17. Re-verify `RECOMMENDED_ROUTINES.md` path claims (`apps/benchmark`, `apps/walkthrough`, `tools/hook-on-version-edit.sh`) and the AUTOMATED_AUDIT_REPORT template reference now that audit reports are gitignored. Re-write the routine to describe the template inline rather than referencing the gitignored file.**
  Kind: implement.
...
- ✅ [3D_E-0706] **Fog design section 8 has two defects outside 3D_E-0657's change, found by its gate.**
  ...
  Resolved 2026-09-26 with the lanes' own proposed fixes: line 302 now
  names runBench in tests/perf_bench_helpers.h; line 308 now says a
  cheaper Low/Med configuration moves the Technique cell and the measured
  figure, agreeing with lines 277 and 294 that 1.75 ms stands; line 348
  now says wintest.sh runs the Release binary local-ci.sh --windows
  builds.
...
- 📋 [3D_E-S0406] **Command palette (Ctrl+Shift+P / Cmd+Shift+P) — fuzzy search over every menu item, panel, tool, and action with keyboard-only navigation. One surface to find anything without memorising menus.**
  Kind: implement.

- 📋 [3D_E-S0407] **"What can I do here?" contextual help overlay (F1 over any viewport / panel shows a tooltip of its purpose + the keyboard shortcuts + the 5 most-used actions).**
  Kind: implement.

- 📋 [3D_E-S0408] **Searchable settings / preferences (the accessibility + input + graphics preferences are scattered; a single Ctrl+, search-over-keys dialog surfaces them).**
  Kind: implement.

- 📋 [3D_E-S0409] **Panel launcher with pinning — first-run shows a pinnable "Most-used panels" strip so the common Window submenu discovery isn't a memorisation exercise.**
  Kind: implement.

- 📋 [3D_E-S0410] **In-editor glossary — hover any domain term (e.g. "navmesh", "froxel", "cascade", "IBL") to see a 1-2 sentence definition + link to the relevant docs page.**
  Kind: implement.

##### Onboarding — "I just installed this; now what?"

- 📋 [3D_E-S0411] **First-run welcome dialog — project template picker (empty / 3D first-person / 2.5D / isometric / biblical walkthrough / blank biblical template) with live preview thumbnails. Already partially covered by Phase 9D template system; this adds the first-run wrapper.**
  Kind: implement.

- 📋 [3D_E-S0412] **Guided tour (dismissable, resumable) — highlights the 6 things a new user needs to know: viewport navigation, panel layout, entity inspector, assets panel, save/load, play mode. ~2 minutes total, spatially anchored callouts.**
  Kind: implement.

- 📋 [3D_E-S0413] **"Next step" hint pane — surfaces the single most useful next action given the current scene state ("scene has no lighting — add a directional light?", "entity has no collider — add a box collider?"). Dismissable per-project.**
  Kind: implement.

- 📋 [3D_E-S0414] **Sample scenes shipped with the engine — each template project opens with a minimal scene that shows the system working, not a blank viewport.**
  Kind: implement.

- 📋 [3D_E-S0415] **Opt-in telemetry (local file only, never uploaded) — tracks which panels the user opens so the first-run tour can adapt.**
  Kind: implement.
...
- 📋 [3D_E-S0416] **EU1. Workspace presets — named layouts (`Modeling` / `Lighting` / `Audio` / `Scripting` / `Animation` / `Play-test`) selectable from a top-bar tab strip à la Blender. Each preset stores panel arrangement + open assets + last-used tool. Per-project override of the global default. Saves the "I just opened the editor and now I have to redock 8 panels" friction every time the user task-switches.**
  Kind: implement.

- 📋 [3D_E-S0417] **EU2. Detachable / multi-monitor panels — any panel can be torn off into a free-standing OS window (ImGui docking branch already supports `ImGuiConfigFlags_ViewportsEnable`; just needs wiring + persistence). Massive QoL on 2-monitor setups (Inspector on the second display while Viewport fills the main).**
  Kind: implement.

- 📋 [3D_E-S0418] **EU3. Live keymap editor — `Edit → Keymap` panel listing every action with: current binding, default binding, conflict highlight (any two actions sharing a chord), filter-by-keyword search, import/export `*.keymap.json`, per-keymap-preset switch (Unity / Unreal / Blender mapping packs). Pairs with the existing remap shipped for in-game input.**
  Kind: implement.

- 📋 [3D_E-S0419] **EU4. Universal "Go to anything" (`Ctrl+P` à la JetBrains / VSCode) — fuzzy-finds across: open scenes, assets, entities by name, components by type, scripts, docs pages, recent files. One surface for "where is X". Distinct from EU's command palette (Ctrl+Shift+P) which finds *actions*; this finds *things*.**
  Kind: implement.

- 📋 [3D_E-S0420] **EU5. Recent-positions stack + bookmarks — `Alt+Left`/`Alt+Right` walks back / forward through the last 20 viewport camera positions + selected entities (browser-style nav). `F2..F9` save / restore named camera bookmarks per scene. Huge time-saver in large architectural scenes.**
  Kind: implement.

- 📋 [3D_E-S0421] **EU6. Scene diff + merge — `Tools → Compare Scenes` shows a tree-diff between two `.scene` JSON files (added / removed / modified entities, with per-component drill-in). 3-way merge for conflicts. Removes "git merge conflict in a 10k-line scene JSON = nightmare" from the multi-user-project lifecycle.**
  Kind: implement.

- 📋 [3D_E-S0422] **EU7. In-editor frame profiler panel — Tracy-protocol client (Tracy is BSD-3, server is independent) wired to the existing `frame_diagnostics`. Per-frame flame graph of subsystem cost + the lock-contention data from MT15. No external app needed. Pairs with the perf-overlay already shipped in Phase 5F.**
  Kind: implement.

- 📋 [3D_E-S0423] **EU8. Embedded REPL / console — `View → Console` panel with a Lua / Wren / chosen-scripting REPL bound to the live scene. Type `Scene.findByName('Wall_03').transform.position.y = 2.5` and watch it apply. Mirrors Godot's REPL + Unity's debug console; speeds up "what does this object's state look like right now?" debugging.**
  Kind: implement.

- 📋 [3D_E-S0424] **EU9. Viewport shading modes hotkey — `Alt+1..6` cycles Wireframe / Shaded / Unlit / Lighting-only / Overdraw / Wireframe-on-shaded. Standard across Blender / Unreal / Unity. Today the project has none of these surface-able shortcuts; deeply useful when diagnosing artifacts.**
  Kind: implement.

##### Workflow ergonomics — "I know what to do; make it fast"

- 📋 [3D_E-S0425] **Keyboard-driven workflow parity — every mouse action in the scene viewport should have a keyboard shortcut (move, rotate, scale, duplicate, delete, parent, frame-selected, toggle-gizmo-space).**
  Kind: implement.

- 📋 [3D_E-S0426] **Chord shortcuts (Ctrl+K Ctrl+S, Leader-style) for advanced actions — matches VSCode/Emacs user expectations and avoids key-space starvation.**
  Kind: implement.

- 📋 [3D_E-S0427] **Undo/redo everywhere, not just scene edits — panel layouts, preference changes, asset operations, workbench fits, shader parameters. Phase 9 started this; polish across remaining surfaces.**
  Kind: implement.

- 📋 [3D_E-S0428] **Unified copy/paste for entities, components, materials, and node-graph subgraphs — same shortcuts, same clipboard.**
  Kind: implement.

- 📋 [3D_E-S0429] **Drag-and-drop across every panel (model viewer → scene, texture viewer → material slot, HDRI viewer → environment, asset panel → entity component).**
  Kind: implement.

- 📋 [3D_E-S0430] **Multi-select editing — select N entities, edit one component field, apply to all.**
  Kind: implement.

- 📋 [3D_E-S0431] **Auto-save + recovery — every N minutes, on window lose-focus, on process crash. File-based, not memory-only.**
  Kind: implement.

- 📋 [3D_E-S0432] **Project-relative paths — the editor must never bake absolute paths into saved scenes (moving a project directory shouldn't break it).**
  Kind: implement.

##### Tooltips & contextual help — "what does this do?"

- 📋 [3D_E-S0433] **Every widget has a tooltip with a 1-sentence description + the keyboard shortcut if any. Audit every panel.**
  Kind: implement.

- 📋 [3D_E-S0434] **Status-bar hints — hovering a menu item shows the full description + link to the relevant docs page.**
  Kind: implement.

- 📋 [3D_E-S0435] **Inline warnings on invalid inputs (red outline + explanation of what's wrong + "fix" button where possible). Already partial in the Formula Workbench; extend to every numeric / enum / path input across the editor.**
  Kind: implement.

- 📋 [3D_E-S0436] **"Why is this greyed out?" — right-click a disabled control and get the reason ("Bake button disabled: no navmesh geometry tagged in the scene").**
  Kind: implement.

##### AI assistance integration hooks

- 📋 [3D_E-S0437] **Editor-exposed command API — every menu action, panel operation, and entity mutation callable via a stable string-based command ID. Same surface as the command palette. Enables AI assistants to drive the editor without hooking directly into C++ headers.**
  Kind: implement.

- 📋 [3D_E-S0438] **Scene-state snapshot / diff format — JSON serialisation of the current scene suitable for feeding to an AI assistant as context (entity list, component values, current selection). Already exists partially via scene serialisation; this formalises the contract.**
  Kind: implement.

- 📋 [3D_E-S0439] **AI chat panel (optional, off-by-default) — hostable via any Claude API / Anthropic SDK key in the user's environment. Does NOT require an AI vendor to use the editor; the editor is fully functional without it. Matches the Rule "editor must be usable without AI".**
  Kind: implement.

- 📋 [3D_E-S0440] **Prompt templates stored per-project — "add a point light near the selected entity", "generate 5 variations of this material", "create a character controller for this mesh". Users without AI get a "prompt library" of suggested workflows as a discovery aid.**
  Kind: implement.

- 📋 [3D_E-S0441] **Keyboard-driven agent invocation (Alt+Enter) — text box where the user types a short description, agent interprets → proposes a diff → user accepts / rejects. Deferred until there's a stable command API.**
  Kind: implement.

##### Performance & responsiveness — "the editor shouldn't feel slow"

- 📋 [3D_E-S0442] **Audit every editor panel's per-frame cost; target < 1 ms per panel at idle. The editor overlay should never push the scene-viewport frame time below 60 FPS on target hardware.**
  Kind: implement.

- 📋 [3D_E-S0443] **Async asset imports — model / texture / HDRI imports must not block the editor UI. Progress shown in a status bar.**
  Kind: implement.

- 📋 [3D_E-S0444] **Incremental scene saves — a 10,000-entity scene shouldn't take 2 seconds to save. Chunk by dirty regions.**
  Kind: implement.

- 📋 [3D_E-S0445] **Panel-level collapse to single line — users rarely need all 30 panels visible; aggressive one-line collapse of inactive panels reclaims screen space.**
  Kind: implement.

##### Accessibility (editor-side, complementing the engine-side Phase 10 work)

- 📋 [3D_E-S0446] **Editor UI scaling presets (independent of the game UI scaling shipped in Phase 10) — 1.0× / 1.25× / 1.5× / 2.0×.**
  Kind: implement.

- 📋 [3D_E-S0447] **Editor high-contrast mode — mirrors the game UI high-contrast toggle but applied to ImGui panels, menu chrome, and the scene viewport gizmos.**
  Kind: implement.

- 📋 [3D_E-S0448] **Screen-reader labels on every ImGui widget (extends the Phase 10 `UIAccessibleRole` enum to the editor surface — currently only the in-game `UIElement` tree is covered).**
  Kind: implement.

- 📋 [3D_E-S0449] **Colourblind-safe gizmo / wireframe / selection palettes.**
  Kind: implement.

- 📋 [3D_E-S0450] **Keyboard-only workflow — verify every panel is navigable with Tab / Shift+Tab / Enter / Space and no mouse. This is the big-ticket item from partially-sighted / motor-impaired users.**
  Kind: implement.

##### Docs surface — "how do I learn this?"

- 📋 [3D_E-S0451] **In-editor help browser — dedicated `Help → Documentation` panel (or `F1` shortcut) that renders the shipped docs inline as a searchable, navigable markdown viewer. No external browser needed. Contents:**
...
- 📋 [3D_E-S0452] **Context-sensitive "Help with this" affordance — every panel, widget group, and major editor feature exposes a small `?` or `Help` button that deep-links into the in-editor help browser at the exact topic for that feature. Examples:**
...
- 📋 [3D_E-S0453] **Help anchor registry — a central map (widget id / feature id → `docs/file.md#anchor`) so `Help with this` buttons across the editor always resolve consistently. Source of truth for which docs page owns which feature; rebinding a help anchor is a one-line change rather than hunting through ImGui code. Editor-internal; never exposed to user-facing strings.**
  Kind: implement.

- 📋 [3D_E-S0454] **Help authored inline when it lives alongside code — every new subsystem ships with a matching `docs/<SUBSYSTEM>.md` page following a template (Overview / When to use it / How to use it in the editor / API reference / Gotchas). Post-phase audit checks that every user-facing feature has a help anchor pointing somewhere non-empty.**
  Kind: implement.

- 📋 [3D_E-S0455] **Video / GIF embeds in panel tooltips for complex operations (one-shot 3-5 second MP4s).**
  Kind: implement.

- 📋 [3D_E-S0456] **Troubleshooting decision tree — "my scene doesn't render" → click-through diagnostic ending in a fix or a GitHub-issue prefill.**
  Kind: implement.

##### Code-hygiene tooling — stop the clang-tidy whack-a-mole

- 📋 [3D_E-S0457] **Principled clang-tidy baseline. Replace the current `bugprone-*,performance-*,modernize-*,readability-*,cppcoreguidelines-*` wildcard (with its ~8 excludes accumulated across audit tool 2.15.0 → 2.17.0) with an explicit enumeration of the ~30 checks the project actually wants active. Commit as a project-level `.clang-tidy` file so IDE / editor integrations pick it up too. Ends the "suppress the top rule, next layer appears" pattern documented in audit tool 2.15–2.17 CHANGELOG entries.**
  Kind: implement.

- 📋 [3D_E-S0458] **Bulk clang-format sweep. Whole-codebase `clang-format -i` pass against the existing `.clang-format` config (which has been deliberately deferred per the file's own header comment). Land in a single dedicated commit so git blame damage is contained to one reviewable unit. Re-enables `readability-braces-around-statements` once the unbraced sites are cleaned up by this pass or by the hybrid-adoption path (CODING_STANDARDS.md §3).**
  Kind: implement.
...
- 📋 [3D_E-S0473] **MT4. ThreadSanitizer build target + CI gate. `cmake -DENGINE_TSAN=ON` + a tagged test suite that exercises the threaded subsystems (cloth GPU dispatch, async texture, logger, autosave) — extended each time a system goes multi-threaded. Gate on the nightly workflow that already exists for the cppcheck audit.**
  Kind: implement.

##### Render path

- 📋 [3D_E-S0474] **MT5. Render-thread split — the existing single-threaded loop becomes: (sim thread) gameplay + physics + animation + culling → produces a `RenderCommandBuffer` snapshot → (render thread) issues GL calls + swap. One-frame latency added; ~15–25 % FPS uplift on the average CPU-bound scene (the editor's per-panel ImGui passes alone are ~3 ms today). Sim thread runs at the desired tick rate; render thread runs at vsync. Prerequisite for MT6 and for the Vulkan backend in Phase 13.**
  Kind: implement.

- 📋 [3D_E-S0475] **MT6. Parallel render-command building — split the `RenderCommandBuffer` into N spatial buckets (typically frustum octants); each bucket built by one job in parallel. Per-bucket buffer then merged via deterministic ordering before submission. Mirrors UE4/idTech 7 secondary-command-buffer pattern. Pairs with the GPU-driven MDI work in Phase 13 `gpu-driven-rendering`; CPU prep stops being the bottleneck before that work lands.**
  Kind: implement.

- 📋 [3D_E-S0476] **MT7. Parallel frustum culling + draw-list build — current `Renderer::cull` walks all entities serially. Convert to a parallel-for over `m_drawables` with per-thread output vectors merged at the end. Cheap, no API change for callers, immediate win on dense scenes. Foundation for MT6.**
  Kind: implement.

##### Simulation path

- 📋 [3D_E-S0477] **MT8. Wire Jolt's existing job system — Phase 8 chose Jolt explicitly for "superior multi-threading" but the project today runs it on a default-constructed `JobSystemSingleThreaded`. Swap to `JobSystemThreadPool` (Jolt's built-in) sharing our MT2 worker pool via a `JobSystem` adaptor. Free 2–4× physics throughput on RX 6600-class CPU; mandatory before vehicle physics (Phase 11B) and ragdoll cluster (Phase 10.9 Slice 8 W13) ship.**
  Kind: implement.

- 📋 [3D_E-S0478] **MT9. Parallel ECS system scheduler — extend `SystemRegistry` with per-system read/write component declarations (`reads<Transform>(), writes<Velocity>()`). Build a per-frame dependency DAG; non-conflicting systems run in parallel. Pattern from Bevy's ECS scheduler + Unity DOTS — both are documented in papers/blog posts, no proprietary code reuse needed. Replay-determinism gate: systems on the determinism set (Phase 11A § 3.4) stay on a single ordered worker; the scheduler ensures stable ordering across runs.**
  Kind: implement.

- 📋 [3D_E-S0479] **MT10. Parallel animation + IK update — skeletal animation tick is embarrassingly parallel per-skeleton. Convert `AnimationSystem::update` to a job-per-skeleton parallel-for; same for IK solvers. Closes one of the largest CPU costs in any scene with > 30 NPCs.**
  Kind: implement.

- 📋 [3D_E-S0480] **MT11. Parallel-safe event bus — current `EventBus` (Phase 9A) was authored single-threaded. Audit + lock: either (a) MPMC queue for cross-thread events + drain on main thread, or (b) per-thread queues merged each frame. Picks the simpler option that preserves ordering guarantees the existing 130+ event-bus tests pin.**
  Kind: implement.

##### Asset + I/O path

- 📋 [3D_E-S0481] **MT12. Generalise the async loader — `AsyncTextureLoader` (existing) is texture-only. Promote to `AsyncAssetLoader<T>` covering model / texture / HDRI / audio / scene-tile loads. Use C++20 coroutines (`co_await loadAsync(path)`) so the call sites stay linear-looking. License-safe coroutine helper: `concurrencpp` (MIT) or a tiny in-house wrapper over `<coroutine>` — both fit the global rule on latest-idiom external-library use.**
  Kind: implement.

- 📋 [3D_E-S0482] **MT13. io_uring path for Linux asset loads (DirectStorage-equivalent without proprietary dep) — batch async reads via `liburing` (LGPL-2.1 with linking exception; fine for our use). GPU-side BCn / BC7 decompression in a compute shader (clean-room from public KhronosGroup / Compressonator notes, no proprietary GDeflate / RTX-IO dependency). Closes the "level load takes 8s vs 1.5s" gap with AAA engines.**
  Kind: implement.

##### Audit / safety

- 📋 [3D_E-S0483] **MT14. Per-system "main-thread-only" assertions — `VESTIGE_ASSERT_MAIN_THREAD()` macro using `std::this_thread::get_id()` vs a captured main-thread id. Apply to every GL call site + ImGui call + GLFW input poll. Catches the inevitable "called GL from a worker" bug in debug builds before TSAN does in CI.**
  Kind: implement.

- 📋 [3D_E-S0484] **MT15. Lock-contention profiler hook — record contention events to the existing performance overlay (`engine/core/frame_diagnostics.cpp`). Surfaces "X% of frame time waiting on lock Y" so we tune from data, not guesses.**
  Kind: implement.
...
- 📋 [3D_E-S0498] **ACES 2.0 tonemap option (opt-in alternative to the ACES 1.3 default; kept side-by-side for HDR output, user-selectable)**
  Kind: implement.

- 📋 [3D_E-S0499] **Film grain (noise overlay, configurable intensity and grain size)**
  Kind: implement.

- 📋 [3D_E-S0500] **Chromatic aberration (RGB channel offset, stronger at screen edges)**
  Kind: implement.

- 📋 [3D_E-S0501] **Vignette (screen edge darkening, configurable intensity and radius)**
  Kind: implement.

- 📋 [3D_E-S0502] **Screen distortion (barrel/pincushion, heat haze, damage effects)**
  Kind: implement.

- 📋 [3D_E-S0503] **Damage/low-health screen effects (red vignette, blur, heartbeat pulse, desaturation) — consumed by Phase 11B Health & Damage feedback.**
  Kind: implement.

- 📋 [3D_E-S0504] **Death screen effects (slow desaturation, blur ramp, fade to black) — consumed by Phase 11B Health.**
  Kind: implement.

- 📋 [3D_E-S0505] **Color grading per-scene (warm golden interiors, cold blue corridors, sickly green toxic areas)**
  Kind: implement.
...
- 📋 [3D_E-S0508] **Screen-space reflections / SSR (real-time reflections on wet floors, polished bronze/gold — complements IBL)**
  Kind: implement.

- 📋 [3D_E-S0509] **Screen-space global illumination / SSGI (one-bounce diffuse indirect lighting from depth+color buffers — practical "free" GI layer, complements probe-based approaches)**
  Kind: implement.

- 📋 [3D_E-S0510] **Depth of field (cinematic focus effect for guided tours and screenshots)**
  Kind: implement.

- 📋 [3D_E-S0511] **Motion blur (per-object and camera-based)**
  Kind: implement.

- 📋 [3D_E-S0512] **Bent normals (GTAO-derived least-occluded direction — feeds specular occlusion and gives diffuse GI / IBL a directional term instead of a flat AO scalar; runtime path extends the existing SSAO, with optional baked bent-normal maps as the offline complement)**
  Kind: implement.
...
- 📋 [3D_E-S0515] **Strand-based hair and fur rendering (physically-based hair model with proper light scattering — relevant for animal fur, priestly garment fringes; ref: MachineGames/Indiana Jones, SIGGRAPH 2025)**
  Kind: implement.
...
- 📋 [3D_E-S0517] **Neural texture compression (2-4x memory reduction via trained neural decompression in shaders — forward-looking, requires cooperative vector hardware support)**
  Kind: implement.
...
- 📋 [3D_E-S0518] **Baked lightmaps with SH-fit storage (WishGI approach — pre-computed GI for static architectural scenes stored as per-mesh SH probes via inverse distribution; ~5% of classic lightmap memory, fragment-shader sampling with no extra pass; ref: Zhu et al. "WishGI" SIGGRAPH 2025 — https://dl.acm.org/doi/10.1145/3730935)**
  Kind: implement.

- 📋 [3D_E-S0519] **Light probes (capture local lighting conditions at probe positions — varying lighting between rooms)**
  Kind: implement.

- 📋 [3D_E-S0520] **Reflection probes (local cubemap captures for accurate indoor reflections — Holy Place vs Holy of Holies)**
  Kind: implement.

- 📋 [3D_E-S0521] **Light probe blending (smooth transitions between probe volumes)**
  Kind: implement.

- 📋 [3D_E-S0522] **Real-time irradiance probe GI (idTech 8 "Fast as Hell" approach — stochastic probe sampling with surfel shading, fully dynamic, proven at 60 FPS on consoles; replaces baked lighting with real-time probes; ref: Sousa SIGGRAPH 2025 — https://advances.realtimerendering.com/s2025/content/SOUSA_SIGGRAPH_2025_Final.pdf)**
  Kind: implement.

- 📋 [3D_E-S0523] **Surfel + SH probe hybrid (surfels sample indirect irradiance via software SDF ray-marching, write into the SH probe grid, temporal accumulation smooths residual noise; geometry-agnostic, no hardware RT required; pairs the idTech 8 surfel allocator with Vestige's planned SH grid)**
  Kind: implement.

- 📋 [3D_E-S0524] **Brixelizer-style software ray-traced GI via sparse SDF fields (compute-only, RDNA2-feasible — primary software-RT fallback for diffuse/specular indirect before the Vulkan/HW-RT backend lands; ref: AMD FidelityFX Brixelizer GI — https://gpuopen.com/fidelityfx-brixelizer/)**
  Kind: implement.

- 📋 [3D_E-S0525] **Radiance cascades (Alexander Sannikov's approach — constant-cost GI independent of scene complexity and light count; 2D proven in production, 3D extension is active research area; ref: Holographic RC arXiv 2505.02041, JTLee98 3D Vulkan PoC)**
  Kind: implement.

- 📋 [3D_E-S0526] **ReSTIR GI (reservoir-based spatiotemporal importance resampling for indirect illumination — dramatically improves convergence when hardware RT is available)**
  Kind: implement.
...
- 📋 [3D_E-S0527] **Vulkan rendering backend (alternative to OpenGL)**
  Kind: implement.

- 📋 [3D_E-S0528] **Vulkan descriptor heap (VK_EXT_descriptor_heap — simplified resource binding, replaces legacy descriptor set model)**
  Kind: implement.

- 📋 [3D_E-S0529] **Ray tracing — reflections (hardware-accelerated on supported GPUs)**
  Kind: implement.

- 📋 [3D_E-S0530] **Ray tracing — ambient occlusion**
  Kind: implement.

- 📋 [3D_E-S0531] **Ray tracing — global illumination**
  Kind: implement.

- 📋 [3D_E-S0532] **ReSTIR DI (reservoir-based spatiotemporal importance resampling for direct illumination — enables hundreds of shadow-casting lights with stochastic evaluation at fixed cost; ref: NVIDIA RTXDI)**
  Kind: implement.
...
- 📋 [3D_E-S0534] **Opacity micromaps (encode alpha transparency directly in the acceleration structure — eliminates expensive any-hit shaders for foliage, fences, curtains)**
  Kind: implement.

- 📋 [3D_E-S0535] **Shader execution reordering / SER (group coherent RT shader invocations — up to 2x RT performance; mandatory in SM 6.9 / Vulkan equivalent)**
  Kind: implement.

- 📋 [3D_E-S0536] **Software ray tracing fallback (screen-space ray marching + voxel/probe traces for GPUs without hardware RT — the most widely deployed approach in shipping games, e.g. UE5 Lumen software path)**
  Kind: implement.

- 📋 [3D_E-S0537] **Deferred rendering pipeline**
  Kind: implement.

- 📋 [3D_E-S0538] **Forward+ / clustered forward rendering (tiled/clustered light culling so the current single-pass forward renderer scales to hundreds of dynamic lights without a G-buffer — keeps MSAA and ordered transparency working; complements the deferred pipeline above rather than replacing it)**
  Kind: implement.

- 📋 [3D_E-S0539] **Visibility buffer rendering (store triangle ID + barycentric coords instead of full G-buffer — compute-based material dispatch and deferred texturing; ref: idTech 8, DOOM: The Dark Ages)**
  Kind: implement.

##### Tessellation and Geometry

- 📋 [3D_E-S0540] **Tessellation (adaptive subdivision for nearby geometry)**
  Kind: implement.

- 📋 [3D_E-S0541] **Displacement mapping (height maps that actually move geometry, unlike normal maps)**
  Kind: implement.

- 📋 [3D_E-S0542] **Tessellated water surfaces (wave simulation via height maps)**
  Kind: implement.

- 📋 [3D_E-S0543] **Tessellated terrain (adaptive detail for landscapes)**
  Kind: implement.
...
- 📋 [3D_E-S0544] **3DGS forward rasterizer — compute-shader pre-sort by view-depth, then alpha-blended quad rasterization of anisotropic 3D Gaussians, depth-tested against the engine's existing Z-buffer so splat scenes composite cleanly with rasterized triangle geometry (no separate render pass required). Target: 60 FPS at ≤ 1 M splats on RX 6600. Ref: Kerbl et al. SIGGRAPH 2023 — https://repo-sam.inria.fr/fungraph/3d-gaussian-splatting/ ; UnrealSplat reference UE5 plugin — https://github.com/JI20/unreal-splat**
  Kind: implement.

- 📋 [3D_E-S0545] **KHR_gaussian_splatting glTF importer — load splat scenes via the Khronos-standardised glTF 2.0 extension (Feb 2026). Splats become a primitive type the existing glTF loader extends to handle, not a separate asset format. Ref: https://github.com/KhronosGroup/glTF/tree/main/extensions/2.0/Khronos/KHR_gaussian_splatting**
  Kind: implement.

- 📋 [3D_E-S0546] **Splat cluster culling — partition splats into spatial clusters (~64-256 splats / cluster), frustum + Hi-Z cull at cluster granularity in the GPU-driven culling compute pass. Reuses the GPU-driven MDI infrastructure (highest-ROI Phase 13 item above), no parallel culling pipeline needed.**
  Kind: implement.

- 📋 [3D_E-S0547] **Splat LOD — distant clusters render at reduced splat count via per-cluster importance sampling. Screen-coverage threshold curve authored through the Formula Workbench (per CLAUDE.md Rule 6 — no hand-coded magic constants for numerical design).**
  Kind: implement.

- 📋 [3D_E-S0548] **Motion vectors + TAA / FSR 2.x compatibility — emit screen-space motion vectors during the splat pass so the engine's existing TAA + FSR 2.x temporal upscaler stays effective on splat content (covers static-camera + moving-camera + moving-splat cases).**
  Kind: implement.

- 📋 [3D_E-S0549] **`GaussianSplatComponent` + scene-graph integration — scene-graph component for splat assets (path, transform, render order, opacity multiplier). Phase 5 editor inspector section so splat scenes drop into the scene like any other renderable. Persistence via the scene serializer's standard JSON path.**
  Kind: implement.

- 📋 [3D_E-S0550] **Asset-pipeline tooling — convert third-party `.ply` / `.splat` captures to the engine's KHR_gaussian_splatting glTF representation. Lives in `tools/asset_import/`. Scope is intentionally small; Khronos's standard removes the need for a custom format.**
  Kind: implement.
...
- 📋 [3D_E-S0556] **Virtual shadow maps (massive virtual texture shadow map — only allocate tiles visible to camera, consistent detail at all distances, eliminates cascade seams; data-structure portion feasible on OpenGL 4.5 via `ARB_sparse_texture`, mesh-shader-optimal version deferred to the Vulkan backend)**
  Kind: implement.

- 📋 [3D_E-S0557] **Percentage-closer soft shadows / PCSS (contact-hardening shadows — sharp near caster, soft further away; author the filter-radius curve via the Formula Workbench)**
  Kind: implement.

- 📋 [3D_E-S0558] **HypeHype stochastic tile-based lighting (first, simpler many-lights rung — two-stage tile resampling with stratified reservoirs, no RT or mesh shaders required; ref: Lempinen SIGGRAPH 2025 — https://advances.realtimerendering.com/s2025/content/s2025_stb_lighting_v1.1_notes.pdf)**
  Kind: implement.

- 📋 [3D_E-S0559] **MegaLights stochastic area shadows (fixed-budget evaluation of hundreds-to-thousands of shadow-casting lights via tile-based reservoir sampling + temporal reuse; SVGF-style denoiser; supersedes PCSS for area lights once SDF ray-marching is in place; ref: Narkowicz & Costa SIGGRAPH 2025 — https://advances.realtimerendering.com/s2025/content/MegaLights_Stochastic_Direct_Lighting_2025.pdf)**
  Kind: implement.
...
- 📋 [3D_E-S0563] **Custom spatial upscaler (Lanczos/bicubic + CAS sharpening — built from scratch as a learning exercise)**
  Kind: implement.

- 📋 [3D_E-S0564] **Frame generation (interpolate additional frames between rendered frames — AMD FSR 3.x or custom optical-flow-based approach; doubles perceived framerate at the cost of latency)**
  Kind: implement.

##### Anti-Aliasing and Filtering

- 📋 [3D_E-S0565] **Global anisotropic filtering quality setting (1x/2x/4x/8x/16x — applied to all scene textures)**
  Kind: implement.
...
- 📋 [3D_E-S0568] **[Highest-ROI OpenGL 4.5 item] GPU-driven MDI with Hi-Z occlusion culling (compute-shader Hi-Z build + `glMultiDrawElementsIndirectCount`; expected 10-30% FPS gain on >1k-object scenes; ref: Anno 117 Pax Romana GDC 2026, idTech 8 SIGGRAPH 2025 — https://schedule.gdconf.com/session/all-rays-lead-to-rome-next-gen-graphics-in-anno-117-pax-romana/915067)**
  Kind: implement.

- 📋 [3D_E-S0569] **GPU frustum and occlusion culling (Hi-Z occlusion culling in compute shader — skip objects hidden behind other objects; lands as part of the GPU-driven MDI item above)**
  Kind: implement.

- 📋 [3D_E-S0570] **Variable-rate shading / variable-rate compute (control shading rate per screen region — full rate for detail areas, reduced rate for flat surfaces; ref: idTech 8 VRCS)**
  Kind: implement.

- 📋 [3D_E-S0571] **GL_EXT_mesh_shader integration (OpenGL mesh shaders — replace vertex/geometry pipeline with task+mesh shader stages for GPU-driven geometry processing; available on AMD RDNA2+ via Mesa, avoids requiring Vulkan)**
  Kind: implement.

- 📋 [3D_E-S0572] **Bindless textures and resources (eliminate texture binding overhead — all textures resident and GPU-addressable)**
  Kind: implement.

- 📋 [3D_E-S0573] **Shader language unification via Slang (migrate GLSL shaders to Slang IR — compiles to SPIR-V for OpenGL 4.5 today via `ARB_gl_spirv` + `ARB_spirv_extensions`, and to Vulkan + DXIL + OptiX + CUDA for future backends; supports generics, interfaces, differentiable shaders; Khronos governance; production use in Source 2 (CS2, Dota 2); prerequisite for low-friction Vulkan backend — ref: https://shader-slang.org/)**
  Kind: implement.
...
- 📋 [3D_E-S0576] **OpenXR integration (cross-platform VR/AR runtime)**
  Kind: implement.

- 📋 [3D_E-S0577] **Stereoscopic rendering (dual-eye viewpoints with correct IPD)**
  Kind: implement.

- 📋 [3D_E-S0578] **VR locomotion system (teleport, smooth movement, comfort options)**
  Kind: implement.

- 📋 [3D_E-S0579] **VR interaction (grab, point, inspect objects)**
  Kind: implement.

- 📋 [3D_E-S0580] **VR performance target (90 FPS stereo — requires aggressive optimization)**
  Kind: implement.

- 📋 [3D_E-S0581] **Foveated rendering (reduce detail in peripheral vision on supported headsets)**
  Kind: implement.
...
- 📋 [3D_E-S0582] **★★★★★ Spatiotemporal blue noise (STBN) textures — swap the engine's**
...
- 📋 [3D_E-S0583] **★★★★★ Screen-space indirect lighting with visibility bitmask**
...
- 📋 [3D_E-S0584] **★★★★ Two-level BVH compute ray tracer (TLAS/BLAS) — prerequisite**
...
- 📋 [3D_E-S0585] **★★★ Hybrid SSR → compute-RT reflection fallback — keep the SSR**
...
- 📋 [3D_E-S0586] **★★★ Physical camera post-process stack — bokeh DoF (gather-based,**
...
- 📋 [3D_E-S0602] **Automatic mesh simplification (quadric error metrics, academic literature)**
  Kind: implement.

- 📋 [3D_E-S0603] **Cluster-based mesh decomposition (splitting meshes into independently cullable/LOD-able chunks — meshlets of ~64-256 triangles)**
  Kind: implement.

- 📋 [3D_E-S0604] **Screen-space error-driven selection (choose cluster detail based on projected pixel coverage)**
  Kind: implement.

- 📋 [3D_E-S0605] **Compute shader-driven culling and selection pipeline**
  Kind: implement.

- 📋 [3D_E-S0606] **Virtual geometry streaming (load/unload clusters on demand from disk)**
  Kind: implement.

- 📋 [3D_E-S0607] **Mesh shader integration (GL_EXT_mesh_shader on OpenGL / VK_EXT_mesh_shader on Vulkan — available on RDNA2+; task shaders cull meshlet clusters, mesh shaders emit surviving triangles)**
  Kind: implement.

- 📋 [3D_E-S0608] **Micropolygon rendering (ref: Ubisoft Anvil engine, GDC 2026 — GPU-driven streaming architecture with automatic LOD, no manual LOD creation, eliminates popping)**
  Kind: implement.

- 📋 [3D_E-S0609] **Hybrid rasterization (hardware for large triangles, compute for sub-pixel)**
  Kind: implement.

- 📋 [3D_E-S0610] **RT acceleration structure co-management (partitioned TLAS that tracks geometry LOD — when geometry detail changes, only affected BVH partitions rebuild; ref: NVIDIA RTX Mega Geometry foliage system)**
  Kind: implement.

- 📋 [3D_E-S0611] **Novel approaches: SDF-based geometry, surfel-based rendering, or other non-traditional representations**
  Kind: implement.
...
- 📋 [3D_E-S0612] **Procedural sky model (Rayleigh/Mie scattering) — physically-based atmosphere**
...
- 📋 [3D_E-S0613] **Full day/night cycle with sun/moon positioning**
...
- 📋 [3D_E-S0614] **Dynamic time-of-day lighting (sun color/intensity changes)**
...
- 📋 [3D_E-S0615] **Stars and night sky**
...
- 📋 [3D_E-S0616] **Ray-marched volumetric clouds — 3D noise-based cloud shapes rendered via ray marching**
...
- 📋 [3D_E-S0617] **Cloud shadow projection — clouds cast soft shadows on terrain and scene geometry**
  - Shadow map from cloud density projected along sun direction
  - Moving shadow patterns as clouds drift across the sky
  Kind: implement.

- 📋 [3D_E-S0618] **Cloud animation — wind-driven cloud movement and morphing**
...
- 📋 [3D_E-S0619] **Weather controller — central system that drives all weather-related subsystems**
...
- 📋 [3D_E-S0623] **Dust storms / sandstorms**
...
- 📋 [3D_E-S0625] **Fog and mist density changes with weather**
...
- 📋 [3D_E-S0626] **Lightning (storm weather state)**
...
- 📋 [3D_E-S0627] **Wind gusts tied to weather**
...
- 📋 [3D_E-S0628] **Crepuscular rays through cloud gaps — volumetric light shafts from sun through cloud openings**
...
- ✅ [3D_E-S0629] **Heightmap-based terrain (import or paint elevation in-editor) — Phase 5I-1**
  Kind: implement.

- ✅ [3D_E-S0630] **CDLOD quadtree LOD with per-vertex morphing — Phase 5I-1**
  Kind: implement.

- ✅ [3D_E-S0631] **Normal map + splatmap texturing (4 layers) — Phase 5I-2**
  Kind: implement.

- ✅ [3D_E-S0632] **Cascaded shadow map integration — Phase 5I-3**
  Kind: implement.

- ✅ [3D_E-S0633] **Terrain height/normal queries + raycast — Phase 5I-3**
  Kind: implement.

- ✅ [3D_E-S0634] **Terrain painting tools — raise, lower, smooth, flatten brushes — Phase 5I-4**
  Kind: implement.

- ✅ [3D_E-S0635] **Multi-layer terrain texturing (paint grass, dirt, rock, sand onto terrain) — Phase 5I-5**
  Kind: implement.

- ✅ [3D_E-S0636] **Terrain serialization — save/load heightmap + splatmap with scene — Phase 5I-6**
  Kind: implement.

- ✅ [3D_E-S0637] **Terrain LOD — distant terrain uses fewer triangles automatically — Phase 5I-1**
  Kind: implement.

- ✅ [3D_E-S0638] **Automatic texture blending based on slope and altitude — Phase 5I enhancement**
  Kind: implement.

- ✅ [3D_E-S0639] **Terrain collision (character controller walks on terrain surface) — Phase 5I enhancement**
  Kind: implement.

- ✅ [3D_E-S0640] **Triplanar mapping for steep slopes — Phase 5I enhancement**
  Kind: implement.

- 📋 [3D_E-S0641] **Terrain chunking (split large terrains into tiles for streaming and culling) — deferred to a future phase (needed for general-purpose engine use with 4km+ terrains)**
  Kind: implement.
...
- 📋 [3D_E-S0642] **`CameraShakeComponent` attached to the camera rig — drives a per-frame offset to the current CameraMode view matrix without disturbing the underlying camera transform.**
  Kind: implement.

- 📋 [3D_E-S0643] **Shake types: impulse (one-shot peak + decay — weapon kick, footfall), continuous (earthquake, rumble), directional (recoil along a vector), trauma (Dead Space-style gore-cam — amplitude tied to recent damage intake).**
  Kind: implement.

- 📋 [3D_E-S0644] **Parameters: amplitude, frequency, duration, falloff curve — authored via Formula Workbench per CLAUDE.md Rule 6, not hand-coded magic constants.**
  Kind: implement.

- 📋 [3D_E-S0645] **Photosensitive safety: amplitude passes through `clampShakeAmplitude(PhotosensitiveLimits)` at the point of application.**
  Kind: implement.

- 📋 [3D_E-S0646] **Composes with Phase 10.8 Camera Modes — shake is applied post-mode, so first-person / third-person / cinematic / vehicle modes all get shake without per-mode code.**
  Kind: implement.

- 📋 [3D_E-S0647] **Editor preview — a slider in the camera inspector that triggers each shake type for tuning.**
  Kind: implement.
...
- 📋 [3D_E-S0648] **Full-screen overlay pass (post-tonemap, pre-UI) with per-flash colour + alpha + envelope curve.**
  Kind: implement.

- 📋 [3D_E-S0649] **Flash types: hit flash (red, short — damage taken), pickup flash (green, short — item gained), stasis flash (cyan, short — Dead Space stasis cast), screen-wipe transition (any colour, long — scene/chapter change), death fade (black, slow — player death).**
  Kind: implement.

- 📋 [3D_E-S0650] **Envelope authored via Formula Workbench (fade-in / plateau / fade-out curve), not hand-coded timing.**
  Kind: implement.

- 📋 [3D_E-S0651] **Photosensitive safety: peak alpha passes through `clampFlashAlpha(PhotosensitiveLimits)` at upload.**
  Kind: implement.

- 📋 [3D_E-S0652] **Queueable — multiple simultaneous flashes blend additively with per-flash colour and independent envelopes.**
  Kind: implement.

- 📋 [3D_E-S0653] **Integrated with Phase 10.8 Post-Processing Effects Suite as the last overlay before UI compositing.**
  Kind: implement.

##### Save File Compression

- 📋 [3D_E-S0654] **zstd integration — vendored via FetchContent (same mechanism as existing deps). Shared with Phase 12 asset packaging — both consumers use one integration, not two.**
  Kind: implement.

- 📋 [3D_E-S0655] **Compressed binary chunk writer/reader primitive — `writeCompressedChunk(ostream, bytes)` / `readCompressedChunk(istream)` with a versioned header so forward compatibility is explicit.**
  Kind: implement.

- 📋 [3D_E-S0656] **Save-file format header — engine version stamp + scene ID + chunk table + per-chunk zstd payload. Designed so the Phase 11B save/checkpoint system just fills in the chunks.**
  Kind: implement.
...
- 📋 [3D_E-S0657] **Input-recording replay (baseline)**
...
- 📋 [3D_E-S0658] **State-snapshot replay (fallback when determinism is not guaranteed)**
...
- 📋 [3D_E-S0659] **Deterministic-physics contract for input-recording mode**
...
- 📋 [3D_E-S0661] **Behavior tree runtime (tick-based evaluation with running/success/failure states)**
...
- 📋 [3D_E-S0662] **Behavior tree editor — visual node graph in the editor**
...
- 📋 [3D_E-S0663] **Utility AI (optional enhancement) — score-based action selection**
...
- 📋 [3D_E-S0664] **Vision sense — cone-of-vision with configurable angle and range**
...
- 📋 [3D_E-S0665] **Hearing sense — sound propagation through the environment**
...
- 📋 [3D_E-S0666] **Alert states — multi-level awareness**
...
- 📋 [3D_E-S0667] **Weapon component — attach to entity for ranged or melee combat**
...
- 📋 [3D_E-S0668] **Hit detection with per-bone damage zones**
...
- 📋 [3D_E-S0669] **Weapon visual effects**
...
- 📋 [3D_E-S0671] **Weapon upgrade system**
...
- 📋 [3D_E-S0672] **Health component — per-entity health pool with damage/heal methods**
...
- 📋 [3D_E-S0673] **Status effects — all screen-effect hooks consume Phase 10.8 PP suite primitives (vignette, distortion, blur) and Phase 10.8 Decal System**
...
- 📋 [3D_E-S0675] **Player death and respawn**
...
- 📋 [3D_E-S0676] **Inventory component — grid or list-based item storage per entity**
...
- 📋 [3D_E-S0677] **Item pickups — world entities that transfer to inventory on interact**
  - Proximity detection + interact key
  - Pickup animation and sound
  - Item glow/highlight for visibility
  Kind: implement.

- 📋 [3D_E-S0678] **Consumable items — use from inventory to apply effects**
  - Health packs, stim packs, antidotes, shields
  - Cooldown between uses
  Kind: implement.

- 📋 [3D_E-S0679] **Equipment slots — weapon, armor, helmet, accessory**
  - Visual change on equip (swap mesh/texture on character)
  - Stat modifications from equipped items
  Kind: implement.

- 📋 [3D_E-S0680] **Loot tables — configurable random item drops from enemies/containers**
...
- 📋 [3D_E-S0681] **Full world state serialization**
...
- 📋 [3D_E-S0682] **Save file management**
...
- 📋 [3D_E-S0685] **Hazard zones — volumes that apply damage/effects to entities inside**
...
- 📋 [3D_E-S0686] **Hazard visual indicators**
...
- 📋 [3D_E-S0687] **Interactive hazards**
...
- 📋 [3D_E-S0688] **`VehicleComponent` — entity-level wrapper around a Jolt `VehicleConstraint`. Stores chassis body + per-wheel state + drivetrain + engine + aero.**
  Kind: implement.

- 📋 [3D_E-S0689] **Suspension model per wheel — spring stiffness, damping, travel limits, anti-roll bar coupling front/rear; Jolt `WheeledVehicleController` is the baseline.**
  Kind: implement.

- 📋 [3D_E-S0690] **Tyre model — for arcade: simplified grip curve + slip-angle factor. For sim: Pacejka "Magic Formula" longitudinal + lateral with load sensitivity. Per-compound (slick / road / wet / off-road) parameters in `FormulaLibrary`. CPU (per-wheel, ~4/frame); Formula Workbench is the authoring tool (Rule 6).**
  Kind: implement.

- 📋 [3D_E-S0691] **Drivetrain — engine torque curve (RPM → torque), multi-ratio gearbox (manual / automatic / sequential), clutch engagement, centre/front/rear differentials (open / locked / limited-slip / torque-vectoring).**
  Kind: implement.

- 📋 [3D_E-S0692] **Aerodynamics — frontal drag (`0.5·ρ·v²·Cd·A`), lift / downforce per axle with speed-squared scaling, frame-coherent slipstream zone behind each vehicle (reduces drag for trailing cars — matters for both Burnout drafting-boost and GT overtaking).**
  Kind: implement.

- 📋 [3D_E-S0693] **Engine audio — RPM-driven sample interpolation (granular synthesis over recorded low/mid/high-rev samples) + on-throttle / off-throttle crossfade + pitch-shift per gear. Leverages existing `audio_music.{h,cpp}` crossfade primitives. Skid-chirp + backfire + turbo-whistle as separate one-shot layers over the ambient engine bed.**
  Kind: implement.

- 📋 [3D_E-S0694] **Vehicle damage model — two tiers: (a) cosmetic (body deformation via vertex displacement + loose parts fracturing off via the existing Phase 8 destruction system) and (b) functional (per-component health: engine power loss, steering drift, brake fade, tyre punctures, overheating). Configurable per game for arcade vs sim emphasis.**
  Kind: implement.

- 📋 [3D_E-S0695] **Crash cameras — slow-motion hold on high-energy impacts with free-orbit camera (Burnout's "heartbeat" moment after a wreck). Respects `AccessibilitySettings.reducedMotion` — on, the slow-mo is instant-cut instead of 0.1× lerp.**
  Kind: implement.

- 📋 [3D_E-S0696] **Vehicle cameras — Cockpit (interior first-person with dashboard rendering), Chase (third-person behind + slightly above, spring-smoothed), Hood, Bumper, Photo-mode (free-orbit), Cinematic (spline + look-at chase). Composes with the Phase 10.8 Camera Modes system — "vehicle" is a context that provides camera presets, not a new camera class.**
  Kind: implement.

- 📋 [3D_E-S0697] **Force-feedback / steering-wheel support — `InputDevice::SteeringWheel` enum value alongside Keyboard/Mouse/Gamepad. GLFW doesn't expose FFB directly; plug in SDL2's haptic API or libuinput under a thin shim. Axes: steering, throttle, brake, clutch, handbrake, paddle-shift. Button mapping runs through the existing `InputBindings`. Deferred to a follow-up slice but the binding enum goes in day one.**
  Kind: implement.

###### Arcade racing (Burnout 3 / Burnout: Revenge archetype)

- 📋 [3D_E-S0698] **Boost meter — fills from risk behaviour: near-miss (oncoming traffic within N metres), drafting (trailing another vehicle in slipstream for T seconds), drifting (slip-angle exceeds threshold for T seconds), airtime (wheels off ground for T seconds), crash (triggers a *Crashbreaker* bonus in Revenge). Tunable weights per game.**
  Kind: implement.

- 📋 [3D_E-S0699] **Boost activation — throttle multiplier + FOV kick + radial blur + audio filter sweep. Crashbreaker detonates the wrecked vehicle into a directed explosion (leverages Phase 8 destruction).**
  Kind: implement.

- 📋 [3D_E-S0700] **Takedowns — directed collision mechanic. Raycast + impulse-angle check classifies a collision as a takedown when the initiating vehicle is in boost and the struck vehicle loses control. Registers in a "takedowns" score with a short hitstop + camera cut.**
  Kind: implement.

- 📋 [3D_E-S0701] **Traffic AI — civilian vehicles scripted on splined routes with lane-change behaviour, speed variance, and reactive braking when the player is ahead of them. High-density (Burnout city) vs low-density (rural roads) modes configurable per scene.**
  Kind: implement.

- 📋 [3D_E-S0702] **Road Rage / Crash Mode / Burning Lap / Grand Prix game-mode primitives — each is a scoring ruleset + win condition + HUD layout. `GameMode` interface with plug-in rulesets; engine ships the four named modes as starters and games compose their own.**
  Kind: implement.

- 📋 [3D_E-S0703] **Signature Takedown tracking — per-opponent persistent record of the most spectacular takedown cam; viewable from a garage UI.**
  Kind: implement.

###### Simulation racing (Gran Turismo archetype)

- 📋 [3D_E-S0704] **Driving assists toggles — ABS, Traction Control (TCS), Stability Control (ESC / ASM), active steering, braking line overlay. Each on its own slider (Off / Weak / Standard / Strong) not just a binary. Pro preset turns everything off.**
  Kind: implement.

- 📋 [3D_E-S0705] **Tyre wear + thermal model — tyres accumulate wear % per km based on slip energy; grip degrades with wear. Per-compound temperature windows (cold / optimal / overheated) — off-window tyres slide. Feeds strategy (endurance pit-stop timing).**
  Kind: implement.

- 📋 [3D_E-S0706] **Fuel model — consumption per lap scales with throttle position + gear + lean map. Empty tank = engine stall.**
  Kind: implement.

- 📋 [3D_E-S0707] **Pit stops — pit-lane volume detection, configurable service menu (tyres / fuel / mechanical repair / aero adjustments), time cost per service.**
  Kind: implement.

- 📋 [3D_E-S0708] **Vehicle tuning / garage UI — per-car setup: spring rates, damper bump / rebound, anti-roll bar stiffness front/rear, camber, toe, caster, ride height, differential preload / coast / power, gearbox ratios, final drive, wing angles. Each parameter pushes a coefficient into the formula evaluator; the vehicle physics picks it up on next tick. Save setups per-track.**
  Kind: implement.

- 📋 [3D_E-S0709] **Licence tests — structured gameplay challenge harness. A "licence test" is a scripted sequence with start position + win condition + three-star time target. Reusable for driving schools, time trials, mission objectives in any genre.**
  Kind: implement.

- 📋 [3D_E-S0710] **Opponent AI — racing line adherence (spline-based ideal path), corner braking points, overtake decision tree (follow / commit / abort), rubber-banding toggle for arcade modes (disabled by default for sim).**
  Kind: implement.

- 📋 [3D_E-S0711] **Track authoring — spline-based track surface generator with banking, elevation, start/finish + sector markers; surface-material zones (tarmac / kerb / grass / gravel) feeding the tyre model's grip multiplier.**
  Kind: implement.

- 📋 [3D_E-S0712] **Lap / sector timing + timing HUD — live split against best lap + rival + car-ahead / car-behind.**
  Kind: implement.

- 📋 [3D_E-S0713] **Photo Mode — pause simulation, free-orbit camera, aperture / focal length / shutter / ISO controls (plumbed into the existing post-process chain), LUT preset picker, 16:9 / 2.39:1 / square / vertical crop grids, export to PNG with optional film-grain overlay. Respects accessibility (no motion in the viewport when reduced-motion is on).**
  Kind: implement.

###### Testing surface

- 📋 [3D_E-S0714] **Deterministic unit tests for the tyre model against published Pacejka coefficient tables.**
  Kind: implement.

- 📋 [3D_E-S0715] **Per-wheel suspension step against known rebound curves.**
  Kind: implement.

- 📋 [3D_E-S0716] **Takedown classifier unit tests (collision angles + speed thresholds).**
  Kind: implement.

- 📋 [3D_E-S0717] **Opponent AI behaviour harness — scripted scenarios (car ahead braking late, overtaking opportunity on straight) with deterministic PASS/FAIL.**
  Kind: implement.
...
- 📋 [3D_E-S0718] **Kinesis (telekinesis) component — pick-up-and-throw tied to a dedicated input. Grab any `PhysicsBody` tagged `kinesisTarget`; charge launches it with a force magnitude + direction from the crosshair. Used both for combat (throw spikes / limbs) and puzzles (move heavy crates, align circuits). Leverages the existing `physics/grab_system` primitive.**
  Kind: implement.

- 📋 [3D_E-S0719] **Stasis gun + ammo economy — ties the already-shipped `StasisSystem` to a first-class weapon with a rechargeable ammo pool. Recharge stations in the world top it up; trigger slows one tagged body. Slow-mo factor configurable per weapon tier.**
  Kind: implement.

- 📋 [3D_E-S0720] **Zero-G traversal mode — 6DOF navigation (yaw + pitch + roll + three-axis thrust) plus magnetic-boot floor-walking at low thrust. CharacterController gains a `GravityMode` (`Normal / Magnetic / FreeFall`) enum. `Magnetic` lets the player walk arbitrary up-vectors (walls / ceilings) by snapping orientation to the tagged surface normal. `FreeFall` exposes thruster inputs. Scripted transitions between modes via trigger volumes.**
  Kind: implement.

- 📋 [3D_E-S0721] **World-space UI pathway — new `UIElement::worldProjection` support in `engine/ui/ui_in_world.{h,cpp}` (not yet shipped — first bullet under this section). World-space quads can bind to a bone socket so the UI follows the rig at all times. Prerequisite for every other diegetic-UI bullet in this section.**
  Kind: implement.

- 📋 [3D_E-S0722] **Diegetic holographic UI — the Phase 11B UI guidance is that *no HUD is drawn in screen-space for this archetype*. Instead, health / stasis / ammo render as world-space quads projected from the player's spine (RIG) + weapon via the world-space UI pathway above. Accessibility: a fallback 2D HUD can be toggled in `Settings` for partially-sighted players.**
  Kind: implement.

- 📋 [3D_E-S0723] **RIG health spine — specific prefab that reads `HealthComponent` and renders a 5-segment health bar on the player's spine. Third-person visible; in first person only the peripheral glow is shown. Damage pulses the glow.**
  Kind: implement.

- 📋 [3D_E-S0724] **In-world holographic panels — interactable door locks, lore logs, objective markers, and upgrade bench UIs all render as diegetic world-space holograms rather than screen overlays. Consumes the world-space UI pathway defined above. Animated scan-line shader variant + chromatic-aberration flicker (Phase 10.8 PP suite chromatic aberration is the source effect).**
  Kind: implement.

- 📋 [3D_E-S0725] **Necromorph-style encounter AI — behaviour-tree node set for "stalk / ambush / wave-attack / play-dead" patterns. Ambush spawn from vent / ceiling colliders tagged `spawner`; "play dead" nodes use the existing ragdoll system for a faked-death pose and re-animate on trigger. Weak-point targeting — AI prefers to expose limbs to the player so dismemberment is rewarded.**
  Kind: implement.

- 📋 [3D_E-S0726] **Audio horror stingers — scripted jump-scare and tension beats wired through the already-shipped `MusicStingerQueue` and `audio_ambient`. Editor surface: a "Scare Marker" component placed in the scene with a delay / trigger-condition / audio clip triple.**
  Kind: implement.

- 📋 [3D_E-S0727] **Chapter system — narrative-progression scaffold above the save system. A *chapter* is a named subregion with entry/exit triggers, a title-card prefab, and an auto-checkpoint on entry. Saves record `currentChapter` alongside the scene snapshot.**
  Kind: implement.

- 📋 [3D_E-S0728] **Weapon alt-fire modes — most Dead Space weapons have a secondary trigger (Plasma Cutter horizontal ↔ vertical, Line Gun beam ↔ mine). `WeaponComponent` gets a `secondaryFire` sibling to `fire` with its own stats + input binding (default: right-click in ADS-off state, toggle weapon orientation in ADS-on).**
  Kind: implement.

- 📋 [3D_E-S0729] **Ammo / health scarcity tuning helpers — survival-horror economy depends on drops being rare but not *too* rare. Editor tool: simulated playthrough harness that reports expected ammo / health surplus per chapter against a target curve, so designers can retune loot tables without full playthroughs.**
  Kind: implement.

- 📋 [3D_E-S0730] **Co-op session support (Dead Space 3 archetype) — scope note: the generic networking primitives live in Phase 20. This bullet just calls out that the horror-specific design — divergent player perspectives, co-op-only encounters, revive mechanic — needs per-scene authoring and a split-inventory flag once Phase 20 lands. Not shippable before Phase 20.**
  Kind: implement.
...
- 📋 [3D_E-S0731] **Replay playback controls**
...
- 📋 [3D_E-S0732] **Ghost / split-time overlays (racing-specific)**
...
- 📋 [3D_E-S0733] **Replay editor integration**
...
- 📋 [3D_E-S0735] **Node-based visual scripting system (connect trigger → action blocks in the editor)**
  Kind: implement.

- 📋 [3D_E-S0736] **Event triggers: player enters area, player looks at object, player presses interact key, timer**
  Kind: implement.

- 📋 [3D_E-S0737] **Actions: open/close door, play sound, show text, move object, toggle light, teleport player**
  Kind: implement.

- 📋 [3D_E-S0738] **Waypoint system — define guided tour paths the player can follow**
  Kind: implement.

- 📋 [3D_E-S0739] **Trigger zones — invisible volumes that fire events when the player enters**
  Kind: implement.

##### Simple Behaviors

- 📋 [3D_E-S0740] **Animated objects (rotating, bobbing, swinging — set in the inspector)**
  Kind: implement.

- 📋 [3D_E-S0741] **Door component (opens on interact, with animation)**
  Kind: implement.

- 📋 [3D_E-S0742] **Torch component (auto-generates fire particles + flickering light)**
  Kind: implement.

- 📋 [3D_E-S0743] **Water component (auto-generates water surface shader + sound)**
  Kind: implement.

##### AI and Navigation

- 📋 [3D_E-S0744] **Navigation mesh generation (automatic walkable surface detection from scene geometry)**
  Kind: implement.

- 📋 [3D_E-S0745] **NavMesh pathfinding (A* or similar on the navigation mesh)**
  Kind: implement.

- 📋 [3D_E-S0746] **NPC component (entity that follows paths, plays animations, responds to triggers)**
  Kind: implement.

- 📋 [3D_E-S0747] **NPC patrol routes (editor-defined waypoint sequences)**
  Kind: implement.

- 📋 [3D_E-S0748] **Crowd simulation (multiple NPCs navigating without colliding — Temple visitors, priests)**
  Kind: implement.

- 📋 [3D_E-S0749] **Line-of-sight checks (NPCs react to player visibility)**
  Kind: implement.
...
- 📋 [3D_E-S0750] **Tension tracking — measure player's current stress level**
...
- 📋 [3D_E-S0751] **Dynamic spawn control**
...
- 📋 [3D_E-S0752] **Encounter scripting**
...
- 📋 [3D_E-S0754] **Camera track editor — keyframed camera paths with easing**
...
- 📋 [3D_E-S0755] **Character animation sequencing**
...
- 📋 [3D_E-S0757] **Seamless transitions**
...
- 📋 [3D_E-S0758] **Scripted events — triggered sequences without full cutscenes**
...
- 📋 [3D_E-S0759] **XPBD cloth particle ↔ Jolt rigid body kinematic attachment (curtains pinned to moving poles)**
  Kind: implement.

- 📋 [3D_E-S0760] **Inextensible tether / distance-max constraint with tagged-union endpoints (particle / rigid body / static anchor)**
  Kind: implement.

- 📋 [3D_E-S0761] **Slider-ring authoring on top of the existing `Jolt::SliderConstraint` wrapper (bronze rings on acacia poles)**
  Kind: implement.

- 📋 [3D_E-S0762] **Fixed-joint authoring for pillar-to-socket and crossbar-to-pillar (existing wrapper, new UI)**
  Kind: implement.

- 📋 [3D_E-S0763] **Editor attachment panel: pin-to-body / slider-ring / tether picker with vertex-picker gizmo**
  Kind: implement.

- 📋 [3D_E-S0764] **Scene-serialization round-trip for attachments + tethers**
  Kind: implement.

- 📋 [3D_E-S0765] **Formula Workbench entries for every new tuning coefficient (ring friction, tether compliance, pendulum damping, cord tensile modulus) — per CLAUDE.md Rule 6**
  Kind: implement.

- 📋 [3D_E-S0766] **Tabernacle structural pass: re-rig the demo scene so nothing hangs in mid air (48 boards + 5 bars/side + 10 inner curtains + 11 goat-hair curtains + 2 coverings + veil + screen + 60 outer pillars + linen walls + tent-pegs-and-cords)**
  Kind: implement.
...
- 📋 [3D_E-S0767] **`localtime_r` → `localtime_s` on MSVC (frame_diagnostics.cpp, visual_test_runner.cpp, recent_files.cpp)**
  Kind: implement.

- 📋 [3D_E-S0768] **`getenv("HOME")` → `getenv("USERPROFILE")` on Windows (frame_diagnostics.cpp, window.cpp)**
  Kind: implement.

- 📋 [3D_E-S0769] **`/proc/self/status` memory tracking → Windows API equivalent (memory_tracker.cpp)**
  Kind: implement.

- 📋 [3D_E-S0770] **ASAN/UBSAN flags: MSVC uses `/fsanitize=address` syntax, GCC/Clang use `-fsanitize=` (CMakeLists.txt)**
  Kind: implement.

- 📋 [3D_E-S0771] **`_FORTIFY_SOURCE` is GCC/Clang-only — skip on MSVC (CMakeLists.txt)**
  Kind: implement.

- 📋 [3D_E-S0772] **Path separator handling (`/` vs `\`) — use `std::filesystem::path` where needed**
  Kind: implement.

- 📋 [3D_E-S0773] **Verify `std::filesystem` links without `-lstdc++fs` on target compilers (GCC < 9 needs it)**
  Kind: implement.

###### Windows Build Setup

- 📋 [3D_E-S0774] **Verify CMake generates valid MSVC / Ninja build on Windows**
  Kind: implement.

- 📋 [3D_E-S0775] **Test with Visual Studio 2022 (MSVC 17) and MinGW-w64**
  Kind: implement.

- 📋 [3D_E-S0776] **Resolve any GLFW / OpenGL context differences on Windows (driver-specific)**
  Kind: implement.

- 📋 [3D_E-S0777] **Verify all FetchContent dependencies build cleanly on MSVC**
  Kind: implement.

- 📋 [3D_E-S0778] **Windows-specific icon and manifest for the executable**
  Kind: implement.

###### CI / CD Pipeline

- 📋 [3D_E-S0779] **GitHub Actions workflow: Linux build (GCC + Clang)**
  Kind: implement.

- 📋 [3D_E-S0780] **GitHub Actions workflow: Windows build (MSVC)**
  Kind: implement.

- 📋 [3D_E-S0781] **Automated test run on both platforms (unit tests + visual test runner)**
  Kind: implement.

- 📋 [3D_E-S0782] **Build artifacts: downloadable binaries for both platforms per commit/tag**
  Kind: implement.

- 📋 [3D_E-S0783] **Release packaging script (zip/tar with executable + assets)**
  Kind: implement.

- 📋 [3D_E-S0784] **Local "pre-push CI" script — run what CI runs, before pushing. A single**
...
- 📋 [3D_E-S0785] **MinGW-w64 toolchain file for building Windows binaries from Linux**
  Kind: implement.

- 📋 [3D_E-S0786] **Verify GLFW cross-compiles cleanly with MinGW**
  Kind: implement.

- 📋 [3D_E-S0787] **Test cross-compiled binary on Windows (or Wine)**
  Kind: implement.

##### Asset Pipeline

- 📋 [3D_E-S0788] **Texture compression (BC7/KTX2 for desktop, ASTC for mobile — compress on import, load directly to GPU)**
  Kind: implement.

- 📋 [3D_E-S0789] **Texture channel packing + material atlasing (pack occlusion / roughness / metallic into a single ORM texture on import, and atlas small material textures to cut sampler binds and draw calls — distinct from the shipped 2D sprite atlas, which handles TexturePacker-JSON sprite sheets)**
  Kind: implement.

- 📋 [3D_E-S0790] **Automatic mipmap generation with quality filtering options**
  Kind: implement.

- 📋 [3D_E-S0791] **Asset cooking / baking (preprocess models, textures, shaders into optimized binary format)**
  Kind: implement.

- 📋 [3D_E-S0792] **Asset manifest and dependency tracking (know exactly what each scene needs)**
  Kind: implement.

- 📋 [3D_E-S0793] **Hot-reload during development (detect changed assets, reload without restarting)**
  Kind: implement.
...
- 📋 [3D_E-S0795] **GPU texture compression pipeline (BC7/BC1 via KTX2 — ~4x VRAM reduction, faster loads)**
  Kind: implement.

- 📋 [3D_E-S0796] **Asset packaging (bundle assets into compressed archives with zstd/LZ4 for distribution)**
  Kind: implement.

- 📋 [3D_E-S0797] **Animation data compression (16-bit half-float positions, smallest-3 quaternion encoding)**
  Kind: implement.

- 📋 [3D_E-S0798] **Mesh compression (Draco or meshoptimizer for glTF geometry data)**
  Kind: implement.

- 📋 [3D_E-S0799] **Strip source assets from builds (exclude .blend, raw PSD/EXR source files)**
  Kind: implement.

- 📋 [3D_E-S0800] **Binary size optimization (link-time optimization, dead code stripping for release builds)**
  Kind: implement.

##### Application Distribution

- 📋 [3D_E-S0801] **Steam SDK integration**
  Kind: implement.

- 📋 [3D_E-S0802] **Steam achievements (if applicable)**
  Kind: implement.

- 📋 [3D_E-S0803] **Installer/packaging for Windows**
  Kind: implement.

- 📋 [3D_E-S0804] **Linux AppImage or Flatpak packaging**
  Kind: implement.

- 📋 [3D_E-S0805] **Save/load system (player position, settings)**
  Kind: implement.

- 📋 [3D_E-S0806] **Controller button prompts (show correct icons for connected controller)**
  Kind: implement.

- 📋 [3D_E-S0807] **Loading screens (with progress indicators for large scenes)**
  Kind: implement.

##### Scene Packaging and Sharing

- 📋 [3D_E-S0808] **Export scene as standalone package (scene + all referenced assets in one archive)**
  Kind: implement.

- 📋 [3D_E-S0809] **Import packaged scenes from other creators**
  Kind: implement.

- 📋 [3D_E-S0810] **Scene versioning — track which engine version a scene was built with**
  Kind: implement.
...
- 📋 [3D_E-S0811] **New Project wizard — step-by-step project setup**
...
- 📋 [3D_E-S0812] **Scene setup assistant — guided scene creation within the wizard**
...
- 📋 [3D_E-S0813] **Gameplay mode selector — choose what kind of experience to build**
...
- 📋 [3D_E-S0814] **Player setup — configure the player character**
...
- 📋 [3D_E-S0815] **Audio setup assistant**
...
- 📋 [3D_E-S0816] **Platform targeting — select and configure target platforms**
  - Linux (AppImage, Flatpak)
  - Windows (standalone .exe, Steam-ready)
  - Steam integration checklist (achievements, cloud saves, overlay)
  Kind: implement.

- 📋 [3D_E-S0817] **Quality presets — configure rendering for target hardware**
...
- 📋 [3D_E-S0818] **Asset validation and optimization**
...
- 📋 [3D_E-S0819] **Final build process**
...
- 📋 [3D_E-S0820] **Third-party clean-clone build validation. The 2026-04-15 dry-run was maintainer-performed; an external "first contact with the README" path is still unexercised. First community PR or Discussions thread that reports a successful build closes this item; no active work required.**
  Kind: implement.

- 📋 [3D_E-S0821] **Biblical content migration to private `Tabernacle` repo. `assets/textures/tabernacle/` and the tabernacle-loading scene code are currently local-only (gitignored). Cleaner long-term home is a private GitHub repo so the maintainer can sync development across machines. Not blocking engine work.**
  Kind: implement.

- 📋 [3D_E-S0822] **Trademark decision on the "Vestige" name — informal use vs formal registration. Deferred until there's something worth protecting at scale.**
  Kind: implement.
...
- ✅ [3D_E-S0824] **Sprite renderer (textured quads with z-ordering, tint, and flip) — shipped in Phase 9F-1 (`engine/renderer/sprite_renderer.{h,cpp}`; flip on `SpriteComponent`).**
  Kind: implement.

- ✅ [3D_E-S0825] **Sprite atlas / batch renderer (minimize draw calls — single VBO for all sprites) — shipped in Phase 9F-1 (`engine/renderer/sprite_atlas.{h,cpp}`, instance-rate VBO).**
  Kind: implement.
...
- ✅ [3D_E-S0834] **2D rigid body component (Box2D or custom — position, rotation, velocity) — shipped in Phase 9F-2 (`engine/systems/physics2d_system.{h,cpp}`) — Jolt `Plane2D`, not a second engine.**
  Kind: implement.
...
since roughly 2024 in techniques Vestige already ships or plans to build.

Findings are recorded here with their evidence so a later session can re-check
them rather than re-research them. Every item states whether it is adopt,
prototype, watch or reject, and why. Nothing here is committed work: the roadmap
status is the commitment, and most of these are deliberately filed as
considered.

Licensing note: nothing in this section is legal advice. Where an item turns on
...
- 💭 [3D_E-0665] **World-space GI without ray-tracing hardware: AMD Brixelizer GI.**
  ...
  What it would fix is the gap the current screen-space GI leaves --
  Brixelizer is world-space, so off-screen geometry contributes. AMD's GDC
  2024 material states it requires no hardware ray tracing, is MIT, and
  costs roughly 1.7 ms at 1080p on an RX 7600 XT at its default 0.5x
  internal scale, with a 128 MB SDF atlas plus ~110 MB at 1080p/0.5x.

  Three honest obstacles. There is NO OpenGL backend -- DX12 and Vulkan only
  -- so it is a shader-translation port of the kind the FSR2 OpenGL port
  proved one person can do, not a drop-in. The RX 6600 is materially slower
  than an RX 7600 XT, so scale that 1.7 ms upward; against a frame with
  roughly 4.9 ms of headroom, it fits only if SSGI is retired or shrunk at
  the same time. And AMD state its radiance cache is filled from
  screen-space information, so surfaces must have been in view before their
  bounced colour is available -- it composes with SSAO/SSGI rather than
  replacing them.

  On a GTX 1050 this is a preset-off feature.

  LICENCE CORRECTION (verified 2026-09-02): "is MIT" holds only at
  FidelityFX-SDK tag v1.1.4, where ffx_brixelizer.h, ffx_brixelizer_raw.h
  and ffx_brixelizergi.h sit under a plain MIT docs/license.md. The CURRENT
  SDK main no longer lists Brixelizer among its effects at all and has no
  brixelizer directory, and its default licence is binary-only
  redistribution (see 3D_E-0659). So the pin would be tag v1.1.4, with the
  rule-8 reason recorded at the pin site: later versions dropped the effect
  and changed the default licence.
  **Layman:** A way to get bounced light from things that are off-screen, on a graphics card with no ray-tracing.
  Kind: investigate.
  Source: tech-survey-2026-09-02.
  Lanes: renderer.

- 💭 [3D_E-0666] **Radiance cascades — watch, and do not spend a slice on it yet.**
  Recorded so a later session does not re-litigate the hype.

  The published work with performance figures is 2D only (Holographic
  Radiance Cascades, arXiv 2505.02041). The first credible world-space 3D
  formulation, Split Radiance Cascades (arXiv 2607.20384, 2026-07-22),
  publishes no performance figures, no GPU, no memory and no code -- treat
  its cost as unknown rather than as good. Path of Exile 2 ships it in
  screen space, exploiting a fixed camera.

  Vestige's case is the worst one for it as published: first-person, free
  camera, sunlit architectural exteriors. The only measured quality report
  on that scenario (Gerassimov, DiGRA) found a "pronounced bloom-like
  effect ... not well-suited for physically plausible daylight
  illumination" outdoors and "excessive visual noise and blur" indoors at
  close range, with temporal stability rated medium -- from a single
  measurement run per condition, which the author states.

  Every open 3D implementation is pre-production; one OpenGL dissertation
  reports the naive grid "was not fast enough for real time".

  Re-evaluate when Split RC publishes timings or reference code.
  **Layman:** A much-discussed new lighting technique that does not suit a first-person outdoor game yet.
  Kind: investigate.
  Source: tech-survey-2026-09-02.
  Lanes: renderer.

- 💭 [3D_E-0667] **Analytic fog primitives for localised unlit fog the froxel grid resolves poorly.**
  Closed-form integrals of radial density functions bounded by spheres,
  boxes and planes, with shader code published (Matej Lou, 2025-02-11).
  Never aliases, costs a handful of ALU.

  The author states plainly that it solves neither scattering nor shadowing
  analytically, so this is a COMPLEMENT to the froxel volume rather than a
  replacement: use it for localised unlit set dressing -- incense, a dust
  shaft, courtyard mist -- that a 64-slice grid resolves badly.

  Low priority. Filed so the option is on record.
  **Layman:** A cheap exact way to draw a shaft of dust or a pocket of mist, where the current grid is too coarse.
  Kind: enhancement.
  Source: tech-survey-2026-09-02.
  Lanes: renderer.

- 💭 [3D_E-0668] **Concurrent binary trees for terrain — watch; CDLOD is not currently a bottleneck.**
  Benyoub and Dupuy (Intel), HPG 2024, arXiv 2407.02215: bisection
  tessellation extended from square domains to arbitrary polygon meshes,
  reporting triangulation in under 0.2 ms on console-level hardware.

  An OpenGL route exists and is permissively licensed -- jdupuy/libcbt is
  dual MIT/Unlicense, with libleb on top and LongestEdgeBisectionDemos
  providing a GPU-driven GLSL terrain renderer. The paper's own reference
  repo is DX12/SM6.6 with an unstated licence.

  Watch rather than adopt: CDLOD ships and works, and CBT's win is planetary
  scale and arbitrary base meshes, neither of which a Tabernacle courtyard
  needs. Revisit only if terrain appears in the GPU timers, or if
  displacement on non-heightfield surfaces is wanted.

  Recorded alongside a negative finding worth keeping: a search of grass and
  impostor literature from 2024 onward found nothing that improves on what
  the engine already ships. That material is either mesh-shader-based
  (unavailable on AMD OpenGL) or restates technique already in place. ~1M
  Bezier blades at ~1.2 ms is competitive with anything published, and the
  vegetation cost worth attacking is its SHADOW cost, which the shadow
  bullet owns.
  **Layman:** A newer terrain technique that would only pay off if we needed planet-scale ground.
  Kind: investigate.
  Source: tech-survey-2026-09-02.
...
- ✅ [3D_E-0669] **tinyexr is pinned below two published memory-safety fixes.**
  ...
  2026-09-02 against the upstream releases page.

  EXR parsing is an attacker-reachable surface the moment an asset is loaded
  from outside the repo, so this is a real exposure rather than hygiene.

  Take v1.0.13, NOT the latest. v1.0.9 to v1.0.13 is patch-level within the
  same 1.x API and should be a one-line pin change. The current latest is
  v3.2.0 (2026-07-08), which is a major API restructure with no migration
  guide in its notes plus a texture toolchain we do not need -- that is a
  separate, scheduled item and must not be bundled with a security patch.

  Do this first among the dependency items.
  **Layman:** The library that reads EXR images has two known memory bugs fixed upstream that we have not taken.
  Kind: security.
  Lanes: deps, security.
  Source: tech-survey-2026-09-02.
...
- 💭 [3D_E-0675] **Correct the record on why Steam Audio was rejected — licensing was not a valid reason.**
  ...
  the record: Steam Audio is Apache-2.0, fully open source, and ships a
  first-class standalone C API whose own documentation names a custom game
  engine as a target. Unity/Unreal/FMOD/Wwise are integrations on top of it,
  not prerequisites.

  The SCOPE half holds, and holds strongly. Vestige has already shipped its
  own volumetric ray occlusion, its own convolution reverb on OpenAL, and
  its own image-source plus Sabine-tail pre-bake. Adopting Steam Audio now
  would replace three shipped subsystems rather than fill a gap, and would
  mean either routing its output through OpenAL as a raw buffer feed --
  giving up OpenAL's own HRTF and effect chain -- or running two
  spatialisers.

  The ONE capability it has that Vestige does not is PATHING: propagation
  and diffraction of sound around corners along multiple paths. That is
  genuinely hard to clean-room and is the only credible trigger to reopen.
  If pathing ever becomes required, budget 2-4 weeks to integrate the C API
  for pathing alone, and note the hybrid is awkward because pathing wants
  its own scene representation and probe bake.

  Maintenance risk to weigh: no release since v4.8.1 (2026-02-11) and no
  commit since 2026-03-25.

  Filed as considered because the action is a record correction plus a
  stated reopen trigger, not an adoption.
...
- 💭 [3D_E-0677] **Offer a user-selectable HRTF, since head-related transfer functions are individual.**
  OpenAL-soft's default HRTF is derived from the SADIE II data set, subject
  D1 (University of York). HRTF fit is highly individual, so a per-user
  choice measurably improves localisation -- which is an accessibility win
  as much as a fidelity one.

  The mechanism already exists and costs nothing at runtime. OpenAL's native
  format is MHR, and its utils/ directory ships makemhr plus SOFA support
  (sofa-info, sofa-support) with dataset definitions for CIAIR, IRC_1005
  (IRCAM LISTEN), MIT_KEMAR and SCUT_KEMAR. So the work is an offline SOFA
  to MHR conversion plus a file picker in the audio settings.

  LICENCE CAUTION, and it is the real cost: HRTF datasets each carry their
  own terms and several academic sets are CC-BY-NC or research-only. Check
  every dataset before shipping it. The SADIE II default is already
  distributed by OpenAL-soft, so that one is settled.

  Budget a day or two, most of it licence checking.
  **Layman:** Let players pick the 3D-audio profile that suits their ears — one size genuinely does not fit all.
  Kind: accessibility.
  Source: tech-survey-2026-09-02.
  Lanes: audio.

- 💭 [3D_E-0678] **Virtualised geometry is clear of Epic, but NVIDIA has live filings — and the licence you pick decides your exposure.**
  NOT LEGAL ADVICE. This is a documentary survey; a real freedom-to-operate
  opinion needs a patent attorney. Patents are jurisdictional and
  time-limited, and everything below is US-centric.

  EPIC: no findable patent stands in the way. Epic's entire enumerated US
  portfolio is 33 documents and not one concerns mesh LOD, clustering,
  geometry streaming or virtualised geometry; the only graphics-adjacent
  titles are the MetaHuman/Cubic Motion line. A full-text search for
  "virtualized geometry" across all assignees returns no Epic result. So
  Nanite anxiety should not block Phase 14.

  THE STATED GAP, because an absence proved this way is weaker than it
  looks: assignee search is assignee-at-issue, so a patent assigned to Epic
  after grant or filed under another corporate name (Epic Games
  International, RAD Game Tools, Quixel, Cubic Motion, 3Lateral) would not
  surface, and applications under 18 months old are unpublished and
  invisible to any search.

  NVIDIA IS THE PARTY WITH LIVE CLAIMS. US 2026/0187903 A1 and US
  2026/0187924 A1, both filed 2024-12-31 and published 2026-07-02, claim the
  cluster-grouping step directly -- scoring candidate triangle groupings by
  a surface area heuristic plus a cut cost based on connections crossing
  between groups. Both are published APPLICATIONS, not granted patents, and
  claims routinely narrow during prosecution; grant status was not checked
  and must be re-checked before shipping. Also live: US 2025/0117964 A1
  (Six Impossible Things Before Breakfast Ltd) on decimation-driven
  continuous per-vertex LOD.

  THE ACTIONABLE ASYMMETRY, and it changes 3D_E-0661. MIT and BSD grant
  copyright rights only and carry NO express patent licence. Apache-2.0
  section 3 grants an irrevocable patent licence from each contributor.
  nvpro-samples/vk_lod_clusters is authored by NVIDIA and licensed
  Apache-2.0 -- so mirroring its cluster-grouping cost function carries a
  patent grant from precisely the entity whose filings are the concern,
  where reimplementing the same SAH-plus-cut-cost scheme from the MIT code
  does not. Do that for the grouping heuristic specifically, keep the
  Apache-2.0 NOTICE, and declare it in THIRD_PARTY_NOTICES.md as a second
  licence alongside the MIT tree.

  Helpful but not immunity: meshoptimizer's cluster builder predates the
  2024-12-31 priority date, so work built on it sits behind prior art.
  **Layman:** Nobody appears to own the patent we feared. The one company with pending claims gives away a patent licence with its own code, so we should copy that one.
  Kind: research.
  Source: tech-survey-2026-09-02.
  Lanes: renderer, legal.

- 💭 [3D_E-0679] **What the IP survey could NOT clear, recorded so nobody mistakes silence for a pass.**
  NOT CLEARED: screen-space ambient occlusion, temporal anti-aliasing, and
  cascaded shadow maps -- all three of which Vestige already ships.

  The survey found NO evidence of encumbrance on any of them and no public
  enforcement history against an engine developer. All three are published
  academic or industry techniques: shadow-map cascades from the 2006
  literature, SSAO from Crytek's 2007 publication, TAA from the 2010s
  reprojection work.

  But that is an ABSENCE OF EVIDENCE, NOT EVIDENCE OF ABSENCE, and the
  reason is mechanical rather than reassuring: the only search host that
  returns assignees in a result list started refusing requests partway
  through, and the fallback host omits the assignee field, so its result
  counts prove nothing about who holds what. Treat all three as unresolved.
  Re-run this before any commercial release, and get counsel rather than
  another survey.

  NOT INVESTIGATED AT ALL: ASTC / PVRTC / ETC texture compression, ray
  tracing (relevant to the planned RT work), Vulkan and mesh-shader
  patents, and the entire audio side.

  CLEARED, for the record. S3TC/DXTn: US 5,956,431 (S3 Incorporated, filed
  1997-10-02) expires 2017-10-02 by the 20-year rule -- arithmetic, since no
  legal-status record could be reached, so the date is computed rather than
  confirmed. Depth-fail stencil shadow volumes ("Carmack's Reverse"): US
  6,384,822 (Creative Technology, filed 1999-10-13) expired around
  2019-10-13 on the same basis. The second frees a technique Vestige has no
  reason to want -- CSM plus contact shadows already ship -- and is recorded
  only because it is the one clearly-identified graphics patent in the
  survey whose expiry frees anything real.
  **Layman:** Three techniques we already use were not checked successfully; that is not the same as them being safe.
  Kind: research.
  Source: tech-survey-2026-09-02.
  Lanes: legal.

- 💭 [3D_E-0680] **Two adoption candidates carry licence conditions that must reach THIRD_PARTY_NOTICES.md.**
  Recorded now so the conditions are not discovered at vendoring time.

  EA's importance-sampled FAST noise (3D_E-0660) is a BSD-3-Clause VARIANT,
  not plain BSD. It adds a fourth clause restricting EA's and SEED's marks
  and logos, which are "distributed with this software solely for
  demonstration purposes" and may not be displayed outside unmodified
  redistribution. No non-commercial clause and no field-of-use limit, so it
  is adoptable -- but strip any EA/SEED branding assets before vendoring and
  reproduce clause 4 in THIRD_PARTY_NOTICES.md.

  SGSR 2 (3D_E-0659) is plain BSD-3-Clause, "Copyright (c) 2023, Qualcomm
  Innovation Center, Inc.", with no additions; SGSR 2 lives under sgsr/v2/
  with no separate licence file, so the root licence governs. Worth knowing
  rather than acting on: BSD-3-Clause grants no express patent licence, and
  Qualcomm is an active patent holder -- the same asymmetry 3D_E-0678
  describes for the geometry work, without an Apache-2.0 alternative to
  reach for.
