...
- [ ] **AX5. Audio LOD ladder** — distant sources downgrade through tiers: full-spatial + HRTF + occlusion → cheap-spatial (panning only) → 2D drop → mute. Tier picked by distance + occlusion + bus priority. Today every active source pays full cost; this halves CPU on dense-source scenes (battlefield / market / crowd). Cheap to implement against the existing `AudioSourceComponent` + `AudioMixer`.
- [ ] **AX6. Air absorption + atmospheric filtering** — distance-based high-frequency rolloff per ISO 9613-1 (free standard summary; open implementation in Steam Audio). Currently distance only attenuates *gain*; air physically also attenuates *high frequencies*, especially over outdoor distances. Cheap one-pole low-pass per source whose cutoff is a function of `(distance, humidity, temperature)` — pulls from the existing `EnvironmentForces` query API for weather coupling. Massive perceived realism for outdoor scenes.
...
- [ ] **AX8. Surround output (5.1 / 7.1)** — OpenAL Soft already supports surround; we just don't expose channel-config selection. Add `Settings → Audio → Output channel layout` (Auto / Stereo / Headphones+HRTF / 5.1 / 7.1) with auto-detection probing `ALC_FORMAT_CHANNELS_SOFT`. Free once exposed.
